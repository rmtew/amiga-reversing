...
def test_tool_registry_cli_inventory_capability_and_configure_path(monkeypatch, capsys) -> None:
    ...
    def route(method: str, path: str, query, body=None):
        ...
        if path == "/api/tools/configuration/path":
            return {
                "ok": True,
    ...
    assert tool_registry.main(["configure-path", "functional", "vasm", "tools/vasm"]) == 0
    ...
    assert calls == [
        ...
        ("POST", "/api/tools/configuration/path", {}, {"kind": "functional", "tool_id": "vasm", "path": "tools/vasm"}),
    ]


def test_tool_registry_cli_project_capabilities(monkeypatch, capsys) -> None:
    def route(method: str, path: str, query, body=None):
        assert method == "GET"
        assert path == "/api/projects/bloodwych/tool-capabilities"
        assert query == {"profile_id": ["source-devpac"]}
        return {"ok": True, "data": {"capabilities": [{"tool_id": "genam", "status": "missing", "missing_runtime_ids": ["vamos"]}]}}

    monkeypatch.setattr(tool_registry.server, "route_request", route)

    assert tool_registry.main(["project-capabilities", "bloodwych", "--profile-id", "source-devpac"]) == 0
    assert json.loads(capsys.readouterr().out)["capabilities"][0]["missing_runtime_ids"] == ["vamos"]
