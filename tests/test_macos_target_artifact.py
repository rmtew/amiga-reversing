...
from amiga_reversing.disasm import macos_listing_source, macos_target_artifact
...
def test_committed_macos_example_target_loads_through_project_record() -> None:
    ...
    assert project.parent_project_id == "macos_hfs_mpw_gm"
    assert project.origin["kind"] == "macos_hfs_resource_code_file"
...
def test_macos_example_asm_renderer_accepts_compact_source_packet(monkeypatch: pytest.MonkeyPatch) -> None:
    code_payloads = {
        0: bytes.fromhex("00000af00000392000000ad00000002000043f3c0001a9f0"),
        1: bytes.fromhex("205f4e75"),
    }
    ...
    def fake_extract(
        ...
        **kwargs: object,
        ...
        assert hfs_bytes == b"hfs"
        assert hfs_path == "MPW-GM/MPW/Tools/Asm"
        assert kwargs.get("project_root") == macos_target_artifact.PROJECT_ROOT
        return code_payloads[resource_id]

    monkeypatch.setattr(
        macos_target_artifact,
            ...
            "source_body_sections": [
                {
                    ...
                    "code0_structured_context": {
                        "jump_table": {"start": 16, "end": 24, "entry_size": 8},
                        "jump_table_rows": [
                            {
                                "entry_index": 0,
                                ...
                                "platform_call": {"symbol_name": "_LoadSeg"},
                            }
                        ],
                    },
                },
                {
                    ...
                    "name": "Main",
                    "label": "CODE_1",
                    "source_body_ranges": [{"kind": "code", "start": 0, "end": 4, "size": 4}],
                    "semantic_source": {
                        "asm_source_includes": ["SegLoad.a"],
                        "rows": [
                            {"kind": "label", "payload_offset": 0},
                            {
                                "kind": "instruction",
                                "payload_offset": 0,
                                "payload_end": 2,
    ...
    source_text = macos_target_artifact.render_macos_example_asm_from_payload(payload, hfs_bytes=b"hfs")

    assert '\tINCLUDE "SegLoad.a"' in source_text
    assert "CODE_0_above_a5_size:\n\tdc.l $00000AF0" in source_text
    assert "CODE_0_jump_table_entry_0:\n\tdc.w $0004\n\tmove.w #1,-(a7)\n\t_LoadSeg" in source_text
    assert "CODE_1:\nCODE_1_loc_00000000:\n\tmovea.l (a7)+,a0\n\trts" in source_text
    assert ";   source_section_id: " not in source_text
...
def test_committed_macos_subtarget_metadata_and_asm_shape() -> None:
    ...
    assert asm_text.startswith("; Generated by amiga-reversing source export\n")
    assert f"; Target: {MACOS_EXAMPLE_PROJECT_ID}\n" in asm_text.split("\n\n", 1)[0]
    assert metadata["origin"]["parent_project_id"] == MACOS_EXAMPLE_CONTAINER_PROJECT_ID
