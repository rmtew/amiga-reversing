...
from amiga_reversing.disasm.amiga_metadata import ResidentAutoinitMetadata
...
class FileImportTargetInfo:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, object]) -> FileImportTargetInfo:
        target_type = payload["target_type"]
        target_metadata = payload["target_metadata"]
        source = payload.get("source")
        entry_path = payload.get("entry_path")
        local_target_id = payload.get("local_target_id")
        assert isinstance(target_type, str)
        assert entry_path is None or isinstance(entry_path, str)
        assert local_target_id is None or isinstance(local_target_id, str)
        return cls(
            target_type=target_type,
            target_metadata=_json_object(target_metadata),
            source=None if source is None else _json_object(source),
...
class FileContentInfo:
    ...
    import_target: FileImportTargetInfo | None = None
    ...
    def from_dict(cls, payload: dict[str, object]) -> FileContentInfo:
        ...
        import_target = payload.get("import_target")
...
class BootloaderReadSetup:
    ...
    dsklen_dma_byte_length: int | None = None
    dsklen_dma_enabled: bool = False
    dsklen_write: bool = False
    ...
    def from_dict(cls, payload: dict[str, object]) -> BootloaderReadSetup:
        ...
        dsklen_dma_byte_length = payload.get("dsklen_dma_byte_length")
        dsklen_dma_enabled = payload.get("dsklen_dma_enabled", False)
        dsklen_write = payload.get("dsklen_write", False)
        ...
        assert dsklen_dma_byte_length is None or isinstance(dsklen_dma_byte_length, int)
        assert isinstance(dsklen_dma_enabled, bool)
        assert isinstance(dsklen_write, bool)
        ...
        return cls(
            ...
            dsklen_dma_byte_length=dsklen_dma_byte_length,
            dsklen_dma_enabled=dsklen_dma_enabled,
            dsklen_write=dsklen_write,
