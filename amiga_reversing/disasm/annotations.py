...
from amiga_reversing.disasm.entity_types import EntityRecord
...
def _parse_entity_addr(addr: str) -> int:
    try:
        return int(addr, 0)
    except ValueError:
        return int(addr, 16)
...
def get_entities_by_int_addr(project_name: str, project_root: Path = PROJECT_ROOT) -> dict[int, EntityRecord]:
    ...
    for entity in _load_entities_by_addr(entities_path).values():
        ...
        result[_parse_entity_addr(entity["addr"])] = merged
    return result
...
def get_entity(project_name: str, addr: str, project_root: Path = PROJECT_ROOT) -> EntityRecord:
    ...
    if entity is None:
        wanted_addr = _parse_entity_addr(addr)
        entity = next((candidate for candidate in entities.values() if _parse_entity_addr(candidate["addr"]) == wanted_addr), None)
    if entity is None:
