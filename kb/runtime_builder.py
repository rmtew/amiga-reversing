...
def _render_os_struct_field(field: OsStructField) -> str:
    parts = [
        ...
        f"size={field['size']!r}",
        f"available_since={field['available_since']!r}",
    ]
    if "names_by_version" in field:
        parts.append(f"names_by_version={dict(sorted(field['names_by_version'].items()))!r}")
...
def _render_os_struct(struct_def: OsStructDef) -> str:
    ...
    parts = [
        ...
        f"available_since={struct_def['available_since']!r}",
        f"fields=({fields})",
    ]
...
def _write_os_runtime_python(path: Path, payload: OsRuntimePayload, *, header: str) -> None:
    ...
    lines = [
        ...
        "    names_by_version: dict[str, str] = field(default_factory=dict)",
