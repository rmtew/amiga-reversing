...
- reject table spans that overlap accepted code unless explicitly modelled as a
  copied/runtime view
   - record unresolved candidate sites so corpus indexing can find similar
     patterns: implemented for recovered indirect sites by status, shape, and
     source pattern
...
- The signal should be reconciled after jump/lookup table work: a good table
  ...
  them.
- First-class table candidate records should replace indirect-site indexing once
  candidate source ranges and rejected table bounds are known.
