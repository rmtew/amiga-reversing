...
| Absolute memory | Tests cover ExecBase literal behavior, stack top EQU, interrupt/vector target stores, runtime aliases, relocation anchors, hardware sinks, display/copper/audio sinks; generated Amiga hardware metadata now carries stable symbol ids and runtime-target kind enums so display/audio/runtime-sink analysis does not branch on static register/role strings; C JSON now exposes `memory_layout_records` for base-layout fields, runtime views, runtime-address references with external hardware sink addresses, normalized range facts, and accepted absolute operands classified as ExecBase, CPU vector, hardware register/range, runtime range, section storage, or absolute memory with conflict state. | Memory-layout records still need higher-level absolute globals and unresolved candidates merged into the same view. |
...
- propagate hardware/display/audio sink types back to source data; implemented
  runtime-sink and display/audio decisions use generated hardware symbol ids and
  runtime-target kind enums, with strings kept for display/export text
