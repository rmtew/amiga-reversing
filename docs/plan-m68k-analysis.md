...
| Lookup/jump tables | Tests cover long dispatch, word-relative dispatch, far targets, runtime-mapped dispatch, mixed labels/raw entries, pointer tables, relative `target-base` rendering, and C JSON `table_records` derived from accepted structured table data. | Table facts still need consumer instruction/source provenance and conflict state beyond structured-data acceptance. |
