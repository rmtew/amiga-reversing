...
| --- | --- | --- |
| RSSET/app slots | `render_state_operand_uses_app_base()` rejects known hardware bases and `_custom` offsets; `render_asm_app_extension_rs()` emits app/resident RSSET layouts from field slots and metadata; policy RSSET regions carry explicit app-layout and storage-kind ids so rendering/reporting does not classify static names; target metadata parses the app-layout flag directly and tests assert app-looking names alone do not infer it; resident/autoinit structured data carries compact platform/field ids so renderer decisions do not parse pseudo-struct text; listing app-slot aggregation stores access/width properties as compact typed values rather than copied strings; C source analysis now records rendered base-layout fields, layout-kind ids, and alias overlays as first-class facts; typed app-slot region fields carry generated owner-struct metadata; listing JSON exposes those layout fields directly. Same-base, different-layout overlaps are marked conflicted in C `memory_layout_records`; section-relative platform storage effects are marked `code_overlap` only when their mapped target range intersects accepted code, while base-relative app offsets are not compared to source-code offsets. Isolated coverage: `test_source_analysis_memory_layout_marks_same_base_layout_overlap_conflicted` and `test_source_analysis_platform_storage_effect_conflicts_only_mapped_section_storage`. Listing and type-flow reporting now require C-produced `app_slot_refs` before classifying app-slot usage; rendered `app_*` text is display-only. | No current row-specific RSSET implementation gap; keep broader typed/platform ownership checks under Typed structs vs app slots. |
| Typed structs vs app slots | Typed app-slot struct regions own their full generated struct extent; interior offsets are blocked from flat RSSET output and render through typed fields where known. `_custom` offset false positives have an isolated test; C JSON now includes resolved/unresolved platform typed accesses, recovered platform storage writes, typed owner metadata on base-layout region records, and aggregate base-layout extent records in `memory_layout_records`; resolved typed accesses now carry generated `struct_size` and `field_size`; unresolved typed-access classification, typed provenance, and platform storage effects now carry numeric C enum ids so corpus/report decisions do not branch on display strings; unresolved typed-field report classifications now have numeric report ids with names kept as display text; memory-layout records now expose normalized range space/start/size/end fields for compact ownership/range consumers; corpus target rows now include target-level memory-layout views summarized from numeric range-space, conflict-state, and absolute-owner ids. Same-base overlaps between app and named/metadata layouts are conflict-marked before export. Resolved and unresolved platform typed-access memory-layout records now also expose owner-range start/size/end, and APP_SLOT-proven owner ranges are conflict-marked in C when they overlap non-app layouts sharing the discovered app base symbol. The manifest carries owner-range fields as numeric examples/xrefs, not parsed display text. Target usage xrefs now carry compact kind/feature ids for typed-flow records, and type-flow/report gates dispatch on those ids rather than static kind/feature strings. Dynamic typed/app xref classes such as `platform_typed_access_struct:<name>`, `platform_struct_field:<name>`, and `app_slot_api_arg_reason:<reason>` also carry `feature_class_id` plus `feature_value`, so report/gate consumers do not classify those records by prefix parsing. Isolated coverage: `test_source_analysis_platform_typed_access_conflicts_with_non_app_layout` plus manifest fixture coverage for id-backed type-flow report/gate paths. Temporary full usage rebuilds found 231 resolved and 248 unresolved typed memory-layout records, 2,163 owner-range examples, 3,584 owner-range xrefs, no current corpus `conflicted` typed owner-range rows, 36,989 xrefs with kind ids, 3,842 typed-flow exact feature ids, and 3,859 dynamic typed/app feature-class xrefs with no missing class ids. | Broaden only if real targets show mapped runtime/source-space conflicts beyond same-base app ownership. |
| ORG/runtime views | Tests cover runtime-copy jump targets, low trampoline suppression, policy runtime ranges, conflict failure, policy-vs-inferred precedence, corpus tags, listing navigation for materialized/suppressed runtime views, and compact C runtime-view relationships for larger-range exits, contained views, and runtime-copy overlays. Corpus indexing now uses numeric materialization reason and relationship ids for runtime-view decisions; names remain display text. Relationship ids now derive target-pattern tags for entry wrappers, contained helpers, overlaid helpers, and final-image-related runtime views without parsing display names. A temporary full usage rebuild found 2 entry-wrapper views, 4 contained-helper views, and 6 final-image-related views across Bloodwych, Conqueror, and imported disk rows. | Need broader imported-disk coverage as more runtime-copy/decompression heuristics are added; no current relationship-id/string-parsing gap. |
| Lookup/jump tables | Tests cover long dispatch, word-relative dispatch, far targets, runtime-mapped dispatch, mixed labels/raw entries, pointer tables, relative `target-base` rendering, C JSON `table_records` derived from accepted structured table data, consumer instruction provenance, source-pattern provenance, explicit `conflicted` flags for overlap handling, and C JSON `table_candidate_records` plus corpus tags/xrefs for unresolved indirect/table candidate sites by status, shape, source instruction range, operand index, source pattern, and rejected direct-stub bounds. Structured-data semantic roles now carry compact role flags, including string roles, so analysis/rendering does not branch on display strings for table/palette/copper/string properties; auto structured-data, inferred runtime-address dedupe, listing rows, listing navigation entries, runtime-address refs, table records, and orphan nearby-data context carry flags for known roles. Corpus indexing consumes C/listing role flags for table/copper/bitmap decisions and no longer infers table classes from rendered comments or display strings. Table conflict state now carries a numeric C enum id; string names remain exported display text only. Direct-stub table-bound rejection now distinguishes insufficient entries from proven candidate spans that overlap accepted code. C IR now separates proven table base from proven table bounds, and unresolved indexed candidates expose rejected undecoded-entry vs unsupported-entry-shape statuses without pretending bounds are known. PC-indexed word loads feeding PC-indexed indirect jumps now promote relative dispatch targets and render table entries as `target-table_base` label expressions, using generated decoded EA shape metadata rather than mnemonic strings. Biased/lower-bound PC-indexed word dispatch is provenance-first: the indexed word load records the relative-table source, and target recovery waits for the indexed control operand's table base so leading unused/bias entries do not become false code starts. Direct indexed dispatch into contiguous nonfallthrough code stubs now promotes the stub entries as code starts without treating broader inline/unrolled code as a table. Interleaved key/stub dispatch now recovers branch-stub entries when an accepted indexed data read using the same data index register proves an adjacent key table immediately before the indexed control table; the scanner requires nonfallthrough direct branch stubs, gates key bytes against accepted code overlap, and uses generated operand access/result metadata rather than mnemonic strings. Backward inline-tail dispatch now recovers Duff-style indexed jumps into a repeated instruction tail followed by a loopback control instruction; the scanner uses decoded form identity, decoded branch target metadata, and accepted-code overlap gates, not target names or byte signatures. Forward inline-tail dispatch now handles the sibling shape where the indexed control base is the first repeated instruction and a later loopback instruction proves the tail extent. Keyed long relative dispatch now tracks a source-offset postincrement long table entry through generated `swap_words` semantics and promotes high-word PC-relative control targets only when the full keyed table scans cleanly without accepted-code overlap; source rendering carries the same keyed-table provenance through intervening accepted instructions, so the table emits as `dc.w target-table_base,$key` instead of raw bytes. Branch-terminated inline stub dispatch now recovers indexed jumps into fixed-width forward stubs when every accepted entry decodes to the same branch-relative position and same forward join target. Orphan-code corpus work queues now require unresolved C signal status before emitting `orphan-code:missing_inbound:*`; suppressed structured-data overlaps remain visible as suppressed signals and nearby-data facts, but no longer become target-pattern work items. Isolated coverage: `test_facts_v2_pc_indexed_word_load_promotes_relative_jump_targets`, `test_facts_v2_biased_pc_indexed_word_load_promotes_relative_jump_targets`, `test_facts_v2_direct_indexed_stub_table_promotes_entries`, `test_facts_v2_indexed_backward_inline_tail_promotes_entries`, `test_facts_v2_indexed_forward_inline_tail_promotes_entries`, `test_facts_v2_swapped_keyed_long_table_promotes_relative_targets`, `test_facts_v2_branch_terminated_indexed_stubs_promote_entries`, `test_facts_v2_interleaved_key_stub_table_promotes_entries`, and `test_orphan_signal_features_use_status_id_for_suppressed_signal`. Temporary full usage rebuild after the first recovery found 16 relative-code-dispatch table records across 11 targets, 248 accepted jump-table indirect sites across 55 targets, `rejected_unsupported_entry_shape` reduced from 56 to 28, and `rejected_undecoded_entry` reduced from 28 to 17. Follow-up rebuild after biased dispatch recovery found 54 relative-code-dispatch table records across 22 targets, 269 accepted jump-table indirect sites across 56 targets, `rejected_unsupported_entry_shape` at 17, `rejected_code_overlap` at 7, and `rejected_insufficient_entries` at 6; real comparators include Workbench `libs/info.library` (`amiga-hunk/1390ed17b817`) and `System/DiskCopy` (`amiga-hunk/3875a3ad05b1`) biased PC-indexed dispatch tables. Follow-up rebuild after direct stub recovery found 281 accepted jump-table indirect sites across 57 targets and `rejected_unsupported_entry_shape` reduced to 12; real comparators include `amiga_hunk_monam302` and Atari `MON030.PRG`/`AMON030.PRG` direct indexed stub tables. Follow-up rebuild after interleaved key/stub recovery found direct stub rows rising from 1,435 to 1,479, Damocles `c/ed` and corpus twin `amiga-hunk/3f3009bac481` gaining 11 recovered stub rows each, MonAm302 gaining 4, `rejected_insufficient_entries` dropping from 6 to 0, `rejected_code_overlap` dropping from 7 to 0, and indexed-indirect unresolved rows dropping from 131 to 46. Follow-up rebuild after backward inline-tail dispatch found accepted jump-table indirect sites rising from 289 to 297 and `rejected_unsupported_entry_shape` dropping from 12 to 4; real affected rows are `amiga-hunk/64a1e661103a` and Voodoo `amiga-hunk/abcec3459986` plus its imported/direct project targets, with GenAm and MonAm exact rebuild comparators covered by `test_project_source_facts_v2_inline_tail_dispatch_voodoo_and_comparators_stay_clean`. Follow-up rebuild after forward inline-tail dispatch found accepted jump-table indirect sites rising from 297 to 299, unresolved indirect rows dropping from 3,211 to 3,209, and `rejected_unsupported_entry_shape` dropping from 4 to 2; real affected rows are Devpac v3.10 `atari-st/7ac7fc3f226e` at `$1D0A` and `$1DD8`. Follow-up rebuild after keyed long relative dispatch found accepted jump-table indirect sites rising from 299 to 300 and `rejected_unsupported_entry_shape` dropping from 2 to 1; real affected row is Workbench FastFileSystem `amiga-hunk/68924da60b37` at `$00AA`, which now reaches 21 table targets and splits the previously raw mixed table/code range at `$0136`. Follow-up rebuild after branch-terminated inline stubs found accepted jump-table indirect sites rising from 300 to 302, unresolved indirect rows dropping from 3,215 to 3,214, and `rejected_unsupported_entry_shape` dropping from 1 to 0; real affected rows are Midwinter II `amiga-hunk/87deaf4aa606` at `$2DAAE` and nested `$2DAFA`. Follow-up rebuild after keyed table rendering found `table:source_pattern:keyed_long_relative_dispatch` rising from 0 to 1, `table:kind:relative_code_dispatch` rising from 54 to 55, and 28 additional `analysis:lookup_table:word_relative_labels` rows; the real affected row is Workbench FastFileSystem `amiga-hunk/68924da60b37` table `$00C2`, rendered as label-relative `dc.w` pairs. Follow-up rebuild after leading biased-table recovery found orphan-code signals dropping from 1,319 to 1,307, unresolved orphan-code signals dropping from 1,310 to 1,303, `orphan-code:missing_inbound:jump_table` dropping from 8 to 3, accepted jump-table indirect sites rising from 302 to 304, and `table:kind:relative_code_dispatch` rising from 55 to 62 with no table code-overlap conflicts; real affected rows include Devpac `BIN/CLINK.TTP` (`atari-st/35a940cca122`) at `$7DD2`, `BIN/STRIP.TTP`, `BIN/SRSPLIT.TTP`, FSPRINT, and Amiga `parallel.device`. Follow-up rebuild after suppressed-signal gating kept total orphan signals at 1,307 and suppressed signals at 4, while `orphan-code:missing_inbound:jump_table` and `target-pattern:orphan_missing_jump_table` dropped from 3 targets/3 rows to 0 because the remaining three examples were decoded false positives overlapping accepted hex-character lookup tables. | Current table-base/status queues have no unsupported-entry-shape rows and no lookup-table missing-inbound work items; next table work should use newly observed unresolved statuses rather than suppressed structured-data overlaps. |
| Absolute memory | Tests cover ExecBase literal behavior, stack top EQU, interrupt/vector target stores, runtime aliases, relocation anchors, hardware sinks, display/copper/audio sinks; generated Amiga hardware metadata now carries stable hardware-base ids, symbol ids for registers/ranges, and runtime-target kind enums so display/audio/runtime-sink analysis does not branch on static base/register/role strings; render platform state stores hardware bases as generated ids and merge/provenance decisions use those ids, with symbols recovered only for source text. Bootloader hardware read setup consumes generated hardware ids and enum access kinds, OpenLibrary/OpenDevice recognition uses generated vector/function ids, and C trace/platform base state carries generated base ids so `SysBase`, `exec.library`, and base-symbol aliases converge before LVO analysis; strings are kept only for exported JSON/source text. C JSON now exposes `memory_layout_records` for base-layout fields, runtime views, runtime-address references with external hardware sink addresses, normalized range facts, accepted absolute operands classified as ExecBase, CPU vector, hardware register/range, runtime range, section storage, or absolute memory with numeric conflict state ids, absolute-owner ids, and memory-layout record-kind ids. Absolute operands found inside orphan terminal-code signals are exported into the same memory-layout view as unresolved absolute-memory candidates rather than promoted to code. Corpus memory-layout views and memory-kind features derive from numeric ids/flags rather than display strings. Corpus decompression rows now expose grouped `decompression:source_load_entry` features tying source range, absolute load address, and entrypoint together for raw/decompressed payload relationships; decompression events, derived-target suggestions, and materialized decompressed child metadata carry ids for status, reason, source kind, payload role/confidence, parent activity, codec support, and suggestion/event/target role, and corpus indexing consumes those ids rather than display strings. Child metadata role display fields are synthesized from ids during materialization, so stale C JSON display strings are not persisted. A temporary full usage rebuild found 19 grouped source/load/entry facts across 9 rows and 2 checked-in decompressed child targets indexed through id-backed child metadata. | No current row-specific implementation gap; keep broader imported-target coverage under Targets. |
| Orphaned code | C analysis now records unresolved terminal-decode islands at accepted-code boundaries or data labels as orphan signals without promoting them to accepted code; nearby structured data carries role flags, source offset, distance, and compact relation id into the signal, so lookup-table-adjacent islands are classified with missing inbound `jump_table` and pointer-table-adjacent islands are classified as callback/function-table evidence without re-parsing semantic-role text. Orphan islands that contain absolute CPU/vector-slot operands are classified with missing inbound `vector`, and their orphan absolute-memory records are owner-classified as `cpu_vector`. Amiga orphan islands that contain generated-metadata LVO call evidence are classified with missing inbound `api`. Policy-named orphan labels are distinguished as `policy_seed` evidence while object/metadata labels remain `metadata`. Source-analysis JSON exposes target-level orphan status counts; corpus missing-inbound work queues are emitted only from status-gated unresolved signal records, not from status-agnostic summaries. Corpus rows now tag orphan missing-inbound target patterns for callbacks, vectors, runtime copies, APIs, and metadata from numeric ids; current jump-table examples are suppressed structured-data overlaps and no longer become work items. | Reconcile signal priority after callback/vector/API/runtime improvements; use the missing-inbound target tags as the work queues and measure decreasing counts when real flow recovery lands. |
| Targets | Bloodwych has many relative lookup tables; Pandora demonstrates wrapper load vs final copied image; Conqueror demonstrates weak low ORG risk; Carrier stresses packed/runtime-copy ambiguity; GenAm/MonAm remain comparator targets. The external hand disassembly `resources/clone_amiga/Bloodwych-68k/asm/BLOODWYCH439_relabel.asm` is available as comparison material for locating source references to Bloodwych data, alongside the existing relabel-data variants; it is not an oracle and must not become target-specific analysis logic. Corpus rows now derive evidence-based `target-pattern:*` tags for relative lookup dispatch, copied runtime code, weak low trampolines, packed runtime copy, packed runtime-copy conflict, orphan-code signals, orphan missing-inbound classes, runtime entry wrappers, and runtime contained helpers without target-name rules. | Need broader imported-disk coverage for future pattern tags as new heuristics land. |
