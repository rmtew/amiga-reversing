...
| Absolute memory | Tests cover ExecBase literal behavior, stack top EQU, interrupt/vector target stores, runtime aliases, relocation anchors, hardware sinks, display/copper/audio sinks; C JSON now exposes `memory_layout_records` for base-layout fields, runtime views, runtime-address references with external hardware sink addresses, normalized range facts, and accepted absolute operands classified as ExecBase, CPU vector, hardware register/range, runtime range, section storage, or absolute memory with conflict state. | Memory-layout records still need higher-level absolute globals and unresolved candidates merged into the same view. |
| Orphaned code | C analysis now records unresolved terminal-decode islands at accepted-code boundaries or data labels as orphan signals without promoting them to accepted code; lookup-table-adjacent islands are classified with missing inbound `jump_table`, and pointer-table-adjacent islands are classified as callback/function-table evidence. | Extend the signal with vector/API evidence classes, richer nearby context, target metrics, and reconciliation after table/callback/vector improvements. |
