...
Orphan missing-inbound corpus evidence after adding target-pattern tags:

| Feature | Target rows | Signals |
| --- | ---: | ---: |
| `target-pattern:orphan_missing_api` | 18 | 47 |
| `target-pattern:orphan_missing_callback` | 3 | 5 |
| `target-pattern:orphan_missing_jump_table` | 7 | 16 |
| `target-pattern:orphan_missing_metadata` | 224 | 764 |
| `target-pattern:orphan_missing_runtime_copy` | 6 | 35 |
| `target-pattern:orphan_missing_vector` | 33 | 187 |

Examples include Starglider callback/function-table candidates, imported Amiga
vector/API candidates, runtime-copy orphan islands in Bloodwych variants, and
jump-table-adjacent lookup-table islands across Amiga and Atari corpus rows.
