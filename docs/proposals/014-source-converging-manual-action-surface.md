...
| App/base-relative slots and RSSET layout regions | App-slot refs, app-slot analysis, regions/gaps/field gaps/suggestions, `rsset_layout_regions`, named layouts | `RSSET`/`RS.*`, `app_0234(a6)` names, typed field paths | Add/edit/rename/remove slots, regions, sizes, storage kinds, semantic types, parser roles, and layout gaps | App-slot context has symbol/displacement/base/access; manual RSSET region identity is `(layout_name, base_symbol, offset)` | `create_manual_rsset_layout_region` adds/replaces manual RSSET layout regions; `remove_manual_rsset_layout_region` filters regions by identity | `target.rsset_region.add/edit/rename/remove` expose target-level RSSET changes; `app_slot.rename/edit/remove` expose selected app-slot rename/edit/removal via RSSET region actions | Planner recognizes explicit add/edit/rename/remove candidates and skips already-satisfied projected state | Manual replay, effective metadata, command payload/execution, rendered RSSET field/reference, removal-to-raw-reference, app-slot command execution, planner alias selection, and exact direct-rebuild tests cover add/edit/rename/remove payloads; parser-role/autonomous candidate workflows remain open | Continue app-slot/RSSET editing in `014-011-app-slot-rsset-editing.md` |
