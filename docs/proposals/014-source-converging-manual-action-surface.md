...
| Runtime/execution views and loader ranges | `execution_views`, runtime address ranges, runtime view starts, absolute labels | ORG/runtime labels and copied-stage source | Add/edit/remove runtime views for copied or relocated code/data when auto-analysis lacks evidence | Execution view identity is `(source_start, source_end, base_addr)`; absolute labels use label identity | `create_manual_execution_view` adds manual execution views; absolute labels can be created through label path | `target.execution_view.add` for target context; label rename only for labels | None | Manual replay, effective metadata, command payload, and target command execution tests cover execution-view add; source/reproduction tests exist; no edit/remove verifier | Add execution-view edit/remove and broader correction/view actions in `014-013-correction-and-view-actions.md` |
