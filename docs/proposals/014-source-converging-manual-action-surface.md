...
| Custom structs, fields, and typed accesses | `custom_structs`, RSSET regions, typed accesses, unresolved typed gaps, type-flow analysis | Struct field refs, typed field paths, gap navigation | Add/edit/rename/remove structs and fields; resolve typed gaps/accesses | Struct identity uses name; field names/offsets exist in metadata; command identity absent | `create_manual_custom_struct` adds/replaces manual struct metadata; no field-level edit/remove/rename actions yet | No typed-access or field-gap edit command | None | Manual replay and effective metadata projection cover custom struct add; auto-analysis/navigation tests cover discovery only | Add commands, field-level actions, render verifiers, and typed-gap workflows in `014-012-structure-field-editing.md` |
