...
| Data-block type/platform binding | Semantic/type | Data-block layouts/elements, custom/platform structs, enum/equate domains, parsed include data | Row bind/clear type commands for element-scoped bindings; custom/platform struct bindings expand into typed seeded field entities; domain expansion remains planned | Should propagate typed facts, rendered nested fields, review items, and interpreted field refs only after type shape reconciles. | Layout id + element offset + type-binding id; active bindings stamp `owner_action_id`; all derived facts owner-scoped. Current struct field entities carry the binding id as source locator but not yet a dedicated descendant owner field. | Bind/clear loop path verifies reload, rendered bound-type token proof, accepted consumed provenance where present, typed field render for custom/platform structs, and exact round-trip; generated type-flow/review cleanup verifier remains missing. | `014-019` model and verifier proof. |
