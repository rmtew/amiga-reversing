...
source-converging. Known NDK equate hints now project through
`effective_metadata.py` into rendered symbolic immediates, but LVO,
struct-offset, and target-local equate edit hints still need effective metadata
projection and rendered-source verification before they count as
source-converging.
