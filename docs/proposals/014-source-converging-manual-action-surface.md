...
Status: First-pass implementation complete. The supported 014 surface has
durable Manual Action Log paths, command catalog exposure, loop selection, and
action-specific verifier gates. Remaining cascade/propagation work is future
capability, not required for 014 closeout.
...
014 closes the first-pass source-converging manual action surface: supported
commands are executable only with durable identity, Manual Action Log replay,
and an action-specific verifier. The following remain explicit future work and
may surface as missing capability during 015 target work: RSSET bind-refine and
same-flow/same-displacement cascades, API argument/return/stored-state
propagation, propagated typed-field cleanup, data-block generated
type-flow/review cleanup, target-side inbound interpreted-reference xrefs, and
non-absolute interpreted-reference kinds.
...
Before widening any one editing family, Proposal 014 needed a full editing
surface audit. That investigation is now folded into this proposal.

Current audit result:

| Editing family | Category | Evidence source | Manual action shape | Reconciliation / cascade policy | Owner / cleanup requirement | Verifier gate | Future/deferred scope |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
| Data-block scalar layouts | Classification/layout | Data ranges, existing data roles, source bytes, active layout context | `create/edit/remove_manual_data_block_layout`, `set/remove/represent_manual_data_block_element` | Splits opaque ranges into elements and projections. Scalar rendering is supported; type/domain binding remains separate. | Layout id plus element offset; removal restores raw/gap state. | Layout/element state, exact directive/value render checks, exact round-trip. | Future: gap-specific rendering refinements and deeper type/platform binding. |
| Interpreted data refs | Layout plus xref-facing semantic | Data-block elements, decoded source bytes, target locators, runtime-address refs | `interpret/remove_manual_data_block_element_ref` | Converts proven byte/word/long values into symbolic refs and source-owned xrefs. | Interpreted-ref id owns symbol projection and source-owned runtime ref; cleanup must match layout/element/source value. | Ref state, symbolic render, source-owned xref cleanup, exact round-trip. | Future: target-side inbound xrefs and non-absolute references. |
| Target EQU/constants | Mixed display and semantic | Target-local EQU table, NDK/equate lookup, immediate operands | `create/rename/remove_manual_target_equate`, semantic equate hints | EQU definitions and symbolic use sites are semantic; EQU definition value style is presentation-only and now durable. | Equate name identity; rename/remove updates owned symbolic use-site representations; semantic hints preserve consumed evidence when selected/planner context supplies it. | Target-equate state, rendered definition value style, or semantic-hint state plus exact round-trip; provenance-backed semantic hints compare consumed evidence on semantic reload and skip checks. | Broader semantic/equate provenance remains separate. |
| API/LVO/register semantics | Semantic/type | LVO rows, NDK library/function metadata, selected register/memory operand contexts, typed-access analysis | `create/remove_manual_semantic_hint`, `create/remove_manual_register_seed` | Consumes API/register evidence and can seed library bases, struct pointers, LVO/field symbols, typed args/returns, and stored state. | Register seed/hint id; library-base, semantic hints, and selected typed struct-pointer register seeds now preserve consumed provenance; future propagated args/returns/type-flow need owner identity and cleanup. | Semantic hint or register-seed state plus exact round-trip; provenance-backed hints/register seeds also compare consumed evidence on semantic reload; broader type-flow verifier missing. | Future: API arg/return/stored-state propagation and broader evidence-scoped lifetimes. |
| RSSET/app-slot regions and bindings | Semantic/type plus layout | App-slot refs, app-slot analysis, RSSET regions/gaps, raw displacement operand parts, register/base evidence | `create/remove_manual_rsset_layout_region`, `create/remove_manual_rsset_use_site_binding`; refine/type actions planned | Regions render `RS.*`; binding links selected numeric uses to a proven base/layout. Flow-equivalent same-displacement candidates are planned, not automatic. | Region identity `(layout_name, base_symbol, offset)`; binding id plus `owner_action_id` and future cascade ids for same-displacement/xref/type descendants. | RSSET region/binding state, selected-use render or ref-only/raw cleanup state, and exact round-trip. | Future: flow-derived base evidence, bind-refine, type/xref descendants, owned cascades. |
| Custom structs and typed fields | Semantic/type | Custom struct metadata, typed gaps/accesses, NDK/platform structs, unresolved typed access analysis | `create/rename/remove_manual_custom_struct(_field)` via target/typed commands | Effective metadata can store structs/fields; the C resolver now consumes register-seed-backed custom structs for rendered field paths. | Struct name and `(struct_name, offset)` field identity; selected typed-field writes carry consumed `struct_pointer` provenance, `parent_evidence_ids`, and projected custom-field owner/cleanup action ids; propagated typed accesses need descendant owner identity. | Selected typed-field commands now prove accepted provenance, complete selected-field identity replay with matching consumed evidence/owner, rendered selected access, and exact round-trip. Target-wide metadata commands and propagated cleanup remain blocked. | Future: propagated typed-access cleanup and explicit namespace selection. |
| Data-block type/platform binding | Semantic/type | Data-block layouts/elements, custom/platform structs, enum/equate domains, parsed include data | Row bind/clear type commands for element-scoped bindings; custom/platform struct bindings expand into typed seeded field entities; scalar domain bindings project `value_domain` for symbolic values | Should propagate typed facts, rendered nested fields, review items, and interpreted field refs only after type shape reconciles. | Layout id + element offset + type-binding id; active bindings stamp `owner_action_id`; clear-type preserves the previous binding with `cleanup_action_id`; all derived facts owner-scoped. Consumed provenance includes `parent_evidence_ids`. Current struct/domain seeded entities carry the binding id, owner action, consumed evidence id, and parent ids; generated type-flow/review descendants still need dedicated owner fields. | Bind/clear loop path verifies complete nested binding identity, matching type-binding owner/cleanup, rendered bound-type token proof, binding-owned seeded descendants, accepted consumed provenance where present, typed field render for custom/platform structs, symbolic scalar domain render, and exact round-trip; generated type-flow/review cleanup verifier remains missing. | Future: generated type-flow/review descendants and cleanup verification. |
| Corrections and execution views | Correction/view | Suppressible seeded metadata, target corrections, runtime/execution view evidence, reproduction mismatches | `suppress_seeded_item`, `create/remove_manual_execution_view` | Correct target-specific facts or runtime mapping. Importer/analyzer class bugs must remain upstream bugs. | Suppression `(kind,hunk,addr)` or execution-view identity plus `(source_start, source_end, base_addr)` replay key; execution views carry owner/cleanup action ids; cleanup must not remove unrelated facts. | Suppressed-item or execution-view state, including view owner/cleanup ids; seeded-item suppression also checks affected listing rows no longer expose the suppressible seeded item; exact round-trip remains required. | Future: broader reproduction/view correction commands. |
...
- Loop verification in `reversing_loop.py` currently proves selected semantic
  hints, library-base and struct-pointer register seeds, including consumed
  provenance for provenance-backed register seeds, RSSET
...
Deferred follow-up investigations after 014 closeout should proceed in this
order. These are future capability tracks, not open 014 closeout blockers:
...
Archived issue routing and readiness:

The `docs/issues/014-*` files were temporary implementation trackers. Their
remaining takeaways are folded into this proposal so Proposal 014 can stand as
the durable archive. Future work should create new focused issues instead of
reopening the deleted 014 issue files.
...
- RSSET propagation: selected-use binding remains scoped. Same-displacement
  ...
  cleanup.
- Historical issue split: `014-002` handled base-evidence and
  descendant-owner identity; `014-010` handled first-slice register/base
  provenance; `014-011` handled RSSET provenance consumption/export/catalog/
  bind/refine/cascade implementation; `014-005` handled verifier gates; and
  `014-006` gated broad planner feed exposure.
- Typed/custom and data-block follow-up: the archived `014-012` track captured
  rendered custom struct/typed-field requirements, while archived `014-019`
  captured data-block type/platform binding requirements.
...
- Review outcome: accepted. First-pass implementation is complete for the
  supported 014 command/verifier surface. The remaining provenance,
  propagation, and cascade items are future capability work to route through
  new focused issues.
...
Future issue creation should follow the post-review checklist above and keep
work grouped by coherent capability rather than creating one implementation
issue per editing family.
...
This model must compose with existing data roles, literal representations,
target-local EQU values and EQU value display styles, API/register type flow,
struct fields, and data/global symbols. The archived data-block investigation
chose a new first-class layout/element metadata family. Implementation was
split into scalar layout metadata, verified GenAm rendering, interpreted
references, and type-flow projection slices.
...
such as xrefs/type-flow/review items, or be display-only. Future issues must
name which of those applies, how derived facts are removed when the owning
manual action is removed, and which verifier proves the analysis-facing effect.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Source labels and function entry labels | Facts v2 code starts, control targets, policy `seeded_code_labels`, `seeded_code_entrypoints`, absolute labels | `loc_*`, `abs_*`, entry labels and symbolic refs | Rename, create, remove, or change scope for code labels when xrefs/behavior justify the name | Listing row locator plus source/runtime label payloads; row index still appears as provenance only | Create/remove/rename/change scope labels | `label.rename` for label elements; review label actions | Planner accepts explicit label rename candidates and mines source-descriptor entrypoint evidence into autonomous `label.rename` candidates when the rendered label is still generated, skipping effective/manual labels already named; review label actions report `manual_label_state` | Label projection, semantic reload, and round-trip in row label-rename path; review-label actions verify manual label state/removal plus round-trip | Future: broaden autonomous label feeds beyond entrypoint evidence. |
| Data/global symbols | `SeededEntityMetadata.name`, app-slot names, absolute/code labels, data-class rows, and rendered symbolic data refs | Replaces anonymous data labels and global slots with semantic names at definitions and use sites | Rename/create/remove data symbols, global data labels, and referenced data names from xrefs | Data row identity is hunk + source range; seeded-entity rename/remove identity is hunk/address/end/type plus source provenance where available; referenced data use-sites use the target hunk/offset/end from internal runtime-address refs and preserve existing use-site symbols as previous-name context | Data seeds can carry `name`; `rename_data_symbol` projects data-row, referenced data, and seeded-entity name overrides without erasing typed seeded facts; seeded data symbol removal uses ranged `suppress_seeded_item` when end is known | `row.seed.data.named`, `element.seed.data.named`, `range.seed.data.named`, and `review.seed.data.named` create named raw data seeds; data rows and internal `data_ref` elements expose `data_symbol.add` for first names and `data_symbol.rename`; rows/use-sites with existing symbols also expose `data_symbol.edit` and `data_symbol.rename_existing`; seeded entity rows expose `data_symbol.remove` with ranged seeded-item identity when available | Planner recognizes explicit data-symbol add/edit/rename/remove candidates, skips already-satisfied projected/effective names/removals using hunk/address/end/type identity, and mines internal `data_ref` listing elements plus data-class definition rows into autonomous data-symbol rename candidates while using `data_symbol.rename_existing` when an old symbol is known | Seed projection can render names when supplied; durable payload matching, manual replay, target/effective metadata merge, command payload, listing annotation, command execution, rendered definition/use-site-name/removal, existing-symbol context, add/edit/rename-existing command routing, ranged suppression, and exact direct-rebuild tests cover data-row/seeded-entity rename/remove plus internal referenced data command routing; broader global verifier absent | Future: broader global workflows. |
...
| Review items and typed resolutions | Manual review items for conflicts, malformed logs, orphan code, unreconciled data, suspicious decode, reproduction mismatches, blockers, review notes | Clears blockers or records decisions | Resolve only with type-specific evidence and verifier | Review item id + evidence fingerprint | Resolve review item; review note actions | Review item actions for current kinds | Generic inspect uses review candidates and explicit review-item command candidates for seed create/remove and label edits; broader autonomous source-value ranking remains open | Review-item source actions route through semantic reload plus round-trip; type-specific verifiers remain incomplete | Supported first-pass path complete; type-specific verifier expansion remains future work. |
...
`data_symbol.remove`; their verifier should check the reloaded suppression,
affected listing rows, and round-trip. Affected locators alone are not proof,
but they are the source rows the verifier must reopen to ensure the selected
`suppressible_seeded_items` identity disappeared.
...
Implementation observation from `014-011`: RSSET binding semantic replay must
validate the durable selected-use identity before matching reloaded state. A
binding id alone is not proof that the executed action preserved the selected
hunk/address/operand/base/layout/base-evidence boundary.
...
Implementation observation from `014-019`: binding id alone is not enough for
generated data-block descendants. Structured-data seeded descendants must also
carry the binding `owner_action_id`, consumed `source_evidence_id`, and
...
Implementation observation from `014-013`: execution-view identity has two
layers. The `(source_start, source_end, base_addr)` tuple is the replay key for
replacement/removal, but command payloads must also preserve an explicit
`execution_view_id` when a candidate or UI selected an existing view. Otherwise
...
Implementation observation from `014-010`/`014-005`: register seeds are
provenance-bearing semantic writes when they consume first-slice
`library_base` or `struct_pointer` evidence. The catalog, Manual Action Log
payload, effective metadata, and semantic-reload verifier must all preserve the
same `source_evidence_id`, source family/status, path/lifetime scope, conflicts,
override cleanup fields, and unordered parent dependency set; matching only the
register plus library or struct name can hide stale evidence.

Implementation observation from `014-010`: generic provenance reports must keep
manual-override correction fields visible (`contradicted_evidence_id`, reason,
and `cleanup_scope`) even before a family-specific command consumes them.
Otherwise the report can prove the new source family while losing the cleanup
boundary required by correction verifiers.

Implementation observation from `014-010`/`014-005`: semantic hints are also
provenance-bearing when selected/planner context supplies consumed evidence.
Catalog parameters, Manual Action Log payloads, semantic reload verification,
...
Implementation observation from `014-010`/`014-005`: semantic-hint
verification derives expected state from the durable action payload before any
local-effect echo. Local effects are useful UI feedback, but using them as the
primary expected semantic state can let a mismatched response hide the Manual
Action Log fact that was actually appended; local-effect-only semantic hint
responses now fail as missing durable payloads.
...
Implementation observation from `014-012`: semantic reload must reject sparse
typed-field payloads before matching reloaded state. A custom-field name without
selected struct/offset identity can otherwise look like the requested field
while proving neither source location nor type ownership.
...
Implementation observation from `014-019`: semantic reload must reject sparse
nested type-binding payloads before matching reloaded element state. Element
layout/offset alone is not enough proof that the durable action preserved the
selected binding id, element width, binding kind, bound type/domain, and owner
or cleanup action identity.
...
Implementation observation from `014-014`: rendered data-symbol text is not
enough proof for add/edit/rename. The verifier must first derive the expected
hunk/address/range/name from the executed durable `data_symbol` payload, compare
it with the selected command identity, and then check semantic seed replay.
...
- Missing command support is a blocker, not permission for scripts or direct
  metadata writes.
- Proposal state is part of the implementation work. When code changes support,
  verifier behavior, loop selection, or remaining gaps, update this matrix and
  create/update the focused follow-up issue that owns that future work.
- Prefer real target source convergence over surface expansion. Before adding
  ...
  rendered-source improvement, and exact round-trip; track verifier and
  planner/feed proof in the new focused issue for that family.
...
The historical implementation issue mapping was:

- `014-001`: source-convergence matrix.
- `014-002`: durable target identities.
- `014-003`: Manual Action Log coverage.
- `014-004`: command catalog coverage.
- `014-005`: verifier coverage.
- `014-006`: loop planner command selection.
- `014-007` through `014-014` and `014-020`: concrete early action families.
- `014-015`: data-block investigation, split into implementation slices
  `014-016` through `014-019`.
- `014-021`: RSSET binding/cascade investigation.
- `014-022`: editing-surface reconciliation audit.

## Status Discipline

- Keep this proposal closed at first-pass complete while the supported 014
  surface has durable identity and action-specific verifier coverage.
- Future provenance, propagation, and cascade work should be tracked as new
  focused capability issues, not as reopened 014 closeout work.
- When future implementation changes this surface, update the relevant matrix
  row with final Manual Action Log, command catalog, loop, and verifier state.
- If work discovers a missing verifier, missing durable identity, stale smoke,
  or metadata-only path, record it in a new focused issue before continuing
  broad planner expansion.
...
- Each deferred edit has a proposal section or future issue with identity,
