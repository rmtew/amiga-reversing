...
| Auto-analysis corrections and suppressions | `target_corrections.json`, `suppressed_seeded_items`, reproduction mismatches, blockers | Removes wrong imported/seeded facts and unblocks source render | Suppress or override target-specific auto facts only when upstream analysis is not objectively wrong for a whole class | Suppression identity is hunk + addr + seeded item kind | `suppress_seeded_item` projects target-specific seeded item suppressions | Review actions can acknowledge/remove manual annotations; no suppress command yet | None | Manual replay and effective metadata tests cover suppression projection; review/reproduction command verifier absent | Add command exposure and execution-view actions in `014-013-correction-and-view-actions.md` |
