...
- Project preview rendering is no longer blocked on the old candidate-row
  transport. 021-004/021-005 recorded that preview rows still used an internal
  `RawBinarySource` bridge, and post-closeout follow-up 021-008 removed it by
  routing preview rows through the native Mac CODE byte artifact helper. Current
  project preview rendering does not construct `RawBinarySource` or temp raw
  files.
...
- 021-008 removed the candidate preview transport blocker by moving candidate
  preview decoding onto `build_macos_code_bytes_listing_artifact_profile`.
  Current `macos_project_payload._preview_decode_rows` uses that native Mac CODE
  byte artifact path, does not construct `RawBinarySource` or temp raw files,
  and still carries candidate/deferred fact ids and parser-use state.
