...
2. Add profile editing through catalog/API/CLI.
...
3. Add an oracle availability detector.
   - Check configured paths and PATH lookup for `vasm`, DevPac/GenAm runner, `vamos`, and `machine68k` where relevant.
...
4. Keep exactness semantics strict.
   ...
   - A target is not `EXACT` because an oracle accepted source; exactness remains byte/semantic comparison against the configured gate.
...
- Add a tool registry under project config for external assembler/emulator paths, separate from Manual Action Log.
- Add report sections for each requested profile so UI can show `exact`, `content match`, `oracle accepted`, or `tool missing` independently.
