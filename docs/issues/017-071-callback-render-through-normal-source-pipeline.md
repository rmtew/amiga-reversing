...
- Accepted callback decisions are projected into effective metadata as seeded-code entrypoints and rendered by `render_project_source_with_c_backend`.
- The verifier compares a baseline without Decision Journal callback projection against the normal effective-metadata render.
- The projection is read-only and derived from Decision Journal state; it does not mutate target metadata or generated source output.
