...
Scope:
Make Classic Mac OS support reachable through the normal project, server API,
and web UI flow.

Problem:
The current web renderer can display a `classic_macos` payload, but the normal
server project payload does not emit one. The starter view is therefore
fixture/helper-tested rather than openable as a real project in the application.

This should be solved by extending the shared project/API/listing model. Do not
add a legacy Mac-only project payload path or a compatibility branch that bypasses
the normal project flow. If the current project model cannot express source
containers, fork/resource metadata, or selected CODE listings cleanly, fix that
model or raise the blocker.

Acceptance criteria:
- A Classic Mac OS project record can be created or loaded from the committed
  MPW-GM fixture metadata.
- The normal project API emits `classic_macos` data in the project payload.
