...
- [ ] Selecting a parameterized palette action opens its session inside the palette.
- [ ] Escape returns from the session to the previous palette search/selection state.
- [ ] Escape from the restored command list closes the palette.
- [ ] Enter commits through the catalog execution path.
- [ ] Server validation errors keep the session open.
- [ ] Web/CDP tests cover submit, cancel, back-stack, and validation-error behavior.
