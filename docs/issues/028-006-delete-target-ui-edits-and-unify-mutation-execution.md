...
- [ ] `/target-edits` and `target_ui_edits.py` are removed after supported flows are replaced.
- [ ] Inline comment edits, label/comment/representation/review workflows, and command palette mutation flows use the same mutation result shape.
- [ ] Mutation responses return durable action id, manual log count/head hash, projection hash, effective metadata hash, affected locators, and optional authoritative refreshed window.
- [ ] Browser workflows do not bypass locator/context validation through direct append routes.
- [ ] No production workflow reads `target_ui_edits.json`.
- [ ] `effective_metadata.py`, reproduction stamping, profile-set copying, source tests, and server route tests no longer depend on `target_ui_edits`.
- [ ] Local listing effects in `app.js` are deleted or narrowed to presentation-only pending/flash state followed by authoritative projection refresh.
