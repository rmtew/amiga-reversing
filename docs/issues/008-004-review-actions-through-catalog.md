...
- [ ] A Review dialog action and the matching palette action share the same catalog action id and execution path.
- [ ] Existing review action e2e coverage still passes through the catalog-backed path.
- [ ] No web-only manual action eligibility remains for catalog-covered review actions.
- [ ] Tests cover at least one seed action and one resolve/acknowledge action from both surfaces.
