...
def _set_job_phase(job_id: str, *, phase_id: str, phase_index: int, phase_count: int) -> None:
    progress_current, progress_total, progress_percent = _phase_progress(phase_index, phase_count)
    _set_job_state(
        job_id,
        phase_id=phase_id,
...
def _project_create_phase_count(filename: str) -> int:
    return _PROJECT_CREATE_DISK_PHASE_COUNT if Path(filename).suffix.lower() == ".adf" else _PROJECT_CREATE_EXECUTABLE_PHASE_COUNT
...
def _build_rows_job(job_id: str, project_name: str) -> None:
    ...
    try:
        ...
        _set_job_phase(job_id, phase_id="build_session", phase_index=1, phase_count=phase_count)
        session = build_project_session(project_name)
        _log_event("listing_job phase", job_id=job_id, project=project_name, phase="emit_rows")
        _set_job_phase(job_id, phase_id="emit_rows", phase_index=2, phase_count=phase_count)
        rows = emit_session_rows(session)
        _PROJECT_ROW_CACHE[project_name] = rows
        _log_event("listing_job done", job_id=job_id, project=project_name, total_rows=len(rows))
        _set_job_state(
            job_id,
            status="ready",
            phase_id="done",
        ...
        _log_event("listing_job failed", job_id=job_id, project=project_name, error=str(exc))
        _set_job_state(
            job_id,
            status="failed",
            phase_id="error",
...
def _start_listing_job(project_name: str) -> AsyncJobPayload:
    ...
    with _JOB_LOCK:
        for _existing_id, job in _ASYNC_JOBS.items():
            if job["job_kind"] == "listing" and job["project_id"] == project_name and job["status"] in {"queued", "building", "ready"}:
...
def resolve_static_response(path: str) -> StaticResponse:
    ...
    if "/" not in relative and direct_file_path != WEB_ROOT.resolve() and not direct_file_path.exists():
        relative = "index.html"
    ...
    def do_GET(self) -> None:
        def handler() -> tuple[bytes, str, int, dict[str, str] | None]:
            ...
            if parsed.path == "/" or not parsed.path.startswith("/api/"):
                ...
                return response["body"], response["content_type"], 200, response["headers"]
    ...
    if len(parts) >= 3 and parts[0] == "api" and parts[1] == "projects":
        ...
        if method == "GET" and len(parts) == 4 and parts[3] == "listing":
            ...
            if project.kind != "binary":
                raise ValueError(f"Project {project_name} does not expose a disassembly listing")
        ...
        if method == "POST" and len(parts) == 5 and parts[3] == "listing" and parts[4] == "open":
            ...
            if project.kind != "binary":
                raise ValueError(f"Project {project_name} does not expose a disassembly listing")
            if not project.ready:
                return {"ok": True, "data": cast(AsyncJobPayload, {
...
def _create_project_from_media(body: dict[str, object], *, job_id: str | None = None) -> ProjectRecord:
    def report(phase_id: str, phase_index: int, phase_count: int) -> None:
        if job_id is None:
            return
        _set_job_state(job_id, status="building")
        _set_job_phase(job_id, phase_id=phase_id, phase_index=phase_index, phase_count=phase_count)
    ...
    if media_path.suffix.lower() == ".adf":
        ...
        base_name = dedupe_project_name(derive_project_name(filename), project_root=PROJECT_ROOT)
        disk_id = None
    _log_event("media_upload saved", filename=filename, path=media_path.as_posix(), project_id=base_name)
    if media_path.suffix.lower() == ".adf":
        ...
        manifest = create_disk_project(
            ...
            project_root=PROJECT_ROOT,
            progress_fn=(
                None if job_id is None else
                lambda phase_id, phase_index, phase_count: report(phase_id, phase_index + 1, phase_count + 1)
            ),
        )
