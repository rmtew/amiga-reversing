...
static int is_register_like_symbol(const char *text, uint8_t target_cpu) {
  ...
  return m68k_parse_register_token(text, 'd', &reg) ||
         m68k_parse_register_token(text, 'a', &reg) ||
         _stricmp(text, "sp") == 0 || _stricmp(text, "pc") == 0 ||
         m68k_parse_control_register_token(text, target_cpu, &control_id,
                                           &control_value);
}
...
static int mnemonic_uses_relative_label_operand(const char *mnemonic) {
  ...
  size_t index;
  for (index = 0; index < sizeof(LABEL_MNEMONICS) / sizeof(LABEL_MNEMONICS[0]);
       ++index) {
...
static int lookup_constant_symbol_for_expression(const char *name,
                                                 uint32_t *out_value,
                                                 void *user_data) {
  const M68kSymbolicParseContext *context =
      (const M68kSymbolicParseContext *)user_data;
...
static void mark_ir_symbol_ref_kind(M68kOperandIR *operand) {
  if (operand == NULL || !operand->symbol_ref.has_name)
    return;
...
static int rewrite_base_relative_symbol_operand(const M68kSymbolicParseContext *context,
    const char *operand, char *out_rewritten, size_t out_rewritten_size,
    char *out_symbolic_name, size_t out_symbolic_name_size) {
  const char *paren = NULL;
  ...
  if (context->lookup_symbol != NULL &&
    ...
    snprintf(out_rewritten, out_rewritten_size, "%d%s",
             (int32_t)symbol_value, paren);
...
static int rewrite_absolute_constant_operand(const M68kSymbolicParseContext *context,
    const char *operand, char *out_rewritten, size_t out_rewritten_size) {
  ...
  if (strchr(operand, '(') != NULL || strchr(operand, ',') != NULL)
    return 0;
  if (suffix == NULL || (_stricmp(suffix, ".w") != 0 && _stricmp(suffix, ".l") != 0))
    return 0;
  expr_len = (size_t)(suffix - operand);
  if (expr_len == 0U || expr_len >= sizeof(expr))
    return 0;
  snprintf(expr, sizeof(expr), "%.*s", (int)expr_len, operand);
  if (!m68k_source_parse_constant_expression(expr, lookup_constant_symbol_for_expression,
          (void *)context, &value)) {
    return 0;
  }
...
static int rewrite_absolute_symbol_operand(const M68kSymbolicParseContext *context,
    const char *operand, char *out_rewritten, size_t out_rewritten_size,
    char *out_symbolic_name, size_t out_symbolic_name_size) {
  const char *suffix = strrchr(operand, '.');
  char symbol[M68K_INSTRUCTION_SPEC_MAX_LABEL_NAME];
  size_t symbol_len;
  if (context == NULL || operand == NULL || out_rewritten == NULL || out_symbolic_name == NULL)
    return 0;
  if (strchr(operand, '(') != NULL || strchr(operand, ',') != NULL || operand[0] == '#')
    return 0;
  if (suffix == NULL || (_stricmp(suffix, ".w") != 0 && _stricmp(suffix, ".l") != 0))
    return 0;
  symbol_len = (size_t)(suffix - operand);
  if (symbol_len == 0U || symbol_len >= sizeof(symbol))
    return 0;
...
int m68k_parse_instruction_with_symbol_fallback_spec(const M68kSymbolicParseContext *context,
    const char *line_text, InstructionSpec *out_instruction, char *out_fallback_line,
  ...
  if (out_fallback_line != NULL && out_fallback_line_size != 0U)
    out_fallback_line[0] = '\0';
  if (context == NULL || line_text == NULL || out_instruction == NULL)
    return 0;
  ...
  if (space != NULL) {
    *space = '\0';
    ...
    if (*operand_text != '\0' &&
        !m68k_split_operands_in_place(operand_text, operands,
                                      M68K_INSTRUCTION_SPEC_MAX_OPERANDS,
                                      &operand_count)) {
      return 0;
    }
  }
  ...
  for (operand_index = 0; operand_index < operand_count; ++operand_index) {
    ...
    if (operand[0] == '#') {
      ...
      if (m68k_source_parse_constant_expression(operand + 1, lookup_constant_symbol_for_expression,
        ...
        snprintf(rewritten_operands[operand_index],
                 sizeof(rewritten_operands[operand_index]), "#%d",
                 (int32_t)immediate_value);
      } else if (context->is_symbol_name != NULL &&
                 context->is_symbol_name(operand + 1, context->user_data)) {
        snprintf(rewritten_operands[operand_index],
                 sizeof(rewritten_operands[operand_index]), "#0");
        snprintf(symbolic_names[operand_index],
                 sizeof(symbolic_names[operand_index]), "%s", operand + 1);
      } else {
        snprintf(rewritten_operands[operand_index],
                 sizeof(rewritten_operands[operand_index]), "%s", operand);
      }
    } else if (rewrite_absolute_constant_operand(context, operand,
                   rewritten_operands[operand_index], sizeof(rewritten_operands[operand_index]))) {
    } else if (rewrite_absolute_symbol_operand(context, operand,
                   rewritten_operands[operand_index], sizeof(rewritten_operands[operand_index]),
                   symbolic_names[operand_index], sizeof(symbolic_names[operand_index]))) {
    } else if (context->is_symbol_name != NULL &&
               context->is_symbol_name(operand, context->user_data) &&
               !is_register_like_symbol(operand, context->target_cpu) &&
               !context->lookup_symbol(operand, NULL, 1, context->user_data)) {
      snprintf(rewritten_operands[operand_index],
               sizeof(rewritten_operands[operand_index]), "%s",
               mnemonic_uses_relative_label_operand(mnemonic_text) ? "2"
                                                                   : "$0.l");
      snprintf(symbolic_names[operand_index],
               sizeof(symbolic_names[operand_index]), "%s", operand);
    } else if (rewrite_pc_relative_symbol_operand(context, operand,
  ...
  parse_ok =
      m68k_plain_parse_instruction_to_spec(rebuilt, context->target_cpu, out_instruction);
  if (!parse_ok)
    return 0;
  for (operand_index = 0; operand_index < operand_count; ++operand_index) {
    if (symbolic_names[operand_index][0] == '\0')
      continue;
...
int m68k_parse_instruction_with_symbol_fallback_ir( const M68kSymbolicParseContext *context, const char *line_text,
    M68kInstructionIR *out_instruction, char *out_fallback_line,
    size_t out_fallback_line_size) {
  InstructionSpec parsed;
  size_t operand_index;
  if (!m68k_parse_instruction_with_symbol_fallback_spec(context, line_text, &parsed,
                                                        out_fallback_line,
  ...
  for (operand_index = 0; operand_index < out_instruction->operand_count &&
                          operand_index < M68K_INSTRUCTION_SPEC_MAX_OPERANDS;
       ++operand_index) {
    if (parsed.operand_label_names[operand_index][0] == '\0')
      continue;
