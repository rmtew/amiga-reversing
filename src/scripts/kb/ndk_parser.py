...
def resolve_constant_value(expr: str, all_constants: dict[str, object], depth: int = 0) -> int | None:
    ...
    m = re.match(r'^([+-])(.+)$', expr)
    if m:
        val = resolve_constant_value(m.group(2), all_constants, depth + 1)
        if val is not None:
            ...
            for fname in sorted(os.listdir(subdir_path)):
                ...
                with open(fpath, encoding="utf-8", errors="replace") as f:
                    for line in f:
                        ...
                        lm = re.match(r'\s+LIBINIT(?:\s+(\S+))?\s*$', line)
                        if lm:
                            base_expr = lm.group(1) or "LIB_USERDEF"
                            lib_count = resolve_constant_value(base_expr, raw_constants)
                            continue

                        lm = re.match(r'\s+LIBDEF\s+(\w+)', line)
                        if lm:
        ...
        for func_name, fd_func in fd_info["functions"].items():
            ...
            if out_reg and (
                ...
                if name_inputs:
                    ...
                    if "Library *" in out_type:
                        name_inputs[0]["semantic_kind"] = "string_ptr"
                        name_inputs[0]["value_domain_name"] = "amiga.library_name"
                    elif out_name == "resource":
                        name_inputs[0]["semantic_kind"] = "string_ptr"
                        name_inputs[0]["value_domain_name"] = "amiga.resource_name"
