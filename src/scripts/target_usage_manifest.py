...
TYPE_PROVENANCE_NAMES = {
    ...
    TYPE_PROVENANCE_API_INPUT: "api_input",
}
...
DERIVED_TARGET_SUGGESTION_DECOMPRESSED_PAYLOAD_NAME = "decompressed_payload"
DERIVED_TARGET_SUGGESTION_KIND_NAMES = {
    DERIVED_TARGET_SUGGESTION_DECOMPRESSED_PAYLOAD: DERIVED_TARGET_SUGGESTION_DECOMPRESSED_PAYLOAD_NAME,
}
PROJECT_ORIGIN_KIND_DERIVED_DECOMPRESSED_PAYLOAD = 1
PROJECT_ORIGIN_KIND_DERIVED_DECOMPRESSED_PAYLOAD_NAME = "derived_decompressed_payload"
PROJECT_ORIGIN_KIND_NAMES = {
    PROJECT_ORIGIN_KIND_DERIVED_DECOMPRESSED_PAYLOAD: PROJECT_ORIGIN_KIND_DERIVED_DECOMPRESSED_PAYLOAD_NAME,
}
PROJECT_TARGET_ROLE_DECOMPRESSED_PAYLOAD = 1
PROJECT_TARGET_ROLE_DECOMPRESSED_PAYLOAD_NAME = "decompressed_payload"
PROJECT_TARGET_ROLE_NAMES = {
    PROJECT_TARGET_ROLE_DECOMPRESSED_PAYLOAD: PROJECT_TARGET_ROLE_DECOMPRESSED_PAYLOAD_NAME,
}
...
def _project_target_classifier_name(entry: dict[str, Any], id_key: str, text_key: str,
    names: dict[int, str]) -> str | None:
    value_id = _int_value(entry.get(id_key))
    if value_id is not None:
        resolved = names.get(value_id)
        if resolved:
            return resolved
    return _string_value(entry.get(text_key))


def _project_target_metadata_classifiers(entry: dict[str, Any]) -> tuple[str | None, str | None, str | None,
    str | None, str | None]:
    return (
        _project_target_classifier_name(
            entry, "project_origin_kind_id", "project_origin_kind", PROJECT_ORIGIN_KIND_NAMES),
        _project_target_classifier_name(entry, "target_role_id", "target_role", PROJECT_TARGET_ROLE_NAMES),
        _project_target_classifier_name(entry, "payload_role_id", "payload_role", DECOMPRESSION_PAYLOAD_ROLE_NAMES),
        _project_target_classifier_name(
            entry, "payload_role_confidence_id", "payload_role_confidence",
            DECOMPRESSION_PAYLOAD_ROLE_CONFIDENCE_NAMES),
        _project_target_classifier_name(
            entry, "parent_remains_active_id", "parent_remains_active",
            DECOMPRESSION_PARENT_REMAINS_ACTIVE_NAMES),
    )


def _add_project_target_metadata_features(entry: dict[str, Any], bag: FeatureBag) -> None:
    origin_kind, target_role, payload_role, payload_role_confidence, parent_remains_active = (
        _project_target_metadata_classifiers(entry)
    )
    ...
    if (origin_kind == PROJECT_ORIGIN_KIND_DERIVED_DECOMPRESSED_PAYLOAD_NAME or
        target_role == PROJECT_TARGET_ROLE_DECOMPRESSED_PAYLOAD_NAME):
...
def _symbol_origin_kind(origin: dict[str, Any]) -> str:
    return (
        _string_value(origin.get("origin_kind"))
        or _string_value(origin.get("origin_kind_name"))
        or "unknown"
    )
...
def _symbol_origin_example(section_index: int | None, origin: dict[str, Any]) -> dict[str, object]:
    origin_section = _int_value(origin.get("section_index"), section_index)
    offset = _int_value(origin.get("offset"))
    symbol_name = _string_value(origin.get("symbol_name")) or _string_value(origin.get("symbol")) or "unknown"
    example = _offset_example(origin_section, offset, symbol_name)
    for key in ("source_section_index", "source_offset", "origin_kind_id", "confidence"):
        value = _int_value(origin.get(key))
        if value is not None:
            example[key] = value
    for key in ("symbol_name", "symbol", "origin_kind", "origin_kind_name", "detail"):
        value = _string_value(origin.get(key))
        if value:
            example[key] = value
    return example
...
def _symbol_origin_features(origin: dict[str, Any]) -> list[str]:
    kind = _symbol_origin_kind(origin)
    features = [
        "analysis:symbol_origin",
        f"analysis:symbol_origin_kind:{_safe_part(kind)}",
    ]
...
def _expected_symbol_access_kind(access: dict[str, Any]) -> str:
    return (
...
def _expected_symbol_access_features(access: dict[str, Any]) -> list[str]:
    ...
    if _int_value(access.get("operand_index")) is not None:
        features.append("analysis:expected_symbol_access:operand")
    return list(dict.fromkeys(features))


def _add_expected_symbol_access_features(
    bag: FeatureBag, section_index: int | None, access: dict[str, Any]
) -> None:
    example = _expected_symbol_access_example(section_index, access)
    for feature in _expected_symbol_access_features(access):
        bag.add(feature, example=example)
...
def _symbol_origin_xrefs(
    ...
    *,
    ...
    row_index, stable_key, row_text = _row_location(row_locations, origin_section, offset)
    symbol_name = _string_value(origin.get("symbol_name")) or _string_value(origin.get("symbol")) or "unknown"
    value = _int_value(origin.get("source_offset"))
    if value is None:
        value = offset
    return [
        _xref(
            ...
            symbol=symbol_name,
            value=value,
            text=row_text or _string_value(origin.get("detail")) or f"symbol origin {symbol_name}",
        )
        for feature in _symbol_origin_features(origin)
    ]


def _expected_symbol_access_xrefs(
    row: dict[str, object],
    access: dict[str, Any],
    row_locations: dict[tuple[int, int], tuple[int, str | None, str | None]],
    *,
    ...
    return [
        _xref(
            ...
            symbol=symbol_name,
            value=value,
            text=row_text or _string_value(access.get("detail")) or f"expected symbol access {symbol_name}",
        )
        for feature in _expected_symbol_access_features(access)
    ]
