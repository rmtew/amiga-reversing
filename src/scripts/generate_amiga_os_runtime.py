...
def write_source(rows: list[tuple[str, str, int, str, dict]],
    ...
    for library_name, base_name, lvo, function_name, other_info in rows:
        ...
        lines.append(
            "  { %s, %s, %d, %s, %s, %du, %du, %du, %du, %s, %s, %s, %du, %du, { %du, %du, %s, %s, %s, %s, %s } },"
            % (
                ...
                "NULL" if available_since is None else f'"{c_string(available_since)}"',
    ...
    lines.extend(
        [
            ...
            "  if (!amiga_os_parse_compatibility_version_rank(version, &rank) || rank > 65535UL || out_rank == NULL)",
            "    return 0;",
            "  *out_rank = (uint16_t)rank;",
            "  return 1;",
            "}",
            "",
            "AmigaOsCompatVersion amiga_os_parse_compatibility_version(const char *version) {",
