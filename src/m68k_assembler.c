...
static int m68k_asm_value_fits_absolute_word(uint32_t value) {
  return value <= 0xFFFFU || m68k_asm_value_fits_signed_word(value);
}
    ...
    case M68K_ASM_EXTENSION_EA_SINGLE_WORD:
      if (((operand->kind == M68K_ASM_OPERAND_EA || operand->kind == M68K_ASM_OPERAND_BF_EA) &&
        ...
        if (operand->ea_mode == 7 && operand->ea_reg == 0 &&
            !m68k_asm_value_fits_absolute_word(operand->value)) return -7;
