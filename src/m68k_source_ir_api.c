...
M68K_ASM_EXPORT int m68k_source_ir_parse_file(const char *path, const char *include_dir, uint8_t target_cpu,
  ...
  if (!m68k_source_pipeline_parse_and_layout(&source, path, out_error, out_error_size) ||
      !m68k_source_pipeline_build_ir(&source, out_source_file, out_error, out_error_size)) {
...
M68K_ASM_EXPORT int m68k_source_ir_render_with_policy( const M68kSourceFileIR *source_file,
    const M68kRenderPolicy *policy, char **out_text, char *out_error, size_t out_error_size) {
  return m68k_source_ir_render_text_with_policy(source_file, policy, out_text, out_error, out_error_size);
}
