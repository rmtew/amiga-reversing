...
static const char *type_provenance_kind_name(uint8_t kind) {
  ...
  case M68K_PLATFORM_TYPE_PROVENANCE_API_INPUT:
    return "api_input";
...
static int append_amiga_target_metadata_json(JsonBuilder *builder, const M68kBackend *backend,
  ...
  if (backend == NULL || backend->platform_kind != M68K_PLATFORM_BACKEND_AMIGA_HUNK) return 0;
...
static int source_analysis_app_slot_range_conflicts_with_non_app_layout(
  ...
  for (index = 0U; index < source_analysis->base_layout_field_count; ++index) {
    ...
    if (field->layout_kind == M68K_BASE_LAYOUT_KIND_APP) continue;
    if (!source_analysis_base_layout_field_uses_app_base(field)) continue;
    if (memory_layout_ranges_overlap(range_start, range_size, field->offset, field->size)) return 1;
  }
  return 0;
}

static int platform_typed_access_owner_range(const M68kRecoveredPlatformTypedAccessIR *access,
...
static int append_source_analysis_memory_layout_records_json(JsonBuilder *builder,
  ...
  for (section_index = 0U; section_index < source_analysis->section_count; ++section_index) {
    ...
    for (typed_access_index = 0U; typed_access_index < section->recovered_platform_typed_access_count;
      ...
      uint8_t conflicted;
      uint8_t conflict_state;
      (void)platform_typed_access_owner_range(access, &owner_range_start, &owner_range_size);
      conflicted = access->type_provenance_kind == M68K_PLATFORM_TYPE_PROVENANCE_APP_SLOT
        ? (uint8_t)source_analysis_app_slot_range_conflicts_with_non_app_layout(source_analysis,
            owner_range_start, owner_range_size)
        : 0U;
    ...
    for (unresolved_typed_access_index = 0U;
      ...
      uint8_t conflicted = access->type_provenance_kind == M68K_PLATFORM_TYPE_PROVENANCE_APP_SLOT
        ? (uint8_t)source_analysis_app_slot_range_conflicts_with_non_app_layout(source_analysis, owner_range_start,
            owner_range_size)
        : 0U;
      uint8_t conflict_state = conflicted ? M68K_ANALYSIS_CONFLICT_STATE_CONFLICTED :
        M68K_ANALYSIS_CONFLICT_STATE_CLEAN;
...
int source_analysis_to_json(const M68kSourceAnalysisIR *source_analysis, char **out_json, M68kDiagSink diagnostics) {
  ...
  for (section_index = 0; section_index < source_analysis->section_count; ++section_index) {
    ...
    if (json_builder_appendf(&builder, "],\"data_reference_count\":%u,\"data_references\":[",
          (unsigned)section->data_reference_count) != 0)
      goto oom;
    for (data_reference_index = 0; data_reference_index < section->data_reference_count;
        ++data_reference_index) {
      const M68kDataReferenceIR *ref = &section->data_references[data_reference_index];
      if (data_reference_index != 0U && json_builder_append(&builder, ",") != 0)
        goto oom;
      if (json_builder_appendf(&builder,
          "{\"source_offset\":%u,\"source_kind\":%u,\"source_kind_name\":",
          (unsigned)ref->source_offset, (unsigned)ref->source_kind) != 0)
        goto oom;
      if (json_builder_append_json_string(&builder,
          m68k_data_reference_source_kind_name(ref->source_kind)) != 0)
        goto oom;
      if (json_builder_appendf(&builder,
          ",\"table_start_offset\":%u,\"table_entry_index\":%u,\"table_entry_offset\":%u,"
          "\"table_entry_size\":%u,\"raw_value\":%u,\"table_kind_id\":%u,\"table_kind\":",
          (unsigned)ref->table_start_offset, (unsigned)ref->table_entry_index,
          (unsigned)ref->table_entry_offset, (unsigned)ref->table_entry_size,
          (unsigned)ref->raw_value, (unsigned)ref->table_kind_id) != 0)
        goto oom;
      if (json_builder_append_nullable_string(&builder,
          m68k_analysis_table_kind_name(ref->table_kind_id)) != 0)
        goto oom;
      if (json_builder_appendf(&builder,
          ",\"source_pattern_id\":%u,\"source_pattern\":",
          (unsigned)ref->source_pattern_id) != 0)
        goto oom;
      if (json_builder_append_nullable_string(&builder,
          m68k_analysis_structured_data_source_pattern_name(ref->source_pattern_id)) != 0)
        goto oom;
      if (json_builder_appendf(&builder,
          ",\"target_section_index\":%u,\"target_offset\":%u,"
          "\"target_status\":%u,\"target_status_name\":",
          (unsigned)ref->target_section_index, (unsigned)ref->target_offset,
          (unsigned)ref->target_status) != 0)
        goto oom;
      if (json_builder_append_json_string(&builder,
    ...
    for (orphan_signal_index = 0; orphan_signal_index < section->orphan_code_signal_count; ++orphan_signal_index) {
      ...
      if (json_builder_appendf(&builder, ",\"nearby_data_flags\":%u",
        ...
        goto oom;
      if (json_builder_appendf(&builder, ",\"arbitration_flags\":%u",
          (unsigned)signal->arbitration_flags) != 0)
        goto oom;
      if (json_builder_appendf(&builder, ",\"nearby_data_table_kind_id\":%u,\"nearby_data_table_kind\":",
          (unsigned)signal->nearby_data_table_kind_id) != 0)
        goto oom;
      if (json_builder_append_nullable_string(&builder,
          m68k_analysis_table_kind_name(signal->nearby_data_table_kind_id)) != 0)
        goto oom;
...
int source_file_listing_runtime_address_row_from_render_plan_with_index(const M68kRenderPlan *render_plan,
  ...
  plan_row = m68k_render_plan_find_row_for_runtime_address(render_plan, address);
  if (plan_row == NULL || plan_row < render_plan->rows || plan_row >= render_plan->rows + render_plan->row_count)
    return 0;
  plan_row_index = (size_t)(plan_row - render_plan->rows);
  for (index = 0U; index < row_index->row_count; ++index) {
