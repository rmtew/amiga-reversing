...
#define M68K_FACTS_V2_TRACE_LONG_TARGET_TABLE 7U
...
typedef struct M68kFactsV2TraceValue {
  uint8_t kind;
  uint8_t has_table_base_address;
  uint8_t has_origin;
  uint8_t reserved;
...
static int scan_indexed_direct_variable_stub_entries(M68kDecodeIR *decode, uint8_t max_cpu,
  ...
  uint8_t **accepted_bytes, uint32_t *out_entries, uint32_t entry_limit, uint32_t *out_entry_count);
static int enqueue_long_target_table_for_indirect_site(M68kDecodeIR *decode, M68kFactIR *facts,
  ...
  const M68kDecodeCandidate *site_candidate, const M68kFactsV2TraceValue *table,
...
static int enqueue_biased_word_relative_targets_for_indexed_operand(M68kDecodeIR *decode, M68kFactIR *facts,
  ...
  const M68kRuntimeAddressSpace *runtime_addresses, const M68kFactsV2TraceValue *table_value,
  const M68kFactsV2TraceState *trace_state, int *out_enqueued);
static int enqueue_word_relative_table_targets_for_indirect_site(M68kDecodeIR *decode, M68kFactIR *facts,
  M68kFactsV2WorkQueue *queue, uint8_t **accepted_start, uint8_t **accepted_bytes,
  M68kFactsV2Profile *profile, uint8_t max_cpu, size_t section_index,
...
static int trace_value_equal(const M68kFactsV2TraceValue *lhs, const M68kFactsV2TraceValue *rhs) {
  uint32_t index;
  if (lhs == NULL || rhs == NULL) return lhs == rhs;
  if (lhs->kind != rhs->kind ||
      lhs->has_table_base_address != rhs->has_table_base_address ||
      lhs->has_origin != rhs->has_origin ||
      lhs->origin_operand_index != rhs->origin_operand_index ||
      lhs->section_index != rhs->section_index ||
      lhs->value != rhs->value ||
      lhs->table_base_address != rhs->table_base_address ||
      lhs->origin_section_index != rhs->origin_section_index ||
      lhs->origin_offset != rhs->origin_offset ||
      lhs->code_start_reason != rhs->code_start_reason ||
      lhs->target_count != rhs->target_count) {
    return 0;
  }
  if (lhs->target_count == 0U) return 1;
  if (lhs->targets == rhs->targets) return 1;
  if (lhs->targets == NULL || rhs->targets == NULL) return 0;
  for (index = 0U; index < lhs->target_count; ++index) {
    if (lhs->targets[index] != rhs->targets[index]) return 0;
  }
  return 1;
}

static int trace_state_equal(const M68kFactsV2TraceState *lhs, const M68kFactsV2TraceState *rhs) {
  size_t index;
  if (lhs == NULL || rhs == NULL) return lhs == rhs;
  for (index = 0U; index < 8U; ++index) {
    if (!trace_value_equal(&lhs->d[index], &rhs->d[index]) ||
        !trace_value_equal(&lhs->a[index], &rhs->a[index])) {
      return 0;
    }
  }
  if (lhs->d_low16_known != rhs->d_low16_known ||
      lhs->d_low16_has_origin != rhs->d_low16_has_origin ||
      memcmp(lhs->d_low16, rhs->d_low16, sizeof(lhs->d_low16)) != 0 ||
      memcmp(lhs->d_low16_origin_offset, rhs->d_low16_origin_offset,
        sizeof(lhs->d_low16_origin_offset)) != 0 ||
      lhs->reglist_copy_valid != rhs->reglist_copy_valid ||
      lhs->reglist_copy_width != rhs->reglist_copy_width ||
      lhs->reglist_copy_mask != rhs->reglist_copy_mask ||
      lhs->reglist_copy_section_index != rhs->reglist_copy_section_index ||
      lhs->reglist_copy_offset != rhs->reglist_copy_offset ||
      lhs->reglist_copy_size != rhs->reglist_copy_size ||
      lhs->copied_entry_valid != rhs->copied_entry_valid ||
      lhs->copied_entry_dest_reg != rhs->copied_entry_dest_reg ||
      lhs->copied_entry_section_index != rhs->copied_entry_section_index ||
      lhs->copied_entry_source_offset != rhs->copied_entry_source_offset ||
      lhs->copied_entry_size != rhs->copied_entry_size) {
    return 0;
  }
  for (index = 0U; index < M68K_FACTS_V2_TRACE_ABSOLUTE_SLOT_LIMIT; ++index) {
    if (lhs->absolute_slots[index].used != rhs->absolute_slots[index].used ||
        lhs->absolute_slots[index].address != rhs->absolute_slots[index].address ||
        !trace_value_equal(&lhs->absolute_slots[index].value, &rhs->absolute_slots[index].value)) {
      return 0;
    }
  }
  for (index = 0U; index < M68K_FACTS_V2_TRACE_STACK_SLOT_LIMIT; ++index) {
    if (lhs->stack_slots[index].used != rhs->stack_slots[index].used ||
        lhs->stack_slots[index].displacement != rhs->stack_slots[index].displacement ||
        !trace_value_equal(&lhs->stack_slots[index].value, &rhs->stack_slots[index].value)) {
      return 0;
    }
  }
  return 1;
...
static int trace_state_matches_queued(const M68kFactsV2TraceState *queued,
  ...
  if (incoming != NULL) return trace_state_equal(queued, incoming);
  trace_state_init_unknown(&unknown);
  return trace_state_equal(queued, &unknown);
}
...
static int trace_value_can_drive_indirect_control(const M68kFactsV2TraceValue *value) {
  ...
  return value->kind == M68K_FACTS_V2_TRACE_SOURCE_OFFSET ||
    ...
    value->kind == M68K_FACTS_V2_TRACE_LONG_TARGET_TABLE ||
...
static int trace_state_can_drive_indirect_control(const M68kFactsV2TraceState *state) {
  ...
  for (reg = 0U; reg < 8U; ++reg) {
    ...
    if (trace_value_is_target_set(&state->d[reg]) ||
        state->d[reg].kind == M68K_FACTS_V2_TRACE_LONG_TARGET_TABLE ||
...
static int label_lookup_append_point(M68kFactsV2LabelLookup *lookup, size_t section_index, uint32_t offset,
    uint8_t confidence) {
  M68kAnalysisLabelPoint *grown;
  size_t next_capacity;
  if (lookup == NULL || lookup->arena == NULL) return -1;
  if (lookup->point_count == lookup->point_capacity) {
    next_capacity = lookup->point_capacity == 0U ? 64U : lookup->point_capacity * 2U;
    grown = (M68kAnalysisLabelPoint *)arena_realloc_copy(lookup->arena, lookup->points,
      lookup->point_capacity * sizeof(*grown), next_capacity * sizeof(*grown));
    if (grown == NULL) return -1;
    lookup->points = grown;
    lookup->point_capacity = next_capacity;
  }
  lookup->points[lookup->point_count].section_index = section_index;
  lookup->points[lookup->point_count].offset = offset;
  lookup->points[lookup->point_count].confidence = confidence;
  ++lookup->point_count;
  return 0;
}

static int label_lookup_create_label(M68kFactsV2LabelLookup *lookup,
  ...
  if (lookup != NULL && section_index < lookup->section_count && lookup->labels != NULL &&
    ...
    return label_lookup_append_point(lookup, section_index, offset, confidence);
  }
  return -1;
}
...
static int seed_facts_from_object(const M68kObject *object, const M68kAnalysisPolicy *policy,
  ...
  for (section_index = 0U; section_index < object->section_count; ++section_index) {
    ...
    if (section->kind == M68K_SECTION_CODE && section->data_size != 0U &&
      ...
      if (label_lookup_create_label(label_lookup, section_index, 0U,
  ...
  if (policy->has_entry_offset && object->section_count != 0U) {
    ...
    if (policy->entry_offset <= extent) {
      ...
      if (label_lookup_create_label(label_lookup, 0U, policy->entry_offset,
  ...
  for (entry_index = 0U; entry_index < policy->runtime_entry_point_count &&
    ...
    if (label_lookup_create_label(label_lookup, target_section, target_offset,
        M68K_FACT_CONFIDENCE_REQUIRED) != 0) return -1;
  }
  for (label_index = 0U; label_index < policy->named_label_count; ++label_index) {
    ...
    if (target_section >= object->section_count) continue;
    if (label_lookup_create_label(label_lookup, target_section, label->offset,
        M68K_FACT_CONFIDENCE_REQUIRED) != 0)
      return -1;
  }
  for (symbol_index = 0U; symbol_index < object->symbol_count; ++symbol_index) {
    ...
    if (label_lookup_create_label(label_lookup, symbol->section_index, symbol->value,
        M68K_FACT_CONFIDENCE_REQUIRED) != 0) return -1;
  }
...
static int trace_value_set_target_set(M68kFactsV2TraceValue *value, Arena *arena, size_t section_index,
    const uint32_t *targets, uint32_t target_count) {
  uint32_t index;
  uint32_t *copied_targets;
  if (value == NULL) return -1;
  memset(value, 0, sizeof(*value));
  value->kind = M68K_FACTS_V2_TRACE_TARGET_SET;
  value->section_index = section_index;
  if (targets == NULL || target_count == 0U) return 0;
  if (arena == NULL || target_count > SIZE_MAX / sizeof(*copied_targets)) {
    trace_value_set_unknown(value);
    return -1;
  }
  copied_targets = (uint32_t *)arena_alloc(arena, (size_t)target_count * sizeof(*copied_targets));
  if (copied_targets == NULL) {
    trace_value_set_unknown(value);
    return -1;
  }
  value->target_count = target_count;
  value->targets = copied_targets;
  for (index = 0U; index < target_count; ++index) copied_targets[index] = targets[index];
  return 0;
}

static void trace_value_set_long_target_table(M68kFactsV2TraceValue *value, size_t section_index,
    uint32_t table_offset) {
  if (value == NULL) return;
  memset(value, 0, sizeof(*value));
  value->kind = M68K_FACTS_V2_TRACE_LONG_TARGET_TABLE;
...
static void trace_value_set_word_relative_table_with_base(M68kFactsV2TraceValue *value, size_t section_index,
    uint32_t table_offset, uint32_t base_address) {
  trace_value_set_word_relative_table(value, section_index, table_offset);
  if (value == NULL) return;
  value->has_table_base_address = 1U;
  value->table_base_address = base_address;
}
...
static void profile_record_table_target_set_limit_hit(M68kFactsV2Profile *profile,
    size_t section_index, uint32_t table_offset, uint32_t capacity) {
  if (profile == NULL) return;
  if (profile->table_target_set_limit_hits == 0U) {
    profile->first_table_target_set_limit_section = (uint32_t)section_index;
    profile->first_table_target_set_limit_offset = table_offset;
    profile->first_table_target_set_limit_capacity = capacity;
  }
  ++profile->table_target_set_limit_hits;
}
...
static uint32_t scan_long_control_target_table(const M68kDecodeSectionIR *section,
    ...
    uint32_t *targets, uint32_t target_limit, M68kFactsV2Profile *profile) {
  uint32_t cursor;
  ...
  for (cursor = table_offset; cursor + 4U <= section->size; cursor += 4U) {
    ...
    if (targets != NULL) {
      if (target_count >= target_limit) {
        profile_record_table_target_set_limit_hit(profile, section->section_index, table_offset, target_limit);
        return 0U;
      }
...
static int trace_state_candidate_loads_long_target_table(const M68kDecodeSectionIR *section,
    ...
    M68kFactsV2TraceValue *out_value, M68kFactsV2TableBaseRef *out_base_ref, M68kFactsV2Profile *profile) {
  M68kInstructionIR instruction;
  const M68kSimFormMetadata *metadata;
  uint8_t dest_reg = 0U;
  uint8_t base_reg = 0U;
  ...
  (void)profile;
  target_count = scan_long_control_target_table(section, runtime_addresses, accepted_bytes, table_offset, NULL,
    0U, NULL);
  if (target_count == 0U && table_offset <= UINT32_MAX - 4U &&
    ...
    target_count = scan_long_control_target_table(section, runtime_addresses, accepted_bytes, scan_offset, NULL,
      0U, NULL);
  }
  ...
  *out_dest_reg = dest_reg;
  trace_value_set_long_target_table(out_value, section->section_index, scan_offset);
  return 1;
}
...
static int trace_state_candidate_swaps_keyed_long_relative_table_entry(const M68kDecodeSectionIR *section,
    ...
    const M68kRuntimeAddressSpace *runtime_addresses, const uint8_t *accepted_start,
    const uint8_t *accepted_bytes, uint8_t *out_dest_reg, M68kFactsV2TraceValue *out_value,
    Arena *arena, M68kFactsV2Profile *profile) {
  M68kInstructionIR instruction;
  const M68kSimFormMetadata *metadata;
  uint8_t reg = 0U;
  uint32_t table_offset = 0U;
  ...
  target_count = scan_keyed_long_relative_control_target_table(section, runtime_addresses, accepted_start,
    accepted_bytes, table_offset, base_address, NULL, 0U, NULL);
  if (target_count == 0U) return 0;
  if (arena == NULL || target_count > SIZE_MAX / sizeof(*targets)) return 0;
  targets = (uint32_t *)arena_alloc(arena, (size_t)target_count * sizeof(*targets));
  if (targets == NULL) return -1;
  if (scan_keyed_long_relative_control_target_table(section, runtime_addresses, accepted_start, accepted_bytes,
      table_offset, base_address, targets, target_count, profile) != target_count) {
    return 0;
  }
  *out_dest_reg = reg;
  if (trace_value_set_target_set(out_value, arena, section->section_index, targets, target_count) != 0)
    return -1;
  return 1;
}
...
static int trace_state_after_candidate(size_t section_index, const M68kDecodeSectionIR *section,
    const M68kDecodeCandidate *candidate, const M68kFactsV2TraceState *before,
    ...
    M68kFactsV2TraceState *after, Arena *arena, M68kFactsV2Profile *profile) {
  ...
  has_table_targets = trace_state_candidate_loads_long_target_table(section, candidate, before, runtime_addresses,
    accepted_bytes, &table_dest_reg, &table_targets, NULL, profile);
  if (!has_table_targets) {
    has_table_targets = trace_state_candidate_adds_word_target_table(section, candidate, before,
      runtime_addresses, accepted_start, accepted_bytes, &table_dest_reg, &table_targets, profile);
  }
  if (!has_table_targets) {
  ...
  if (!has_table_targets) {
    int swap_result = trace_state_candidate_swaps_keyed_long_relative_table_entry(section, candidate, before,
      runtime_addresses, accepted_start, accepted_bytes, &table_dest_reg, &table_targets, arena, profile);
    if (swap_result < 0) return -1;
    has_table_targets = swap_result;
...
static void facts_v2_classify_absolute_operand_observation(uint8_t platform_kind,
    const M68kRuntimeAddressSpace *runtime_addresses, size_t section_index, uint32_t section_size,
    uint32_t address, M68kAddressObservationIR *observation) {
...
static int append_address_observations_for_accepted(const M68kDecodeIR *decode, const M68kFactIR *facts,
  ...
  for (section_index = 0U; section_index < decode->section_count; ++section_index) {
    ...
    for (candidate_index = 0U; candidate_index < section->candidate_count; ++candidate_index) {
      ...
      for (operand_index = 0U; operand_index < candidate->operand_count && operand_index < 4U &&
        ...
        observation.access_width = has_absolute_operand && facts_v2_absolute_ref_access_kind(access_kind)
        ...
        observation.confidence = (uint8_t)M68K_FACT_CONFIDENCE_TOOL_INFERRED;
        ...
        if (relocation != NULL && relocation->target_section_index < decode->section_count) {
          ...
          facts_v2_classify_absolute_operand_observation(platform_kind, runtime_addresses, section_index,
            section->size,
            address, &observation);
        }
        observation.conflicted = owner_section_index < decode->section_count ?
          (uint8_t)facts_v2_access_overlaps_accepted_code(accepted_bytes[owner_section_index],
            decode->sections[owner_section_index].size, observation.owner_offset, observation.access_width,
            access_kind) : 0U;
        observation.conflict_state = observation.conflicted ? M68K_ANALYSIS_CONFLICT_STATE_CODE_OVERLAP :
...
static int enqueue_word_relative_table_targets_for_indirect_site(M68kDecodeIR *decode, M68kFactIR *facts,
  ...
  if (decode == NULL || facts == NULL || queue == NULL || accepted_start == NULL || accepted_bytes == NULL ||
      profile == NULL || site_candidate == NULL || runtime_addresses == NULL || section == NULL ||
      out_enqueued == NULL || section_index >= decode->section_count || section->data == NULL ||
      table_offset >= section->size) {
    return 0;
  }
  ...
  for (cursor = table_offset; cursor + 2U <= section->size; cursor += 2U) {
    ...
    if (skip_zero_displacement && displacement == 0) continue;
    {
      ...
      if (control_address_to_section_offset(runtime_addresses, section->section_index, section->size,
          base_address, &base_source_offset) &&
          emitted_target_count >= 2U && cursor == base_source_offset && displacement == 0) {
        break;
      }
    }
    if (saw_far_displacement && emitted_target_count >= 2U && cursor != table_offset && displacement == 0 &&
        cursor + 4U <= section->size && m68k_read_u16be(section->data + cursor + 2U) == 0U) {
      break;
    }
    if (displacement < -M68K_FACTS_V2_WORD_DISPATCH_LOCAL_LIMIT ||
    ...
    if (target_offset > table_offset && target_offset < first_forward_target)
      first_forward_target = target_offset;
    if (emitted_target_count >= 2U && first_forward_target != UINT32_MAX &&
        cursor + 2U >= first_forward_target) {
      break;
    }
    *out_enqueued = 1;
  }
...
static int enqueue_biased_word_relative_targets_for_indexed_operand(M68kDecodeIR *decode, M68kFactIR *facts,
  ...
  if (decode == NULL || table_value == NULL || trace_state == NULL || out_enqueued == NULL ||
      section_index >= decode->section_count || table_value->kind != M68K_FACTS_V2_TRACE_WORD_RELATIVE_TABLE ||
      table_value->section_index != section_index) {
    return 0;
  }
  section = &decode->sections[section_index];
  if (!indexed_control_operand_base_ref(section, candidate, operand_index, runtime_addresses, trace_state,
    ...
    if (!trace_state_control_target_operand_address(candidate, operand_index, section_index, trace_state,
      ...
      if (indexed_control_operand_data_index_reg(candidate, operand_index, &reg)) {
        ...
        if (value->kind == M68K_FACTS_V2_TRACE_WORD_RELATIVE_TABLE) {
          ...
          if (enqueue_biased_word_relative_targets_for_indexed_operand(decode, facts, queue, accepted_start,
              accepted_bytes, profile, max_cpu, section_index, candidate, operand_index, runtime_addresses,
              value, trace_state, &target_set_enqueued) != 0)
            return -1;
      ...
      if (value != NULL && value->kind == M68K_FACTS_V2_TRACE_LONG_TARGET_TABLE &&
          value->section_index == section_index) {
        if (enqueue_long_target_table_for_indirect_site(decode, facts, queue, accepted_start, accepted_bytes,
            profile, max_cpu, section_index, candidate, value, runtime_addresses, &decode->sections[section_index],
            trace_state) != 0) {
          return -1;
        }
        enqueued = 1;
      ...
      if (value != NULL && value->kind == M68K_FACTS_V2_TRACE_WORD_RELATIVE_TABLE &&
          value->section_index == section_index && value->has_table_base_address) {
        ...
        if (word_table_enqueued) enqueued = 1;
        continue;
      }
      {
        int target_set_enqueued = 0;
...
static int enqueue_immediate_indirect_target_set(M68kDecodeIR *decode, M68kFactIR *facts,
  ...
  for (reg = 0U; reg < 8U; ++reg) {
    ...
    if (value->kind == M68K_FACTS_V2_TRACE_LONG_TARGET_TABLE && value->section_index == section_index) {
      if (enqueue_long_target_table_for_indirect_site(decode, facts, queue, accepted_start, accepted_bytes,
          profile, max_cpu, section_index, next_candidate, value, runtime_addresses, section, trace_state) != 0) {
        return -1;
      }
      continue;
    }
    if (value->kind == M68K_FACTS_V2_TRACE_WORD_RELATIVE_TABLE && value->section_index == section_index &&
        value->has_table_base_address) {
      int enqueued = 0;
      if (enqueue_word_relative_table_targets_for_indirect_site(decode, facts, queue, accepted_start,
...
static int enqueue_target_set_for_indirect_site(M68kDecodeIR *decode, M68kFactIR *facts,
  ...
  uint32_t index;
  uint32_t source_offset;
  if (site_candidate == NULL || targets == NULL) return 0;
  if (targets->kind == M68K_FACTS_V2_TRACE_LONG_TARGET_TABLE) {
    return enqueue_long_target_table_for_indirect_site(decode, facts, queue, accepted_start, accepted_bytes,
      profile, max_cpu, section_index, site_candidate, targets, runtime_addresses, section, NULL);
  }
  if (targets->kind == M68K_FACTS_V2_TRACE_WORD_RELATIVE_TABLE && targets->has_table_base_address) {
    int enqueued = 0;
...
static int enqueue_long_target_table_for_indirect_site(M68kDecodeIR *decode, M68kFactIR *facts,
  ...
  uint32_t target_count = 0U;
  uint32_t source_offset;
  if (site_candidate == NULL || table == NULL || table->kind != M68K_FACTS_V2_TRACE_LONG_TARGET_TABLE ||
      table->section_index != section_index || section == NULL || section->data == NULL ||
      table->value >= section->size) {
    return 0;
  }
  source_offset = site_candidate->offset;
  trace_state_init_unknown(&unknown_state);
  for (cursor = table->value; cursor + 4U <= section->size; cursor += 4U) {
    uint32_t target = m68k_read_u32be(section->data + cursor);
    uint32_t target_offset = 0U;
    if (accepted_range_has_code_byte_local(accepted_bytes[section_index], section->size, cursor, 4U)) break;
    if (target == 0U) continue;
    if ((target & 1U) != 0U || !control_address_to_section_offset(runtime_addresses,
        section->section_index, section->size, target, &target_offset)) {
      break;
...
static int collect_callback_field_target_from_store(M68kDecodeIR *decode,
  ...
  for (index = count; index > 0U; --index) {
    ...
    if (trace_state_after_candidate(section_index, section, candidate, &state, relocation_lookup, NULL,
        runtime_addresses, accepted_start[section_index], accepted_bytes[section_index], &next_state, arena,
        NULL) != 0)
      return -1;
    state = next_state;
  }
...
static int trace_callback_site_linear_state(const M68kFactsV2RelocationLookup *relocation_lookup,
    const M68kRuntimeAddressSpace *runtime_addresses, uint8_t **accepted_start, uint8_t **accepted_bytes,
    size_t section_index, const M68kDecodeSectionIR *section, const M68kDecodeCandidate *site_candidate,
    Arena *arena, M68kFactsV2TraceState *out_state) {
  ...
  for (index = count; index > 0U; --index) {
    const M68kDecodeCandidate *candidate = previous[index - 1U];
    M68kFactsV2TraceState next_state;
    if (trace_state_after_candidate(section_index, section, candidate, out_state, relocation_lookup, NULL,
        runtime_addresses, accepted_start[section_index], accepted_bytes[section_index], &next_state, arena,
        NULL) != 0)
      return 0;
...
static int enqueue_callback_field_indirect_targets(M68kDecodeIR *decode, M68kFactIR *facts,
  ...
  (void)trace_callback_site_linear_state(relocation_lookup, runtime_addresses, accepted_start, accepted_bytes,
    section_index, section, site_candidate, queue->arena, &site_state);
  for (index = 0U; index < targets->count; ++index) {
    const M68kFactsV2CallbackFieldTarget *target = &targets->items[index];
    if (target->section_index != section_index || target->base_reg != base_reg ||
        target->displacement != displacement) {
...
static int enqueue_backward_sliced_indirect_table_targets(M68kDecodeIR *decode, M68kFactIR *facts,
  ...
  for (candidate_index = 0U; candidate_index < count; ++candidate_index) {
    ...
    for (replay_index = count; replay_index > candidate_index + 1U; --replay_index) {
      ...
      M68kFactsV2TraceState next_state;
      if (trace_state_after_candidate(section_index, section, candidate, &state, relocation_lookup, facts,
          runtime_addresses, accepted_start[section_index], accepted_bytes[section_index], &next_state, queue->arena,
          profile) != 0)
        return -1;
      state = next_state;
    }
    ...
    if (trace_state_candidate_loads_long_target_table(section, cursor_candidate, &state, runtime_addresses,
        accepted_bytes[section_index], &dest_reg, &targets, &base_ref, profile) && dest_reg == control_reg) {
    ...
    if (trace_state_candidate_adds_word_target_table(section, cursor_candidate, &state, runtime_addresses,
        accepted_start[section_index], accepted_bytes[section_index], &dest_reg, &targets, profile) &&
...
typedef struct M68kAcceptedReachabilityNode {
  size_t section_index;
  uint32_t offset;
} M68kAcceptedReachabilityNode;

typedef struct M68kAcceptedReachabilityQueue {
  M68kAcceptedReachabilityNode *items;
  size_t count;
  size_t capacity;
  size_t cursor;
} M68kAcceptedReachabilityQueue;

static int code_start_reason_is_executable_seed(uint32_t reason) {
  switch (reason) {
    case M68K_FACT_CODE_START_REASON_SECTION_ENTRY:
    case M68K_FACT_CODE_START_REASON_POLICY_ENTRY_OFFSET:
    case M68K_FACT_CODE_START_REASON_POLICY_ENTRY_POINT:
...
static int accepted_reachability_enqueue(Arena *arena, M68kAcceptedReachabilityQueue *queue,
    uint8_t **reachable_start, uint8_t **accepted_start, const M68kDecodeIR *decode,
    size_t section_index, uint32_t offset) {
  if (arena == NULL || queue == NULL || reachable_start == NULL || accepted_start == NULL || decode == NULL ||
      section_index >= decode->section_count || offset >= decode->sections[section_index].size ||
      accepted_start[section_index] == NULL || reachable_start[section_index] == NULL ||
      accepted_start[section_index][offset] == 0U || reachable_start[section_index][offset] != 0U) {
    return 0;
  }
  if (queue->count == queue->capacity) {
    size_t next_capacity = queue->capacity != 0U ? queue->capacity * 2U : 64U;
    size_t old_size = queue->capacity * sizeof(*queue->items);
    size_t new_size = next_capacity * sizeof(*queue->items);
    M68kAcceptedReachabilityNode *grown =
      (M68kAcceptedReachabilityNode *)arena_realloc_copy(arena, queue->items, old_size, new_size);
    if (grown == NULL) return -1;
    queue->items = grown;
    queue->capacity = next_capacity;
  }
  reachable_start[section_index][offset] = 1U;
  queue->items[queue->count].section_index = section_index;
  queue->items[queue->count].offset = offset;
  ++queue->count;
  return 0;
...
static int prune_unreachable_accepted_code(M68kDecodeIR *decode, const M68kFactIR *facts,
    uint8_t **accepted_start, uint8_t **accepted_bytes, uint32_t *accepted_count, Arena *arena, uint8_t max_cpu) {
  ArenaMark mark;
  uint8_t **reachable_start;
  M68kAcceptedReachabilityQueue queue;
  size_t section_index;
  size_t fact_index;
  if (decode == NULL || facts == NULL || accepted_start == NULL || accepted_bytes == NULL || arena == NULL)
    return -1;
  mark = arena_mark(arena);
  memset(&queue, 0, sizeof(queue));
  reachable_start = (uint8_t **)arena_calloc(arena, decode->section_count != 0U ? decode->section_count : 1U,
    sizeof(*reachable_start));
  if (reachable_start == NULL) goto fail;
  for (section_index = 0U; section_index < decode->section_count; ++section_index) {
    uint32_t section_size = decode->sections[section_index].size;
    reachable_start[section_index] = (uint8_t *)arena_calloc(arena, section_size != 0U ? section_size : 1U, 1U);
    if (reachable_start[section_index] == NULL) goto fail;
  }
  for (fact_index = 0U; fact_index < facts->fact_count; ++fact_index) {
    const M68kFact *fact = &facts->facts[fact_index];
    if ((fact->kind != M68K_FACT_CODE_ACCEPTED && fact->kind != M68K_FACT_CODE_START) ||
        !code_start_reason_is_executable_seed(fact->reason)) {
      continue;
    }
    if (accepted_reachability_enqueue(arena, &queue, reachable_start, accepted_start, decode,
        fact->section_index, fact->offset) != 0) {
      goto fail;
    }
  }
  while (queue.cursor < queue.count) {
    M68kAcceptedReachabilityNode node = queue.items[queue.cursor++];
    const M68kDecodeSectionIR *section;
    const M68kDecodeCandidate *candidate = NULL;
    size_t target_index;
    if (node.section_index >= decode->section_count) continue;
    section = &decode->sections[node.section_index];
    if (node.offset >= section->size || accepted_start[node.section_index][node.offset] == 0U) continue;
    if (m68k_decode_ir_ensure_candidate_at(decode, node.section_index, node.offset, max_cpu, &candidate,
        m68k_diag_sink(NULL)) != 0) {
      goto fail;
    }
    if (candidate == NULL) continue;
    for (target_index = 0U; target_index < candidate->target_count; ++target_index) {
      const M68kDecodeTarget *target = &candidate->targets[target_index];
      if (!target->has_section || target->kind == M68K_DECODE_TARGET_DATA) continue;
      if (accepted_reachability_enqueue(arena, &queue, reachable_start, accepted_start, decode,
    ...
    if (candidate_has_normal_fallthrough(candidate) &&
      ...
      if (accepted_reachability_enqueue(arena, &queue, reachable_start, accepted_start, decode,
          node.section_index, fallthrough) != 0) {
        goto fail;
      }
    }
  }
  for (section_index = 0U; section_index < decode->section_count; ++section_index) {
    const M68kDecodeSectionIR *section = &decode->sections[section_index];
    uint32_t offset;
    if (accepted_start[section_index] == NULL || accepted_bytes[section_index] == NULL) continue;
    for (offset = 0U; offset < section->size; ++offset) {
      const M68kDecodeCandidate *candidate = NULL;
      if (accepted_start[section_index][offset] == 0U || reachable_start[section_index][offset] != 0U) continue;
      if (m68k_decode_ir_ensure_candidate_at(decode, section_index, offset, max_cpu, &candidate,
          m68k_diag_sink(NULL)) != 0) {
        goto fail;
      }
      if (candidate != NULL) {
        clear_accepted_candidate(accepted_start[section_index], accepted_bytes[section_index], candidate,
          accepted_count);
      } else {
        accepted_start[section_index][offset] = 0U;
        accepted_bytes[section_index][offset] = 0U;
        if (offset + 1U < section->size) accepted_bytes[section_index][offset + 1U] = 0U;
        if (accepted_count != NULL && *accepted_count != 0U) --*accepted_count;
      }
    }
  }
  ...
  for (section_index = 0U; section_index < decode->section_count; ++section_index) {
    ...
    for (candidate_index = 0U; candidate_index < section->candidate_count; ++candidate_index) {
      ...
      for (target_index = 0U; target_index < candidate->target_count; ++target_index) {
        ...
        if (label_lookup_create_label(label_lookup, section_index, anchor->offset,
            M68K_FACT_CONFIDENCE_TOOL_INFERRED) != 0) {
          return -1;
        }
      }
...
static int replay_runtime_sink_provenance_fallthrough(const M68kObject *object, M68kDecodeIR *decode,
  ...
  for (step = 0U; step < replay_limit && cursor < section->size; ++step) {
    ...
    if (trace_state_after_candidate(section_index, section, candidate, &state, relocation_lookup, facts,
        ...
        queue != NULL ? queue->arena : NULL, profile) != 0)
      return -1;
...
static int target_prefix_uses_trace_runtime_sink(M68kDecodeIR *decode, const M68kFactsV2RelocationLookup *relocation_lookup,
    ...
    const M68kFactsV2TraceState *trace_state, Arena *arena, uint8_t max_cpu) {
  const uint32_t scan_limit = 8U;
  const M68kDecodeSectionIR *section;
  M68kFactsV2TraceState state;
  uint32_t cursor = target_offset;
...
static int run_reachable_fixed_point(const M68kObject *object, M68kDecodeIR *decode, M68kFactIR *facts,
  ...
  while (work_queue_pop(queue, &item)) {
    ...
    if (trace_state_after_candidate(item.section_index, section, candidate, &item.trace_state, relocation_lookup,
        facts, runtime_addresses, accepted_start[item.section_index], accepted_bytes[item.section_index],
        &next_trace_state, queue->arena, profile) != 0)
      return -1;
    ...
    for (target_index = 0U; target_index < candidate->target_count; ++target_index) {
      ...
      }
      {
        ...
        if (target_prefix_uses_trace_runtime_sink(decode, relocation_lookup, runtime_addresses, facts,
            ...
            target_trace_state, queue->arena, max_cpu)) {
          allow_trace_variant = 1U;
        }
...
static int facts_v2_has_hard_failures(const M68kFactsV2Profile *profile) {
  return profile != NULL && (profile->unresolved_labels != 0U ||
    ...
    profile->table_target_set_limit_hits != 0U ||
...
static int facts_v2_collect_profile_internal(const M68kObject *object, const M68kAnalysisPolicy *policy,
  ...
  M68kSourceQualityHardwareBaseSeed *source_quality_hardware_base_seeds = NULL;
  size_t source_quality_hardware_base_seed_count = 0U;
  ...
  start = clock();
  fail_stage = "accepted code reachability prune";
  if (prune_unreachable_accepted_code(&decode, &facts, accepted_start, accepted_bytes,
      &out_profile->accepted_instructions, workflow.arena, max_cpu) != 0) {
    goto fail;
  }
  end = clock();
  add_elapsed_seconds_local(&out_profile->fixed_point_rebuild_accepted_seconds, start, end);
  ...
  if (m68k_render_lookup_build(&render_lookup, object, &decode, &facts, policy, accepted_start,
      label_lookup.points, label_lookup.point_count) != 0) goto fail;
  ...
  if (source_analysis != NULL && append_platform_storage_layouts_from_object(object, source_analysis) != 0) {
    m68k_diag_add(diagnostics, M68K_DIAG_SEVERITY_ERROR, M68K_DIAG_CODE_RENDER_FAILED,
      "facts_v2 platform storage layout append failed");
    goto fail;
  }
  if (source_analysis != NULL &&
      facts_v2_append_incomplete_analysis_from_profile(out_profile, source_analysis) != 0) {
    m68k_diag_add(diagnostics, M68K_DIAG_SEVERITY_ERROR, M68K_DIAG_CODE_RENDER_FAILED,
      "facts_v2 incomplete analysis append failed");
    goto fail;
  }
  if (source_analysis != NULL) {
    if (append_recovered_indirect_sites_for_accepted(&decode, accepted_start, accepted_bytes, source_analysis,
        max_cpu) != 0) {
      m68k_diag_add(diagnostics, M68K_DIAG_SEVERITY_ERROR, M68K_DIAG_CODE_RENDER_FAILED,
    ...
    if (append_address_observations_for_accepted(&decode, &facts, &relocation_lookup, object->platform_backend_kind,
        &runtime_addresses, accepted_start, accepted_bytes, source_analysis) != 0) {
      m68k_diag_add(diagnostics, M68K_DIAG_SEVERITY_ERROR, M68K_DIAG_CODE_RENDER_FAILED,
        "facts_v2 address observation append failed");
      goto fail;
    }
    if (append_platform_media_transfers_from_facts(&decode, &facts, source_analysis) != 0) {
      m68k_diag_add(diagnostics, M68K_DIAG_SEVERITY_ERROR, M68K_DIAG_CODE_RENDER_FAILED,
        "facts_v2 platform media transfer append failed");
      goto fail;
    }
    if (render_lookup.inferred_hardware_base_seed_count != 0U) {
      size_t seed_index;
      source_quality_hardware_base_seeds = (M68kSourceQualityHardwareBaseSeed *)calloc(
        render_lookup.inferred_hardware_base_seed_count, sizeof(*source_quality_hardware_base_seeds));
      if (source_quality_hardware_base_seeds == NULL) {
        m68k_diag_add(diagnostics, M68K_DIAG_SEVERITY_ERROR, M68K_DIAG_CODE_OUT_OF_MEMORY, "out of memory");
        goto fail;
      }
      source_quality_hardware_base_seed_count = render_lookup.inferred_hardware_base_seed_count;
      for (seed_index = 0U; seed_index < source_quality_hardware_base_seed_count; ++seed_index) {
        const M68kRenderInferredHardwareBaseSeed *source_seed =
          &render_lookup.inferred_hardware_base_seeds[seed_index];
        M68kSourceQualityHardwareBaseSeed *dest_seed = &source_quality_hardware_base_seeds[seed_index];
        dest_seed->section_index = source_seed->section_index;
        dest_seed->offset = source_seed->offset;
        dest_seed->reg_index = source_seed->reg_index;
        dest_seed->conflicted = source_seed->conflicted;
        dest_seed->hardware_base_id = source_seed->hardware_base_id;
      }
    }
    if (m68k_source_quality_analyze_with_policy_and_hardware_base_seeds(source_analysis, &decode, &facts,
        accepted_start, accepted_bytes, policy, source_quality_hardware_base_seeds,
        source_quality_hardware_base_seed_count) != 0) {
      m68k_diag_add(diagnostics, M68K_DIAG_SEVERITY_ERROR, M68K_DIAG_CODE_RENDER_FAILED,
        "facts_v2 source quality analysis failed");
      goto fail;
    }
    free(source_quality_hardware_base_seeds);
    source_quality_hardware_base_seeds = NULL;
    source_quality_hardware_base_seed_count = 0U;
