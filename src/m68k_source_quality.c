...
static int source_quality_relocation_fact_at_offset(const M68kFactIR *facts, size_t section_index,
    uint32_t offset);
static int source_quality_section_has_label_at(const M68kSectionAnalysisIR *section, uint32_t offset);
static int source_quality_text_alpha_byte(uint8_t value);
...
static int source_quality_structured_data_item_is_untyped_bytes_placeholder(
    const M68kAnalysisStructuredDataItem *item);
...
static int source_quality_existing_structured_data_conflicts_with_api_text_buffer(
    const M68kSourceAnalysisIR *source_analysis, uint32_t section_index, uint32_t offset) {
  size_t index;
  if (source_analysis == NULL) return 0;
  for (index = 0U; index < source_analysis->structured_data_item_count; ++index) {
...
static int source_quality_has_pointer_table_at_same_start(const M68kSourceAnalysisIR *source_analysis,
    const M68kAnalysisStructuredDataItem *item) {
  size_t index;
  ...
  for (index = 0U; index < source_analysis->structured_data_item_count; ++index) {
    const M68kAnalysisStructuredDataItem *existing = &source_analysis->structured_data_items[index];
    if (existing->has_section_index && existing->section_index == item->section_index &&
        ...
        (existing->semantic_role_flags & M68K_ANALYSIS_STRUCTURED_DATA_ROLE_POINTER_TABLE) != 0U) {
      return 1;
    }
  }
  for (policy_index = 0U; policy_index < source_analysis->policy.structured_data_item_count; ++policy_index) {
    const M68kAnalysisStructuredDataItem *existing = &source_analysis->policy.structured_data_items[policy_index];
    if (existing->has_section_index && existing->section_index == item->section_index &&
        existing->offset == item->offset && existing->kind == item->kind &&
        (existing->semantic_role_flags & M68K_ANALYSIS_STRUCTURED_DATA_ROLE_POINTER_TABLE) != 0U) {
      return 1;
...
static int append_pc_relative_lookup_table_item(M68kSourceAnalysisIR *source_analysis,
  ...
  m68k_analysis_structured_data_item_set_semantic_role_flags(&item,
    source_pattern_id == M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_INDEXED_LOCAL_POINTER_READ ||
  ...
  if (source_quality_has_pointer_table_at_same_start(source_analysis, &item)) return 0;
...
typedef struct M68kSourceQualityDataScalarValue {
  uint8_t known;
  uint32_t value;
} M68kSourceQualityDataScalarValue;
...
typedef struct M68kSourceQualityAudioPointerState {
  ...
  M68kSourceQualityDataScalarValue data_scalars[8];
...
static void source_quality_audio_pointer_state_set_reg(M68kSourceQualityAudioPointerState *state,
  ...
  if (reg_kind == 1U) state->data_scalars[reg_index].known = 0U;
}
...
static void source_quality_audio_pointer_state_copy_reg(M68kSourceQualityAudioPointerState *state,
  ...
  if (reg_kind == 1U) {
    state->data_regs[reg_index] = *source;
    state->data_scalars[reg_index].known = 0U;
  }
  else if (reg_kind == 2U) state->addr_regs[reg_index] = *source;
}

static void source_quality_audio_pointer_state_set_data_scalar(M68kSourceQualityAudioPointerState *state,
...
static void source_quality_audio_pointer_state_update_after_instruction(
  ...
  M68kSourceQualityDataScalarValue source_scalar;
  uint32_t target_section_index = 0U;
  uint32_t target_offset = 0U;
  uint32_t absolute_offset = 0U;
  uint32_t scalar_value = 0U;
  uint8_t dest_reg = 0U;
  uint8_t dest_reg_kind = 0U;
  ...
  if (source_quality_instruction_loads_immediate_to_register(instruction, &dest_reg_kind, &dest_reg, &scalar_value) &&
      dest_reg_kind == 1U) {
    source_quality_audio_pointer_state_set_data_scalar(state, dest_reg, scalar_value);
    return;
  }
  ...
  if ((instruction->mnemonic_id == M68K_ASM_MNEMONIC_MOVE ||
    ...
    if (source_quality_operand_data_register_index(&instruction->operands[1], &dest_reg)) {
      if (source_quality_operand_data_register_index(&instruction->operands[0], &source_reg)) {
...
static int source_quality_candidate_indexed_read_from_known_local_base(const M68kDecodeCandidate *candidate,
    const M68kInstructionIR *instruction, const M68kSourceQualityAudioPointerState *state,
    uint32_t *out_section_index, uint32_t *out_offset, uint8_t *out_index_register_kind,
    uint8_t *out_index_register) {
  const M68kSimFormMetadata *metadata;
  size_t operand_index;
  ...
  if (candidate == NULL || instruction == NULL || state == NULL || out_section_index == NULL ||
      out_offset == NULL) {
    return 0;
  }
  metadata = m68k_sim_metadata_for_instruction(instruction);
  ...
  for (operand_index = 0U; operand_index < candidate->operand_count && operand_index < instruction->operand_count &&
    ...
    if (metadata->operand_access_kinds[operand_index] != M68K_SIM_ACCESS_MEMORY_READ ||
        !((metadata->operand_ea_uses_index[operand_index] != 0U) ||
        ...
        operand->kind != M68K_ASM_OPERAND_EA || operand->value.ea_mode != 6U ||
        operand->value.ea_reg >= 8U ||
...
static int source_quality_candidate_dest_direct_register_kind(const M68kDecodeCandidate *candidate,
    const M68kInstructionIR *instruction, uint8_t *out_register_kind, uint8_t *out_register) {
  const M68kSimFormMetadata *metadata;
  size_t dest_index = (size_t)-1;
  uint8_t reg = 0U;
  if (out_register_kind != NULL) *out_register_kind = M68K_ANALYSIS_REGISTER_NONE;
  if (out_register != NULL) *out_register = 0U;
  if (candidate == NULL || instruction == NULL || out_register_kind == NULL || out_register == NULL) return 0;
  metadata = m68k_sim_metadata_for_instruction(instruction);
  if (metadata == NULL) return 0;
  if (metadata->dest_operand_index < candidate->operand_count) dest_index = metadata->dest_operand_index;
  if (dest_index >= instruction->operand_count) return 0;
  if (source_quality_operand_data_register_index(&instruction->operands[dest_index], &reg) && reg < 8U) {
    *out_register_kind = M68K_ANALYSIS_REGISTER_DATA;
    *out_register = reg;
    return 1;
  }
  if (source_quality_operand_address_register_index(&instruction->operands[dest_index], &reg) && reg < 8U) {
    *out_register_kind = M68K_ANALYSIS_REGISTER_ADDRESS;
    *out_register = reg;
    return 1;
  }
  return 0;
...
static int source_quality_candidate_indexed_read_dest_address_reg(const M68kDecodeCandidate *candidate,
    const M68kInstructionIR *instruction, uint8_t *out_dest_reg) {
  uint8_t reg_kind = M68K_ANALYSIS_REGISTER_NONE;
  uint8_t reg = 0U;
  if (out_dest_reg != NULL) *out_dest_reg = 0U;
  if (!source_quality_candidate_dest_direct_register_kind(candidate, instruction, &reg_kind, &reg) ||
      reg_kind != M68K_ANALYSIS_REGISTER_ADDRESS) {
    return 0;
  }
  if (out_dest_reg != NULL) *out_dest_reg = reg;
  return 1;
}

static int source_quality_runtime_pointer_value_to_accepted_source_offset(const M68kSectionAnalysisIR *section_analysis,
    const uint8_t *accepted_start, uint32_t section_size, uint32_t value, uint32_t *out_source_offset) {
  uint32_t source_offset = 0U;
  uint32_t logical_address = 0U;
  if (out_source_offset != NULL) *out_source_offset = 0U;
  if (section_analysis == NULL || accepted_start == NULL || value == 0U) return 0;
  if (value < section_size && accepted_start[value] != 0U) {
    if (out_source_offset != NULL) *out_source_offset = value;
    return 1;
  }
  if (!source_quality_any_runtime_address_source_offset(section_analysis, value, section_size, &source_offset))
    return 0;
  if (source_offset >= section_size || accepted_start[source_offset] == 0U) return 0;
  if (!source_quality_source_logical_address(section_analysis, source_offset, &logical_address) ||
...
static uint32_t source_quality_scan_indexed_local_pointer_table_span(
    const M68kSourceAnalysisIR *source_analysis, const M68kSectionAnalysisIR *table_analysis,
    const M68kDecodeSectionIR *table_section, const uint8_t *accepted_start, const uint8_t *accepted_bytes,
    uint32_t table_offset) {
  uint32_t cursor;
  uint32_t count = 0U;
  if (source_analysis == NULL || table_analysis == NULL || table_section == NULL || table_section->data == NULL ||
      accepted_start == NULL || accepted_bytes == NULL || table_offset >= table_section->size) {
    return 0U;
  }
  for (cursor = table_offset; cursor + 4U <= table_section->size; cursor += 4U) {
    ...
    if (source_quality_section_has_interior_label(table_analysis, cursor, 4U)) break;
    if (source_quality_accepted_range_has_code_byte(accepted_bytes, table_section->size, cursor, 4U)) break;
    if (source_quality_existing_structured_data_overlaps(source_analysis, (uint32_t)table_section->section_index,
        cursor, 4U)) {
      break;
    }
    if (source_quality_range_ownership_covers_noncode_or_conflict(table_analysis, cursor)) break;
    target = m68k_read_u32be(table_section->data + cursor);
    if ((target & 1U) != 0U ||
        !source_quality_runtime_pointer_value_to_accepted_source_offset(table_analysis, accepted_start,
          table_section->size, target, NULL)) {
      break;
    }
    ++count;
  }
...
static int append_indexed_local_pointer_table_items(M68kSourceAnalysisIR *source_analysis,
    const M68kDecodeIR *decode, uint8_t *const *accepted_start, uint8_t *const *accepted_bytes) {
  size_t decode_index;
  if (source_analysis == NULL || decode == NULL || accepted_start == NULL || accepted_bytes == NULL) return 0;
  for (decode_index = 0U; decode_index < decode->section_count; ++decode_index) {
    ...
    section_analysis = source_analysis_section_by_index(source_analysis, (uint32_t)section->section_index);
    ...
    for (candidate_index = 0U; candidate_index < section->candidate_count; ++candidate_index) {
      ...
      uint32_t item_size = 0U;
      uint8_t item_kind = 0U;
      uint8_t index_register_kind = M68K_ANALYSIS_REGISTER_NONE;
      uint8_t index_register = 0U;
      if (!source_quality_candidate_is_accepted_start(section, accepted_start[decode_index], candidate)) continue;
      if (m68k_decode_candidate_to_instruction(candidate, &instruction) != 0) continue;
      if (source_quality_structured_data_kind_for_candidate_size(candidate, &item_size, &item_kind) &&
          item_size == 4U && item_kind == M68K_ANALYSIS_STRUCTURED_DATA_LONGS &&
          source_quality_candidate_indexed_read_from_known_local_base(candidate, &instruction, &state,
            &target_section_index, &target_offset, &index_register_kind, &index_register) &&
          target_section_index < decode->section_count) {
        const M68kDecodeSectionIR *target_section = &decode->sections[target_section_index];
        M68kSectionAnalysisIR *target_analysis =
          source_analysis_section_by_index(source_analysis, target_section_index);
        uint32_t table_offset = target_offset;
        uint32_t span = 0U;
        uint8_t table_offset_is_code = 0U;
        uint8_t dest_reg = 0U;
        uint8_t target_register_kind = M68K_ANALYSIS_REGISTER_NONE;
        uint8_t target_register = 0U;
        uint8_t target_register_known = 0U;
        if (target_analysis != NULL && accepted_start[target_section_index] != NULL &&
            accepted_bytes[target_section_index] != NULL && target_section->data != NULL &&
            target_offset <= target_section->size && item_size <= target_section->size - target_offset) {
          table_offset_is_code = (uint8_t)source_quality_accepted_range_has_code_byte(
            accepted_bytes[target_section_index], target_section->size, table_offset, item_size);
          target_register_known = (uint8_t)source_quality_candidate_dest_direct_register_kind(candidate,
            &instruction, &target_register_kind, &target_register);
          if (!table_offset_is_code &&
              !source_quality_section_has_interior_label(target_analysis, table_offset, item_size)) {
            span = source_quality_scan_indexed_local_pointer_table_span(source_analysis, target_analysis,
              target_section, accepted_start[target_section_index], accepted_bytes[target_section_index],
              table_offset);
          }
          if (span == 0U && table_offset_is_code &&
              source_quality_candidate_indexed_read_dest_address_reg(candidate, &instruction, &dest_reg) &&
              source_quality_candidate_later_is_indirect_control_through_addr_reg(section,
                accepted_start[decode_index], candidate, dest_reg) &&
              table_offset <= UINT32_MAX - item_size &&
              table_offset + item_size <= target_section->size &&
              item_size <= target_section->size - (table_offset + item_size) &&
              !source_quality_accepted_range_has_code_byte(accepted_bytes[target_section_index],
                target_section->size, table_offset + item_size, item_size) &&
              !source_quality_section_has_interior_label(target_analysis, table_offset + item_size, item_size)) {
            table_offset += item_size;
            span = source_quality_scan_indexed_local_pointer_table_span(source_analysis, target_analysis,
              target_section, accepted_start[target_section_index], accepted_bytes[target_section_index],
              table_offset);
          }
          if (span != 0U &&
              append_pc_relative_lookup_table_item(source_analysis, target_section_index, table_offset, span,
                M68K_ANALYSIS_STRUCTURED_DATA_LONGS,
                M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_INDEXED_LOCAL_POINTER_READ,
                (uint32_t)section->section_index, candidate->offset, 1U, index_register_kind, index_register,
                target_register_known, target_register_kind, target_register, NULL, 0U, 0U, 0U) != 0) {
            return -1;
          }
        }
      }
...
typedef struct M68kSourceQualityWordOffsetValue {
  uint8_t known;
  uint32_t table_section_index;
  uint32_t table_offset;
} M68kSourceQualityWordOffsetValue;

typedef struct M68kSourceQualityWordOffsetState {
  M68kSourceQualityWordOffsetValue data_regs[8];
} M68kSourceQualityWordOffsetState;

static uint32_t source_quality_scan_indexed_word_relative_data_table_span(
    const M68kSourceAnalysisIR *source_analysis, const M68kSectionAnalysisIR *table_analysis,
    const M68kDecodeSectionIR *table_section, const uint8_t *accepted_bytes, uint32_t table_offset,
    const M68kDecodeSectionIR *target_section, uint32_t target_base_offset) {
  uint32_t cursor;
  if (source_analysis == NULL || table_analysis == NULL || table_section == NULL || target_section == NULL ||
  ...
  for (cursor = table_offset; cursor + 2U <= table_section->size; cursor += 2U) {
    ...
    uint32_t target_offset;
    if (cursor != table_offset && source_quality_section_has_label_at(table_analysis, cursor)) break;
    if (source_quality_section_has_interior_label(table_analysis, cursor, 2U)) break;
    if (source_quality_existing_structured_data_overlaps(source_analysis, (uint32_t)table_section->section_index,
        cursor, 2U)) {
      break;
    }
    if (source_quality_range_ownership_covers_noncode_or_conflict(table_analysis, cursor)) break;
    if (source_quality_accepted_range_has_code_byte(accepted_bytes, table_section->size, cursor, 2U)) break;
...
static uint32_t source_quality_count_word_relative_table_label_targets(
    const M68kSectionAnalysisIR *target_analysis, uint32_t target_base_offset,
    const M68kDecodeSectionIR *table_section, uint32_t table_offset, uint32_t span) {
  uint32_t cursor;
  uint32_t distinct_offsets[2];
  uint32_t count = 0U;
  if (target_analysis == NULL || table_section == NULL || table_section->data == NULL ||
      table_offset > table_section->size || span > table_section->size - table_offset) {
    return 0U;
  }
  for (cursor = 0U; cursor + 2U <= span; cursor += 2U) {
    ...
    int64_t target_offset = (int64_t)target_base_offset + displacement;
    uint32_t target_offset_u32;
    uint32_t index;
    int seen = 0;
    if (target_offset < 0 || target_offset > UINT32_MAX) continue;
    target_offset_u32 = (uint32_t)target_offset;
    if (!source_quality_section_has_label_at(target_analysis, target_offset_u32)) continue;
    for (index = 0U; index < count; ++index) {
      if (distinct_offsets[index] == target_offset_u32) {
        seen = 1;
        break;
      }
    }
    if (seen) continue;
    distinct_offsets[count] = target_offset_u32;
    ++count;
...
static uint32_t source_quality_count_word_relative_table_accepted_targets(
    const uint8_t *target_accepted_start, uint32_t target_section_size, uint32_t target_base_offset,
    const M68kDecodeSectionIR *table_section, uint32_t table_offset, uint32_t span) {
  uint32_t cursor;
  uint32_t distinct_offsets[2];
  uint32_t count = 0U;
  if (target_accepted_start == NULL || table_section == NULL || table_section->data == NULL ||
      table_offset > table_section->size || span > table_section->size - table_offset ||
      target_base_offset >= target_section_size) {
    return 0U;
  }
  for (cursor = 0U; cursor + 2U <= span; cursor += 2U) {
    int32_t displacement = (int32_t)(int16_t)m68k_read_u16be(table_section->data + table_offset + cursor);
    ...
    uint32_t index;
    int seen = 0;
    if (target_offset < 0 || target_offset >= target_section_size) continue;
    target_offset_u32 = (uint32_t)target_offset;
    if (!target_accepted_start[target_offset_u32]) continue;
    for (index = 0U; index < count; ++index) {
      if (distinct_offsets[index] == target_offset_u32) {
        seen = 1;
        break;
...
static uint32_t source_quality_word_relative_table_target_upper_bound(
    const M68kDecodeSectionIR *table_section, uint32_t table_offset, uint32_t span,
    const M68kDecodeSectionIR *target_section, uint32_t target_base_offset, uint32_t target_offset) {
  uint32_t cursor;
  uint32_t upper_bound;
  if (table_section == NULL || target_section == NULL || table_section->data == NULL ||
      table_offset > table_section->size || span > table_section->size - table_offset ||
      target_base_offset >= target_section->size || target_offset >= target_section->size) {
    return 0U;
  }
  upper_bound = target_section->size;
  if (table_section->section_index == target_section->section_index &&
      target_offset < table_offset && table_offset < upper_bound) {
    upper_bound = table_offset;
  }
  for (cursor = 0U; cursor + 2U <= span; cursor += 2U) {
    int32_t displacement = (int32_t)(int16_t)m68k_read_u16be(table_section->data + table_offset + cursor);
    int64_t target_offset_signed = (int64_t)target_base_offset + displacement;
    ...
    other_target = (uint32_t)target_offset_signed;
    if (other_target > target_offset && other_target < upper_bound && other_target < target_section->size) {
      upper_bound = other_target;
    }
  }
  return upper_bound > target_offset ? upper_bound : 0U;
}

static uint8_t source_quality_range_ownership_kind_at(const M68kSectionAnalysisIR *section, uint32_t offset,
    uint8_t *out_kind) {
  size_t index;
  if (out_kind != NULL) *out_kind = M68K_RANGE_OWNERSHIP_UNKNOWN;
  if (section == NULL) return 0U;
  for (index = 0U; index < section->range_ownership_count; ++index) {
    const M68kRangeOwnershipIR *range = &section->range_ownerships[index];
    if (offset < range->start_offset || offset >= range->end_offset) continue;
    if (out_kind != NULL) *out_kind = range->kind;
    return 1U;
  }
  return 0U;
}

static int source_quality_word_offset_string_terminator_byte(uint8_t value) {
  return value == 0U || value == 0xffU;
}

static int source_quality_word_offset_bounded_string_shape_ok(const M68kDecodeSectionIR *section,
    uint32_t offset, uint32_t size) {
  uint32_t cursor;
  ...
  if (source_quality_word_offset_string_terminator_byte(section->data[offset + size - 1U])) --text_size;
...
static uint32_t source_quality_word_offset_c_string_span(const M68kDecodeSectionIR *section,
    const uint8_t *accepted_bytes, const M68kSectionAnalysisIR *section_analysis, uint32_t offset,
    uint32_t upper_bound, uint32_t min_text_size) {
  uint32_t cursor;
  uint32_t text_size = 0U;
  uint32_t scan_bound;
  if (section == NULL || section->data == NULL || accepted_bytes == NULL || section_analysis == NULL ||
      offset >= section->size) {
    return 0U;
  }
  scan_bound = upper_bound > offset && upper_bound <= section->size ? upper_bound : section->size;
  cursor = offset;
  while (cursor < scan_bound && !source_quality_word_offset_string_terminator_byte(section->data[cursor])) {
    if (!m68k_ir_byte_is_quoted_string_safe(section->data[cursor])) return 0U;
    if (source_quality_accepted_range_has_code_byte(accepted_bytes, section->size, cursor, 1U)) return 0U;
    if (source_quality_range_ownership_kind_at(section_analysis, cursor, NULL)) return 0U;
    if (cursor != offset && source_quality_section_has_label_at(section_analysis, cursor)) return 0U;
    ++cursor;
    ++text_size;
  }
  if (cursor >= scan_bound || !source_quality_word_offset_string_terminator_byte(section->data[cursor]) ||
      text_size < min_text_size ||
      source_quality_accepted_range_has_code_byte(accepted_bytes, section->size, cursor, 1U) ||
      source_quality_range_ownership_kind_at(section_analysis, cursor, NULL)) {
    return 0U;
  }
  return text_size + 1U;
}

static uint32_t source_quality_pointer_table_target_c_string_span(const M68kDecodeSectionIR *section,
    const uint8_t *accepted_bytes, const M68kSectionAnalysisIR *section_analysis, uint32_t offset,
    uint32_t min_text_size) {
  uint32_t cursor;
  uint32_t text_size = 0U;
  if (section == NULL || section->data == NULL || section_analysis == NULL || offset >= section->size) {
    return 0U;
  }
  cursor = offset;
...
static uint32_t source_quality_pointer_string_pool_next_offset(const M68kDecodeSectionIR *section,
    uint32_t offset, uint32_t span) {
  uint32_t cursor;
  uint32_t padding = 0U;
  if (section == NULL || offset > UINT32_MAX - span) return UINT32_MAX;
  cursor = offset + span;
  while (cursor < section->size && section->data[cursor] == 0U && padding < 2U) {
    ++cursor;
    ++padding;
  }
  return cursor;
...
static int source_quality_existing_string_item_starts_at(const M68kSourceAnalysisIR *source_analysis,
    uint32_t section_index, uint32_t offset) {
  const M68kAnalysisStructuredDataItem *item =
    source_quality_structured_data_item_covering_offset(source_analysis, section_index, offset);
  return item != NULL && item->has_section_index && item->section_index == section_index &&
    item->offset == offset && item->kind == M68K_ANALYSIS_STRUCTURED_DATA_STRING &&
    (item->semantic_role_flags & M68K_ANALYSIS_STRUCTURED_DATA_ROLE_STRING) != 0U;
}

static int source_quality_pointer_string_pool_has_next(const M68kDecodeSectionIR *section,
    const uint8_t *accepted_bytes, const M68kSourceAnalysisIR *source_analysis,
    const M68kSectionAnalysisIR *section_analysis, uint32_t offset, uint32_t span) {
  uint32_t next = source_quality_pointer_string_pool_next_offset(section, offset, span);
  if (next == UINT32_MAX || next >= section->size || next == offset) return 0;
  if (source_quality_existing_string_item_starts_at(source_analysis, (uint32_t)section->section_index, next)) return 1;
  return source_quality_pointer_table_target_c_string_span(section, accepted_bytes, section_analysis, next, 2U) != 0U;
}

static int source_quality_pointer_string_pool_has_previous(const M68kDecodeSectionIR *section,
    const uint8_t *accepted_bytes, const M68kSourceAnalysisIR *source_analysis,
  ...
  uint32_t padding = 0U;
  uint32_t span;
  if (section == NULL || section->data == NULL || offset == 0U || offset > section->size) return 0;
  previous_end = offset;
  while (previous_end > 1U && section->data[previous_end - 1U] == 0U &&
      section->data[previous_end - 2U] == 0U && padding < 2U) {
    --previous_end;
    ++padding;
  }
  if (previous_end == 0U || section->data[previous_end - 1U] != 0U) return 0;
  start = previous_end - 1U;
  while (start > 0U && !source_quality_word_offset_string_terminator_byte(section->data[start - 1U])) {
    --start;
  }
  if (start == previous_end - 1U) return 0;
  if (source_quality_existing_string_item_starts_at(source_analysis, (uint32_t)section->section_index, start)) {
    return 1;
  }
  span = source_quality_pointer_table_target_c_string_span(section, accepted_bytes, section_analysis, start, 2U);
  return span != 0U && start + span == previous_end;
}

static int source_quality_pointer_table_target_has_dense_string_pool_context(const M68kDecodeSectionIR *section,
    const uint8_t *accepted_bytes, const M68kSourceAnalysisIR *source_analysis,
    const M68kSectionAnalysisIR *section_analysis, uint32_t offset, uint32_t span) {
  return source_quality_pointer_string_pool_has_next(section, accepted_bytes, source_analysis, section_analysis, offset,
      span) ||
    source_quality_pointer_string_pool_has_previous(section, accepted_bytes, source_analysis, section_analysis, offset);
}

static uint32_t source_quality_word_relative_table_bounded_string_span(
    const M68kSourceAnalysisIR *source_analysis, const M68kSectionAnalysisIR *section_analysis,
    const M68kDecodeSectionIR *section, const uint8_t *accepted_bytes, uint32_t offset, uint32_t upper_bound) {
  uint32_t cursor;
  uint32_t span;
  if (source_analysis == NULL || section_analysis == NULL || section == NULL || section->data == NULL ||
      accepted_bytes == NULL || offset >= section->size || upper_bound <= offset || upper_bound > section->size) {
    return 0U;
  }
  ...
  for (cursor = offset; cursor < upper_bound; ++cursor) {
    if (!m68k_ir_byte_is_quoted_string_safe(section->data[cursor])) return 0U;
    if (source_quality_accepted_range_has_code_byte(accepted_bytes, section->size, cursor, 1U)) return 0U;
    if (source_quality_existing_structured_data_overlaps(source_analysis, (uint32_t)section->section_index,
        cursor, 1U)) {
      return 0U;
    }
    if (source_quality_range_ownership_kind_at(section_analysis, cursor, NULL)) return 0U;
  ...
  if (!source_quality_word_offset_bounded_string_shape_ok(section, offset, span)) return 0U;
  return span;
}

static uint32_t source_quality_word_relative_table_bounded_raw_target_span(
    const M68kSourceAnalysisIR *source_analysis, const M68kSectionAnalysisIR *section_analysis,
    const M68kDecodeSectionIR *section, const uint8_t *accepted_bytes, uint32_t offset, uint32_t upper_bound) {
  uint32_t cursor;
  uint32_t span;
  ...
  if (source_quality_accepted_range_has_code_byte(accepted_bytes, section->size, offset, span)) return 0U;
  if (source_quality_section_has_interior_label(section_analysis, offset, span)) return 0U;
  for (cursor = offset; cursor < upper_bound; ++cursor) {
    if (source_quality_existing_structured_data_overlaps(source_analysis, (uint32_t)section->section_index,
        cursor, 1U)) {
      return 0U;
...
static int append_word_offset_raw_target_item(M68kSourceAnalysisIR *source_analysis,
    uint32_t section_index, uint32_t offset, uint32_t size, uint32_t consumer_section,
    uint32_t consumer_offset) {
  M68kAnalysisStructuredDataItem item;
  const char *source_pattern;
  if (source_analysis == NULL || size == 0U ||
      source_quality_has_structured_data_item(source_analysis, section_index, offset, size,
        M68K_ANALYSIS_STRUCTURED_DATA_BYTES,
        M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_WORD_OFFSET_STRING_TABLE)) {
    return 0;
  }
  memset(&item, 0, sizeof(item));
  item.has_section_index = 1U;
...
static int append_word_offset_string_target_item(M68kSourceAnalysisIR *source_analysis,
    uint32_t section_index, uint32_t offset, uint32_t size, uint32_t consumer_section,
    uint32_t consumer_offset) {
  M68kAnalysisStructuredDataItem item;
  const char *source_pattern;
...
static int append_pointer_table_target_string_item(M68kSourceAnalysisIR *source_analysis,
    uint32_t section_index, uint32_t offset, uint32_t size, uint32_t consumer_section,
    uint32_t consumer_offset) {
  M68kAnalysisStructuredDataItem item;
  const char *source_pattern;
...
static int source_quality_pointer_table_value_to_source_offset(const M68kSectionAnalysisIR *section_analysis,
    uint32_t section_size, uint32_t value, uint32_t *out_source_offset) {
  uint32_t source_offset = 0U;
  uint32_t logical_address = 0U;
  if (out_source_offset != NULL) *out_source_offset = 0U;
  if (section_analysis == NULL || value == 0U) return 0;
  ...
  if (!source_quality_any_runtime_address_source_offset(section_analysis, value, section_size, &source_offset))
    return 0;
  if (source_offset >= section_size ||
      !source_quality_source_logical_address(section_analysis, source_offset, &logical_address) ||
      logical_address != value) {
    return 0;
  }
  if (out_source_offset != NULL) *out_source_offset = source_offset;
...
static int source_quality_pointer_table_value_to_pointer_string_target(
    const M68kSourceAnalysisIR *source_analysis, const M68kSectionAnalysisIR *section_analysis,
    uint32_t section_size, uint32_t value, uint32_t *out_source_offset) {
  uint32_t source_offset = 0U;
  const M68kAnalysisStructuredDataItem *target_item;
  if (!source_quality_pointer_table_value_to_source_offset(section_analysis, section_size, value,
      &source_offset)) {
    return 0;
  }
  target_item = source_quality_structured_data_item_covering_offset(source_analysis,
    (uint32_t)section_analysis->section_index, source_offset);
  if (target_item == NULL || target_item->kind != M68K_ANALYSIS_STRUCTURED_DATA_STRING ||
      target_item->source_pattern_id != M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_POINTER_STRING_TABLE) {
    return 0;
  }
  if (out_source_offset != NULL) *out_source_offset = source_offset;
  return 1;
...
static int append_pointer_table_target_strings_for_item(M68kSourceAnalysisIR *source_analysis,
    const M68kDecodeIR *decode, uint8_t *const *accepted_bytes, const M68kAnalysisStructuredDataItem *item) {
  const M68kDecodeSectionIR *section;
  M68kSectionAnalysisIR *section_analysis;
  size_t decode_index = 0U;
  uint32_t cursor;
  uint32_t available;
  uint32_t section_index;
  uint32_t dense_string_count = 0U;
  int explicit_pointer_string_table;
  int allow_single_dense_target;
  if (source_analysis == NULL || decode == NULL || accepted_bytes == NULL || item == NULL ||
      !item->has_section_index || item->kind != M68K_ANALYSIS_STRUCTURED_DATA_LONGS ||
  ...
  explicit_pointer_string_table =
    item->source_pattern_id == M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_POINTER_STRING_TABLE;
  allow_single_dense_target =
    item->source_pattern_id == M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_RELOCATION_POINTER_TABLE;
  section_index = item->section_index;
  section = source_quality_decode_section_by_index(decode, section_index, &decode_index);
  section_analysis = source_analysis_section_by_index(source_analysis, section_index);
  if (section_analysis == NULL || section == NULL || section->data == NULL || item->offset >= section->size) {
    return 0;
  }
  available = section->size - item->offset;
  if (available > item->size) available = item->size;
  if (!explicit_pointer_string_table && !allow_single_dense_target) {
    for (cursor = 0U; cursor + 4U <= available; cursor += 4U) {
      uint32_t value = m68k_read_u32be(section->data + item->offset + cursor);
      uint32_t target_offset = 0U;
      uint32_t span;
      if (!source_quality_pointer_table_value_to_source_offset(section_analysis, section->size, value,
          &target_offset)) {
        continue;
      }
      span = source_quality_pointer_table_target_c_string_span(section, accepted_bytes[decode_index],
        section_analysis, target_offset, 2U);
      if (span == 0U) continue;
      if (source_quality_pointer_table_target_has_dense_string_pool_context(section, accepted_bytes[decode_index],
          source_analysis, section_analysis, target_offset, span)) {
        ++dense_string_count;
      }
    }
    if (dense_string_count < 2U) return 0;
  }
  for (cursor = 0U; cursor + 4U <= available; cursor += 4U) {
    uint32_t value = m68k_read_u32be(section->data + item->offset + cursor);
    uint32_t target_offset = 0U;
    ...
    span = source_quality_pointer_table_target_c_string_span(section, accepted_bytes[decode_index],
      section_analysis, target_offset, 2U);
    if (span == 0U) continue;
    if (!explicit_pointer_string_table &&
        !source_quality_pointer_table_target_has_dense_string_pool_context(section, accepted_bytes[decode_index],
          source_analysis, section_analysis, target_offset, span)) {
      continue;
    }
    if (append_pointer_table_target_string_item(source_analysis, section_index, target_offset, span,
        section_index, item->offset) != 0) {
      return -1;
    }
  }
  return 0;
...
static int append_pointer_table_target_strings(M68kSourceAnalysisIR *source_analysis,
    const M68kDecodeIR *decode, uint8_t *const *accepted_bytes, const M68kAnalysisPolicy *policy) {
  size_t index;
  uint16_t policy_index;
  if (source_analysis == NULL || decode == NULL || accepted_bytes == NULL) return 0;
  for (index = 0U; index < source_analysis->structured_data_item_count; ++index) {
    if (append_pointer_table_target_strings_for_item(source_analysis, decode, accepted_bytes,
  ...
  for (policy_index = 0U; policy_index < source_analysis->policy.structured_data_item_count; ++policy_index) {
    if (append_pointer_table_target_strings_for_item(source_analysis, decode, accepted_bytes,
        &source_analysis->policy.structured_data_items[policy_index]) != 0) {
      return -1;
    }
  }
  if (policy != NULL) {
    for (policy_index = 0U; policy_index < policy->structured_data_item_count; ++policy_index) {
      if (append_pointer_table_target_strings_for_item(source_analysis, decode, accepted_bytes,
          &policy->structured_data_items[policy_index]) != 0) {
        return -1;
      }
    }
  }
...
static int source_quality_auto_string_has_nearby_terminal_code_context(const M68kDecodeSectionIR *section,
    uint32_t offset) {
  uint32_t start;
  uint32_t cursor;
...
static int source_quality_auto_string_boundary_at(const M68kSourceAnalysisIR *source_analysis,
    const M68kSectionAnalysisIR *section_analysis, uint32_t section_index, uint32_t offset) {
  return source_quality_section_has_label_at(section_analysis, offset) ||
    source_quality_policy_structured_data_item_covers_offset(source_analysis, section_index, offset) ||
    source_quality_structured_data_item_covering_offset(source_analysis, section_index, offset) != NULL;
}
...
static int source_quality_auto_string_candidate_start(const M68kDecodeSectionIR *section, uint32_t offset) {
  uint8_t value;
  if (section == NULL || section->data == NULL || offset >= section->size) return 0;
  value = section->data[offset];
  if (m68k_ir_byte_is_quoted_string_safe(value) && source_quality_auto_string_start_byte(value)) return 1;
  return value >= 4U && value <= 80U && offset + 1U < section->size &&
    source_quality_auto_string_start_byte(section->data[offset + 1U]);
}

static int source_quality_auto_string_interior_clear(const M68kSourceAnalysisIR *source_analysis,
    const M68kSectionAnalysisIR *section_analysis, const M68kFactIR *facts, uint32_t section_index,
    uint32_t offset) {
  if (source_analysis == NULL || section_analysis == NULL) return 0;
  return !source_quality_section_has_label_at(section_analysis, offset) &&
    !source_quality_relocation_fact_at_offset(facts, section_index, offset) &&
    !source_quality_structured_data_item_covers_offset(source_analysis, section_index, offset) &&
    !source_quality_range_ownership_kind_at(section_analysis, offset, NULL);
}

static int source_quality_auto_string_start_blocked(const M68kSourceAnalysisIR *source_analysis,
    uint32_t section_index, uint32_t offset) {
  const M68kAnalysisStructuredDataItem *item =
    source_quality_structured_data_item_covering_offset(source_analysis, section_index, offset);
  return item != NULL &&
    !(item->offset == offset && source_quality_structured_data_item_is_untyped_bytes_placeholder(item));
}

static int source_quality_auto_string_looks_length_prefixed(const M68kSectionAnalysisIR *section_analysis,
    const M68kDecodeSectionIR *section, uint32_t offset, uint32_t text_size) {
  if (section_analysis == NULL || section == NULL || section->data == NULL || offset == 0U || text_size > 255U)
    return 0;
  if (source_quality_section_has_label_at(section_analysis, offset)) return 0;
  return section->data[offset - 1U] == (uint8_t)text_size;
}

static int source_quality_auto_string_looks_length_prefixed_sequence(
    const M68kSectionAnalysisIR *section_analysis, const M68kDecodeSectionIR *section, uint32_t offset,
    uint32_t text_end) {
  uint32_t cursor;
  uint32_t record_count = 0U;
  if (section_analysis == NULL || section == NULL || section->data == NULL || offset == 0U ||
  ...
  while (cursor < text_end) {
    ...
    if (!source_quality_auto_string_start_byte(section->data[payload])) return 0;
...
enum {
  SOURCE_QUALITY_AUTO_STRING_RECORD_CONTEXT_NONE = 0,
  SOURCE_QUALITY_AUTO_STRING_RECORD_CONTEXT_ADJACENT_TABLE = 1,
  SOURCE_QUALITY_AUTO_STRING_RECORD_CONTEXT_CONTROL_STREAM = 2
};

static void source_quality_auto_string_record_context_visit_item(const M68kAnalysisStructuredDataItem *item,
    uint32_t offset, uint32_t *prior_count, uint32_t *nearest_end) {
  uint32_t item_end;
  ...
  item_end = item->offset + item->size;
  if (item_end > offset || offset - item_end > 192U) return;
  ++*prior_count;
  if (item_end > *nearest_end) *nearest_end = item_end;
}

static int source_quality_auto_string_record_sequence_context_kind(const M68kSourceAnalysisIR *source_analysis,
    const M68kDecodeSectionIR *section, uint32_t offset) {
  size_t index;
  uint16_t policy_index;
  uint32_t prior_count = 0U;
  uint32_t nearest_end = 0U;
  if (source_analysis == NULL || section == NULL || section->data == NULL || offset == 0U) return 0;
  for (index = 0U; index < source_analysis->structured_data_item_count; ++index) {
    const M68kAnalysisStructuredDataItem *item = &source_analysis->structured_data_items[index];
    if (!item->has_section_index || item->section_index != section->section_index) continue;
    source_quality_auto_string_record_context_visit_item(item, offset, &prior_count, &nearest_end);
  }
  for (policy_index = 0U; policy_index < source_analysis->policy.structured_data_item_count; ++policy_index) {
    const M68kAnalysisStructuredDataItem *item = &source_analysis->policy.structured_data_items[policy_index];
    if (!item->has_section_index || item->section_index != section->section_index) continue;
  ...
  if (nearest_end == offset) return SOURCE_QUALITY_AUTO_STRING_RECORD_CONTEXT_ADJACENT_TABLE;
  for (index = nearest_end; index < offset; ++index) {
    if (!m68k_ir_byte_is_quoted_string_safe(section->data[index]) ||
        source_quality_word_offset_string_terminator_byte(section->data[index])) {
      return SOURCE_QUALITY_AUTO_STRING_RECORD_CONTEXT_CONTROL_STREAM;
    }
  }
  return SOURCE_QUALITY_AUTO_STRING_RECORD_CONTEXT_NONE;
}

static int source_quality_auto_string_has_record_sequence_context(const M68kSourceAnalysisIR *source_analysis,
    const M68kDecodeSectionIR *section, uint32_t offset) {
  return source_quality_auto_string_record_sequence_context_kind(source_analysis, section, offset) !=
    SOURCE_QUALITY_AUTO_STRING_RECORD_CONTEXT_NONE;
}

static uint32_t source_quality_auto_renderable_string_span_with_options(
    const M68kSourceAnalysisIR *source_analysis, const M68kDecodeSectionIR *section,
    const M68kSectionAnalysisIR *section_analysis, const M68kFactIR *facts, const uint8_t *accepted_bytes,
    uint32_t offset, uint32_t min_text_size, int require_space) {
  uint32_t cursor;
  uint32_t text_size = 0U;
  uint32_t section_index;
  int has_space = 0;
  ...
  section_index = (uint32_t)section->section_index;
  has_record_sequence_context = source_quality_auto_string_has_record_sequence_context(source_analysis, section,
    offset);
  if (!source_quality_auto_string_boundary_at(source_analysis, section_analysis, section_index, offset) &&
      source_quality_auto_string_has_nearby_terminal_code_context(section, offset)) {
    return 0U;
  }
  if (offset > 0U && m68k_ir_byte_is_quoted_string_safe(section->data[offset - 1U]) &&
      !source_quality_section_has_label_at(section_analysis, offset) && !has_record_sequence_context) {
    return 0U;
  }
  cursor = offset;
  while (cursor < section->size && m68k_ir_byte_is_quoted_string_safe(section->data[cursor])) {
    if (cursor != offset &&
        !source_quality_auto_string_interior_clear(source_analysis, section_analysis, facts, section_index, cursor)) {
      return 0U;
    }
  ...
  if (cursor >= section->size || !source_quality_word_offset_string_terminator_byte(section->data[cursor]) ||
    ...
    return 0U;
  if (source_quality_auto_string_looks_length_prefixed(section_analysis, section, offset, text_size)) return 0U;
  if (source_quality_auto_string_looks_length_prefixed_sequence(section_analysis, section, offset, cursor))
    return 0U;
  if (section->data[cursor] == 0xffU && has_lowercase) return 0U;
  if (!source_quality_auto_string_interior_clear(source_analysis, section_analysis, facts, section_index, cursor)) {
    return 0U;
  }
  if (source_quality_auto_string_range_has_code_byte(section, accepted_bytes, offset, text_size + 1U)) return 0U;
  return text_size + 1U;
}

static uint32_t source_quality_auto_renderable_string_span(const M68kSourceAnalysisIR *source_analysis,
    const M68kDecodeSectionIR *section, const M68kSectionAnalysisIR *section_analysis, const M68kFactIR *facts,
    const uint8_t *accepted_bytes, uint32_t offset) {
  return source_quality_auto_renderable_string_span_with_options(source_analysis, section, section_analysis, facts,
    accepted_bytes, offset, 8U, 1);
}

static uint32_t source_quality_auto_renderable_line_terminated_string_span(
    const M68kSourceAnalysisIR *source_analysis, const M68kDecodeSectionIR *section,
    const M68kSectionAnalysisIR *section_analysis, const M68kFactIR *facts, const uint8_t *accepted_bytes,
    uint32_t offset, uint32_t min_text_size, int require_space, int record_context_kind) {
  uint32_t cursor;
  uint32_t text_size = 0U;
  ...
  section_index = (uint32_t)section->section_index;
  has_boundary_evidence = record_context_kind == SOURCE_QUALITY_AUTO_STRING_RECORD_CONTEXT_ADJACENT_TABLE ||
    source_quality_auto_string_boundary_at(source_analysis, section_analysis, section_index, offset);
  ...
  if (break_count == 0U || cursor >= section->size ||
      !source_quality_word_offset_string_terminator_byte(section->data[cursor]) || text_size < min_text_size ||
      (require_space && !has_space && record_context_kind == SOURCE_QUALITY_AUTO_STRING_RECORD_CONTEXT_NONE)) {
    return 0U;
  }
  if (source_quality_auto_string_looks_length_prefixed(section_analysis, section, offset, text_size)) return 0U;
  if (source_quality_auto_string_looks_length_prefixed_sequence(section_analysis, section, offset, cursor))
  ...
  if (!source_quality_auto_string_interior_clear(source_analysis, section_analysis, facts, section_index, cursor))
    return 0U;
  if (source_quality_auto_string_range_has_code_byte(section, accepted_bytes, offset, cursor - offset + 1U))
    return 0U;
  return cursor - offset + 1U;
}

static uint32_t source_quality_auto_renderable_bounded_string_span(const M68kSourceAnalysisIR *source_analysis,
    const M68kDecodeSectionIR *section, const M68kSectionAnalysisIR *section_analysis, const M68kFactIR *facts,
    const uint8_t *accepted_bytes, uint32_t offset) {
  uint32_t cursor;
  uint32_t span;
  ...
  section_index = (uint32_t)section->section_index;
  if (!source_quality_auto_string_boundary_at(source_analysis, section_analysis, section_index, offset)) return 0U;
  cursor = offset;
  while (cursor < section->size && m68k_ir_byte_is_quoted_string_safe(section->data[cursor]) &&
      !source_quality_auto_string_range_has_code_byte(section, accepted_bytes, cursor, 1U) &&
      !source_quality_relocation_fact_at_offset(facts, section_index, cursor) &&
      (cursor == offset || !source_quality_section_has_label_at(section_analysis, cursor))) {
    if (source_quality_structured_data_item_covers_offset(source_analysis, section_index, cursor)) break;
    if (source_quality_range_ownership_kind_at(section_analysis, cursor, NULL)) break;
    ++cursor;
  }
  span = cursor - offset;
  if (cursor < section->size && source_quality_word_offset_string_terminator_byte(section->data[cursor]) &&
...
static uint8_t source_quality_source_pattern_for_string_record_context(int context_kind,
    uint8_t fallback_pattern_id) {
  if (context_kind == SOURCE_QUALITY_AUTO_STRING_RECORD_CONTEXT_CONTROL_STREAM)
    return M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_CONTROL_STRING_STREAM;
  if (context_kind == SOURCE_QUALITY_AUTO_STRING_RECORD_CONTEXT_ADJACENT_TABLE)
    return M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_STRING_TABLE_SEQUENCE;
  return fallback_pattern_id;
}

static uint32_t source_quality_role_flags_for_string_record_context(int context_kind) {
...
static int append_source_quality_generic_string_item(M68kSourceAnalysisIR *source_analysis,
    uint32_t section_index, uint32_t offset, uint32_t size, uint32_t role_flags, uint8_t source_pattern_id) {
  M68kAnalysisStructuredDataItem item;
  const char *source_pattern;
  if (source_analysis == NULL || size == 0U ||
      source_quality_has_structured_data_item(source_analysis, section_index, offset, size,
...
static int append_generic_string_items_for_section(M68kSourceAnalysisIR *source_analysis,
    const M68kDecodeSectionIR *section, M68kSectionAnalysisIR *section_analysis, const M68kFactIR *facts,
    const uint8_t *accepted_bytes) {
  uint32_t offset = 0U;
  if (source_analysis == NULL || section == NULL || section_analysis == NULL || section->data == NULL) return 0;
  while (offset < section->size) {
    uint32_t span;
    uint8_t source_pattern_id = M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_TERMINATED_TEXT;
    int record_context_kind;
    int candidate_start = source_quality_auto_string_candidate_start(section, offset) ||
      (m68k_ir_byte_is_quoted_string_safe(section->data[offset]) &&
       source_quality_section_has_label_at(section_analysis, offset));
    if (!candidate_start ||
        source_quality_auto_string_range_has_code_byte(section, accepted_bytes, offset, 1U) ||
        source_quality_auto_string_start_blocked(source_analysis, (uint32_t)section->section_index, offset)) {
      ++offset;
      continue;
    }
    record_context_kind = source_quality_auto_string_record_sequence_context_kind(source_analysis, section, offset);
    span = source_quality_auto_renderable_string_span(source_analysis, section, section_analysis, facts,
      accepted_bytes, offset);
    if (span == 0U) {
      span = source_quality_auto_renderable_line_terminated_string_span(source_analysis, section, section_analysis,
        facts, accepted_bytes, offset, 8U, 1, record_context_kind);
    }
    if (span == 0U) {
      span = source_quality_auto_renderable_bounded_string_span(source_analysis, section, section_analysis, facts,
        accepted_bytes, offset);
      if (span != 0U) source_pattern_id = M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_BOUNDED_TEXT;
    }
    if (span != 0U) {
      if (append_source_quality_generic_string_item(source_analysis, (uint32_t)section->section_index, offset, span,
          source_quality_role_flags_for_string_record_context(record_context_kind),
          source_quality_source_pattern_for_string_record_context(record_context_kind, source_pattern_id)) != 0) {
        return -1;
      }
      offset += span;
      continue;
...
static int append_generic_string_items(M68kSourceAnalysisIR *source_analysis,
    const M68kDecodeIR *decode, const M68kFactIR *facts, uint8_t *const *accepted_bytes) {
  size_t section_index;
  if (source_analysis == NULL || decode == NULL) return 0;
  for (section_index = 0U; section_index < source_analysis->section_count; ++section_index) {
    ...
    const M68kDecodeSectionIR *section = source_quality_decode_section_by_index(decode,
      (uint32_t)section_analysis->section_index, &decode_index);
    if (section == NULL) continue;
    if (append_generic_string_items_for_section(source_analysis, section, section_analysis, facts,
        accepted_bytes != NULL ? accepted_bytes[decode_index] : NULL) != 0) {
      return -1;
    }
  }
...
static int source_quality_auto_multiline_text_byte(uint8_t value) {
  return m68k_ir_byte_is_quoted_string_safe(value) || value == 0x0dU || value == 0x0aU || value == 0x09U;
}

static int source_quality_auto_multiline_text_start(const M68kDecodeSectionIR *section, uint32_t offset) {
  uint32_t cursor;
  if (section == NULL || section->data == NULL || offset >= section->size) return 0;
  ...
  return cursor < section->size && source_quality_auto_string_start_byte(section->data[cursor]);
}

static int source_quality_auto_multiline_text_shape_ok(uint32_t text_size, uint32_t alpha_count,
...
static int source_quality_structured_data_item_is_weak_generic_text(
    const M68kAnalysisStructuredDataItem *item) {
  if (item == NULL || item->kind != M68K_ANALYSIS_STRUCTURED_DATA_STRING ||
      (item->semantic_role_flags & M68K_ANALYSIS_STRUCTURED_DATA_ROLE_STRING) == 0U) {
    return 0;
  }
  return item->source_pattern_id == M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_TERMINATED_TEXT ||
    item->source_pattern_id == M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_BOUNDED_TEXT ||
    item->source_pattern_id == M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_CONTROL_STRING_STREAM;
}

static int source_quality_auto_multiline_text_boundary_at(const M68kSourceAnalysisIR *source_analysis,
    const M68kSectionAnalysisIR *section_analysis, uint32_t section_index, uint32_t offset) {
  if (source_analysis == NULL || section_analysis == NULL) return 0;
  return source_quality_section_has_label_at(section_analysis, offset) ||
    source_quality_policy_structured_data_item_covers_offset(source_analysis, section_index, offset) ||
...
static int source_quality_auto_multiline_text_interior_clear(const M68kSourceAnalysisIR *source_analysis,
    const M68kSectionAnalysisIR *section_analysis, const M68kFactIR *facts, uint32_t section_index,
    uint32_t span_start, uint32_t offset) {
  const M68kAnalysisStructuredDataItem *item;
  int weak_generic_text;
  int span_start_placeholder;
  if (source_analysis == NULL || section_analysis == NULL) return 0;
  item = source_quality_structured_data_item_covering_offset(source_analysis, section_index, offset);
  weak_generic_text = source_quality_structured_data_item_is_weak_generic_text(item);
  span_start_placeholder = item != NULL && item->offset == span_start &&
    source_quality_structured_data_item_is_untyped_bytes_placeholder(item);
  if (item != NULL &&
      !span_start_placeholder &&
      !weak_generic_text) {
    return 0;
  }
  item = source_quality_compatible_string_item_starting_at(source_analysis, section_index, offset);
  if (item != NULL && !source_quality_structured_data_item_is_weak_generic_text(item)) return 0;
  if (!weak_generic_text && !span_start_placeholder &&
      source_quality_policy_structured_data_item_covers_offset(source_analysis, section_index, offset)) {
    return 0;
  }
  if (source_quality_section_has_label_at(section_analysis, offset)) return 0;
...
static uint32_t source_quality_auto_multiline_string_span(const M68kSourceAnalysisIR *source_analysis,
    const M68kDecodeSectionIR *section, const M68kSectionAnalysisIR *section_analysis, const M68kFactIR *facts,
    const uint8_t *accepted_bytes, uint32_t offset) {
  uint32_t cursor;
  uint32_t text_size = 0U;
  ...
  uint32_t section_index;
  if (source_analysis == NULL || section == NULL || section_analysis == NULL || section->data == NULL ||
      section->section_index > UINT32_MAX || offset >= section->size ||
      !source_quality_auto_multiline_text_start(section, offset)) {
    return 0U;
  }
  section_index = (uint32_t)section->section_index;
  if (section->kind == M68K_SECTION_CODE && !source_quality_section_has_label_at(section_analysis, offset)) {
    return 0U;
  }
  cursor = offset;
  while (cursor < section->size && source_quality_auto_multiline_text_byte(section->data[cursor])) {
    uint8_t value = section->data[cursor];
    if (cursor != offset && !source_quality_auto_multiline_text_interior_clear(source_analysis,
        section_analysis, facts, section_index, offset, cursor)) {
      if (source_quality_auto_multiline_text_boundary_at(source_analysis, section_analysis, section_index,
          cursor) &&
          source_quality_auto_multiline_text_shape_ok(text_size, alpha_count, line_break_count, text_line_count,
            has_space) &&
          !source_quality_auto_string_range_has_code_byte(section, accepted_bytes, offset, cursor - offset)) {
        return cursor - offset;
      }
      return 0U;
    }
    if (value == 0x0dU || value == 0x0aU) {
      ...
      if (value == ' ') has_space = 1;
      if (source_quality_text_alpha_byte(value)) ++alpha_count;
    }
    ++cursor;
  }
  ...
  if (!source_quality_auto_multiline_text_interior_clear(source_analysis, section_analysis, facts, section_index,
      offset, cursor)) {
    return 0U;
  }
  if (source_quality_auto_string_range_has_code_byte(section, accepted_bytes, offset, cursor - offset + 1U))
...
static void source_quality_remove_contained_weak_generic_text_items(M68kSourceAnalysisIR *source_analysis,
    uint32_t section_index, uint32_t offset, uint32_t size) {
  size_t index = 0U;
  uint32_t end = offset + size;
  if (source_analysis == NULL || size == 0U || offset > UINT32_MAX - size) return;
  while (index < source_analysis->structured_data_item_count) {
    M68kAnalysisStructuredDataItem *item = &source_analysis->structured_data_items[index];
    uint32_t item_end;
    if (!item->has_section_index || item->section_index != section_index ||
        !source_quality_structured_data_item_is_weak_generic_text(item) ||
        item->size == 0U || item->offset > UINT32_MAX - item->size) {
      ++index;
      continue;
    }
    item_end = item->offset + item->size;
    if (item->offset >= offset && item_end <= end) {
      size_t tail_count = source_analysis->structured_data_item_count - index - 1U;
      if (tail_count != 0U) {
        memmove(&source_analysis->structured_data_items[index], &source_analysis->structured_data_items[index + 1U],
          tail_count * sizeof(source_analysis->structured_data_items[index]));
      }
      --source_analysis->structured_data_item_count;
      continue;
    }
    ++index;
  }
  {
    uint16_t policy_index = 0U;
    while (policy_index < source_analysis->policy.structured_data_item_count) {
      M68kAnalysisStructuredDataItem *item = &source_analysis->policy.structured_data_items[policy_index];
      uint32_t item_end;
      if (!item->has_section_index || item->section_index != section_index ||
          !source_quality_structured_data_item_is_weak_generic_text(item) ||
          item->size == 0U || item->offset > UINT32_MAX - item->size) {
      ...
      item_end = item->offset + item->size;
      if (item->offset >= offset && item_end <= end) {
        uint16_t tail_count = (uint16_t)(source_analysis->policy.structured_data_item_count - policy_index - 1U);
        if (tail_count != 0U) {
          memmove(&source_analysis->policy.structured_data_items[policy_index],
            &source_analysis->policy.structured_data_items[policy_index + 1U],
            (size_t)tail_count * sizeof(source_analysis->policy.structured_data_items[policy_index]));
        }
...
static int append_source_quality_multiline_string_item(M68kSourceAnalysisIR *source_analysis,
    uint32_t section_index, uint32_t offset, uint32_t size) {
  M68kAnalysisStructuredDataItem item;
  const char *source_pattern;
  if (source_analysis == NULL || size == 0U ||
  ...
  m68k_analysis_structured_data_item_set_semantic_role_flags(&item,
    M68K_ANALYSIS_STRUCTURED_DATA_ROLE_STRING);
  source_quality_remove_contained_weak_generic_text_items(source_analysis, section_index, offset, size);
  if (source_quality_update_same_offset_structured_data_item(source_analysis, &item)) return 0;
  return m68k_ir_source_analysis_append_structured_data_item(source_analysis, &item);
}

static int append_multiline_string_items_for_section(M68kSourceAnalysisIR *source_analysis,
    const M68kDecodeSectionIR *section, M68kSectionAnalysisIR *section_analysis, const M68kFactIR *facts,
    const uint8_t *accepted_bytes) {
  uint32_t offset = 0U;
  if (source_analysis == NULL || section == NULL || section_analysis == NULL || section->data == NULL)
  ...
  while (offset < section->size) {
    ...
    const M68kAnalysisStructuredDataItem *covering_item =
      source_quality_structured_data_item_covering_offset(source_analysis, (uint32_t)section->section_index,
        offset);
    if (covering_item != NULL &&
        !source_quality_structured_data_item_is_weak_generic_text(covering_item) &&
        !(covering_item->offset == offset &&
          source_quality_structured_data_item_is_untyped_bytes_placeholder(covering_item))) {
      ++offset;
      continue;
    }
    span = source_quality_auto_multiline_string_span(source_analysis, section, section_analysis, facts,
      accepted_bytes, offset);
    if (span != 0U) {
      if (append_source_quality_multiline_string_item(source_analysis, (uint32_t)section->section_index,
          offset, span) != 0) {
        return -1;
      }
      offset += span;
...
static int append_multiline_string_items(M68kSourceAnalysisIR *source_analysis,
    const M68kDecodeIR *decode, const M68kFactIR *facts, uint8_t *const *accepted_bytes) {
  size_t section_index;
  if (source_analysis == NULL || decode == NULL) return 0;
  for (section_index = 0U; section_index < source_analysis->section_count; ++section_index) {
    ...
    const M68kDecodeSectionIR *section = source_quality_decode_section_by_index(decode,
      (uint32_t)section_analysis->section_index, &decode_index);
    if (section == NULL) continue;
    if (append_multiline_string_items_for_section(source_analysis, section, section_analysis, facts,
        accepted_bytes != NULL ? accepted_bytes[decode_index] : NULL) != 0) {
      return -1;
    }
  }
...
static uint32_t append_word_relative_table_string_targets(M68kSourceAnalysisIR *source_analysis,
    ...
    uint32_t consumer_section_index, uint32_t consumer_offset) {
  const M68kDecodeSectionIR *table_section;
  const M68kDecodeSectionIR *target_section;
  M68kSectionAnalysisIR *target_analysis;
  uint32_t cursor;
  uint32_t string_count = 0U;
  if (source_analysis == NULL || decode == NULL || accepted_bytes == NULL ||
      table_section_index >= decode->section_count || target_section_index >= decode->section_count) {
    return 0U;
  }
  ...
  if (table_section->data == NULL || target_section->data == NULL || target_analysis == NULL ||
      table_offset > table_section->size || span > table_section->size - table_offset ||
      target_base_offset >= target_section->size || accepted_bytes[target_section_index] == NULL) {
    return 0U;
  }
  for (cursor = 0U; cursor + 2U <= span; cursor += 2U) {
    int32_t displacement = (int32_t)(int16_t)m68k_read_u16be(table_section->data + table_offset + cursor);
    ...
    if (source_quality_range_ownership_kind_at(target_analysis, target_offset, &owner_kind)) {
      if (owner_kind == M68K_RANGE_OWNERSHIP_TEXT) ++string_count;
      continue;
    }
    upper_bound = source_quality_word_relative_table_target_upper_bound(table_section, table_offset, span,
      target_section, target_base_offset, target_offset);
    string_span = source_quality_word_offset_c_string_span(target_section, accepted_bytes[target_section_index],
      target_analysis, target_offset, upper_bound, 2U);
    if (string_span == 0U) {
      string_span = source_quality_word_relative_table_bounded_string_span(source_analysis, target_analysis,
        target_section, accepted_bytes[target_section_index], target_offset, upper_bound);
    }
    if (string_span == 0U &&
        source_quality_word_relative_table_bounded_raw_target_span(source_analysis, target_analysis,
          target_section, accepted_bytes[target_section_index], target_offset, upper_bound) != 0U) {
      continue;
    }
    if (string_span == 0U) continue;
    ++string_count;
  }
  if (string_count < 2U) return string_count;
  string_count = 0U;
  for (cursor = 0U; cursor + 2U <= span; cursor += 2U) {
    int32_t displacement = (int32_t)(int16_t)m68k_read_u16be(table_section->data + table_offset + cursor);
    int64_t target_offset_signed = (int64_t)target_base_offset + displacement;
    ...
    string_span = source_quality_word_offset_c_string_span(target_section, accepted_bytes[target_section_index],
      target_analysis, target_offset,
      source_quality_word_relative_table_target_upper_bound(table_section, table_offset, span, target_section,
        target_base_offset, target_offset), 2U);
    if (string_span == 0U) {
      uint32_t upper_bound = source_quality_word_relative_table_target_upper_bound(table_section, table_offset,
        span, target_section, target_base_offset, target_offset);
      string_span = source_quality_word_relative_table_bounded_string_span(source_analysis, target_analysis,
        target_section, accepted_bytes[target_section_index], target_offset, upper_bound);
      if (string_span == 0U) {
        uint32_t raw_span = source_quality_word_relative_table_bounded_raw_target_span(source_analysis,
          target_analysis, target_section, accepted_bytes[target_section_index], target_offset, upper_bound);
        if (raw_span != 0U &&
            append_word_offset_raw_target_item(source_analysis, target_section_index, target_offset, raw_span,
              table_section_index, table_offset) != 0) {
          return 0U;
        }
        continue;
      }
    }
    if (append_word_offset_string_target_item(source_analysis, target_section_index, target_offset, string_span,
        table_section_index, table_offset) != 0) {
      return 0U;
    }
    ++string_count;
  }
  if (append_pc_relative_lookup_table_item(source_analysis, table_section_index, table_offset, span,
      M68K_ANALYSIS_STRUCTURED_DATA_WORDS,
      M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_WORD_OFFSET_STRING_TABLE, consumer_section_index,
      consumer_offset, 0U, M68K_ANALYSIS_REGISTER_NONE, 0U, 0U, M68K_ANALYSIS_REGISTER_NONE, 0U, NULL,
      1U, target_section_index, target_base_offset) != 0) {
    return 0U;
  }
  return string_count;
}

static int source_quality_candidate_indexed_word_read_to_data_reg(const M68kDecodeCandidate *candidate,
    const M68kInstructionIR *instruction, const M68kSourceQualityAudioPointerState *state,
  ...
  if (out_dest_reg != NULL) *out_dest_reg = 0U;
  if (candidate == NULL || instruction == NULL || state == NULL ||
      out_table_section_index == NULL || out_table_offset == NULL || out_dest_reg == NULL ||
      !source_quality_structured_data_kind_for_candidate_size(candidate, &item_size, &item_kind) ||
      item_size != 2U || item_kind != M68K_ANALYSIS_STRUCTURED_DATA_WORDS ||
      !source_quality_candidate_indexed_read_from_known_local_base(candidate, instruction, state,
        out_table_section_index, out_table_offset, NULL, NULL) ||
      !source_quality_candidate_dest_data_register(instruction, &dest_reg) || dest_reg >= 8U) {
    return 0;
  }
...
static int source_quality_candidate_adds_word_offset_to_known_base(const M68kDecodeCandidate *candidate,
    const M68kInstructionIR *instruction, const M68kSourceQualityAudioPointerState *data_state,
    const M68kSourceQualityWordOffsetState *word_state, uint32_t *out_table_section_index,
  ...
  if (candidate == NULL || instruction == NULL || data_state == NULL || word_state == NULL ||
      out_table_section_index == NULL || out_table_offset == NULL || out_base_section_index == NULL ||
      out_base_offset == NULL) {
    return 0;
  }
  metadata = m68k_sim_metadata_for_instruction(instruction);
  ...
  if (instruction->mnemonic_id == M68K_ASM_MNEMONIC_ADD && instruction->size_suffix == 'l' &&
      source_quality_operand_address_register_index(&instruction->operands[source_index], &addr_reg) &&
      source_quality_operand_data_register_index(&instruction->operands[dest_index], &data_reg) &&
      addr_reg < 8U && data_reg < 8U && data_state->addr_regs[addr_reg].known &&
      word_state->data_regs[data_reg].known) {
    *out_table_section_index = word_state->data_regs[data_reg].table_section_index;
    *out_table_offset = word_state->data_regs[data_reg].table_offset;
    *out_base_section_index = data_state->addr_regs[addr_reg].target_section_index;
    *out_base_offset = data_state->addr_regs[addr_reg].target_offset;
    return 1;
  }
  if (instruction->mnemonic_id == M68K_ASM_MNEMONIC_ADDA && instruction->size_suffix == 'w' &&
      source_quality_operand_data_register_index(&instruction->operands[source_index], &data_reg) &&
      source_quality_operand_address_register_index(&instruction->operands[dest_index], &addr_reg) &&
      addr_reg < 8U && data_reg < 8U && data_state->addr_regs[addr_reg].known &&
      word_state->data_regs[data_reg].known) {
    *out_table_section_index = word_state->data_regs[data_reg].table_section_index;
    *out_table_offset = word_state->data_regs[data_reg].table_offset;
...
static int source_quality_candidate_adds_indexed_word_offset_to_known_base(
    const M68kDecodeCandidate *candidate, const M68kInstructionIR *instruction,
  ...
  if (candidate == NULL || instruction == NULL || state == NULL ||
      ...
      item_size != 2U || item_kind != M68K_ANALYSIS_STRUCTURED_DATA_WORDS ||
      !source_quality_candidate_indexed_read_from_known_local_base(candidate, instruction, state,
        out_table_section_index, out_table_offset, out_index_register_kind, out_index_register)) {
    return 0;
  }
  metadata = m68k_sim_metadata_for_instruction(instruction);
  if (metadata == NULL || metadata->operation_type != M68K_SIM_OP_ADD ||
...
static void source_quality_word_offset_state_clear_written_data_reg(M68kSourceQualityWordOffsetState *state,
    const M68kInstructionIR *instruction) {
  uint8_t dest_reg = 0U;
  if (state == NULL || instruction == NULL) return;
  if (source_quality_candidate_dest_data_register(instruction, &dest_reg) && dest_reg < 8U)
    state->data_regs[dest_reg].known = 0U;
}

static int append_indexed_local_scalar_table_items(M68kSourceAnalysisIR *source_analysis,
    const M68kDecodeIR *decode, uint8_t *const *accepted_start, uint8_t *const *accepted_bytes) {
  size_t decode_index;
  if (source_analysis == NULL || decode == NULL || accepted_start == NULL || accepted_bytes == NULL) return 0;
  for (decode_index = 0U; decode_index < decode->section_count; ++decode_index) {
    ...
    M68kSectionAnalysisIR *section_analysis;
    M68kSourceQualityAudioPointerState state;
    M68kSourceQualityWordOffsetState word_state;
    size_t candidate_index;
    if (section->section_index > UINT32_MAX || accepted_start[decode_index] == NULL ||
        accepted_bytes[decode_index] == NULL) {
      return -1;
    ...
    if (section_analysis == NULL) continue;
    source_quality_audio_pointer_state_clear_all(&state);
    memset(&word_state, 0, sizeof(word_state));
    for (candidate_index = 0U; candidate_index < section->candidate_count; ++candidate_index) {
      const M68kDecodeCandidate *candidate = &section->candidates[candidate_index];
      M68kInstructionIR instruction;
      uint32_t target_section_index = 0U;
      uint32_t target_offset = 0U;
      uint32_t base_section_index = 0U;
      ...
      uint8_t index_register_kind = M68K_ANALYSIS_REGISTER_NONE;
      uint8_t index_register = 0U;
      int word_offset_match;
      int indexed_word_offset_match = 0;
      if (!source_quality_candidate_is_accepted_start(section, accepted_start[decode_index], candidate)) continue;
      if (m68k_decode_candidate_to_instruction(candidate, &instruction) != 0) continue;
      word_offset_match = source_quality_candidate_adds_word_offset_to_known_base(candidate, &instruction, &state,
        &word_state, &target_section_index, &target_offset, &base_section_index, &base_offset);
      if (!word_offset_match) {
        indexed_word_offset_match = source_quality_candidate_adds_indexed_word_offset_to_known_base(candidate,
          &instruction, &state, &target_section_index, &target_offset, &base_section_index, &base_offset,
          &index_register_kind, &index_register);
      }
      if ((word_offset_match || indexed_word_offset_match) &&
          target_section_index < decode->section_count && base_section_index < decode->section_count &&
          accepted_bytes[target_section_index] != NULL) {
        const M68kDecodeSectionIR *target_section = &decode->sections[target_section_index];
        const M68kDecodeSectionIR *base_section = &decode->sections[base_section_index];
        M68kSectionAnalysisIR *target_analysis = source_analysis_section_by_index(source_analysis,
          target_section_index);
        M68kSectionAnalysisIR *base_analysis = source_analysis_section_by_index(source_analysis,
          base_section_index);
        uint32_t span = 0U;
        uint8_t has_target = 0U;
        if (target_analysis != NULL && base_analysis != NULL && target_offset <= target_section->size) {
          span = source_quality_scan_indexed_word_relative_data_table_span(source_analysis, target_analysis,
            target_section, accepted_bytes[target_section_index], target_offset, base_section, base_offset);
          if (span != 0U) {
            has_target = source_quality_count_word_relative_table_label_targets(base_analysis, base_offset,
              target_section, target_offset, span) >= 2U ? 1U : 0U;
          }
          if (span != 0U &&
              append_pc_relative_lookup_table_item(source_analysis, target_section_index, target_offset, span,
                M68K_ANALYSIS_STRUCTURED_DATA_WORDS,
                M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_INDEXED_LOCAL_SCALAR_READ,
                (uint32_t)section->section_index, candidate->offset,
                index_register_kind != M68K_ANALYSIS_REGISTER_NONE, index_register_kind, index_register,
                0U, M68K_ANALYSIS_REGISTER_NONE, 0U, NULL, has_target, base_section_index, base_offset) != 0) {
            return -1;
          }
          if (span != 0U &&
              append_word_relative_table_string_targets(source_analysis, decode, accepted_bytes,
                target_section_index, target_offset, span, base_section_index, base_offset,
                (uint32_t)section->section_index, candidate->offset) >= 2U) {
            source_quality_word_offset_state_clear_written_data_reg(&word_state, &instruction);
          }
        }
      }
      if (instruction.mnemonic_id == M68K_ASM_MNEMONIC_ADDA &&
        ...
        if (target_section_index < decode->section_count && accepted_bytes[target_section_index] != NULL) {
          ...
          uint32_t table_target_section = 0U;
          uint32_t table_target_offset = 0U;
          if (target_analysis != NULL && target_section->data != NULL &&
              target_offset <= target_section->size && item_size <= target_section->size - target_offset &&
              !source_quality_accepted_range_has_code_byte(accepted_bytes[target_section_index],
                target_section->size, target_offset, item_size) &&
              !source_quality_section_has_interior_label(target_analysis, target_offset, item_size) &&
              !source_quality_existing_structured_data_overlaps(source_analysis, target_section_index,
                target_offset, item_size) &&
              !source_quality_range_ownership_covers_noncode_or_conflict(target_analysis, target_offset)) {
            span = source_quality_pc_relative_lookup_table_span(source_analysis, target_analysis, target_section,
              accepted_bytes[target_section_index], target_offset, item_size);
          }
          if (span != 0U && instruction.operand_count >= 2U &&
              source_quality_operand_address_register_index(
                &instruction.operands[instruction.operand_count - 1U], &target_base_reg) &&
              target_base_reg < 8U && state.addr_regs[target_base_reg].known) {
            uint32_t target_base_section_index = state.addr_regs[target_base_reg].target_section_index;
            M68kSectionAnalysisIR *base_analysis = source_analysis_section_by_index(source_analysis,
              target_base_section_index);
            if (base_analysis != NULL && target_base_section_index < decode->section_count) {
              const M68kDecodeSectionIR *base_section = &decode->sections[target_base_section_index];
              int later_indirect_control = source_quality_candidate_later_is_indirect_control_through_addr_reg(
                section, accepted_start[decode_index], candidate, target_base_reg);
              uint32_t label_target_count = source_quality_count_word_relative_table_label_targets(base_analysis,
                state.addr_regs[target_base_reg].target_offset, target_section, target_offset, span);
              uint32_t accepted_target_count = accepted_start[target_base_section_index] != NULL &&
                  !later_indirect_control ?
                source_quality_count_word_relative_table_accepted_targets(accepted_start[target_base_section_index],
                  base_section->size, state.addr_regs[target_base_reg].target_offset, target_section, target_offset,
                  span) : 0U;
              has_target = label_target_count >= 2U || accepted_target_count >= 2U ? 1U : 0U;
              table_target_section = target_base_section_index;
              table_target_offset = state.addr_regs[target_base_reg].target_offset;
            }
          }
          if (span != 0U &&
              append_pc_relative_lookup_table_item(source_analysis, target_section_index, target_offset, span,
                item_kind, M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_INDEXED_LOCAL_SCALAR_READ,
                (uint32_t)section->section_index, candidate->offset,
                index_register_kind != M68K_ANALYSIS_REGISTER_NONE, index_register_kind, index_register,
                0U, M68K_ANALYSIS_REGISTER_NONE, 0U, NULL, has_target, table_target_section,
                table_target_offset) != 0) {
            return -1;
          }
        }
      }
      if (source_quality_candidate_indexed_word_read_to_data_reg(candidate, &instruction, &state,
          &target_section_index, &target_offset, &dest_data_reg)) {
        word_state.data_regs[dest_data_reg].known = 1U;
        word_state.data_regs[dest_data_reg].table_section_index = target_section_index;
        word_state.data_regs[dest_data_reg].table_offset = target_offset;
      } else {
        source_quality_word_offset_state_clear_written_data_reg(&word_state, &instruction);
      }
      source_quality_dispatch_pointer_state_update_after_instruction(&state, section_analysis, section, candidate,
...
static int source_quality_candidate_reads_postincrement_from_reg(const M68kDecodeCandidate *candidate,
    const M68kInstructionIR *instruction, uint8_t reg, uint32_t item_size, uint8_t item_kind) {
  const M68kSimFormMetadata *metadata;
  uint32_t candidate_item_size = 0U;
  uint8_t candidate_item_kind = 0U;
  size_t operand_index;
  if (candidate == NULL || instruction == NULL ||
      !source_quality_structured_data_kind_for_candidate_size(candidate, &candidate_item_size,
        &candidate_item_kind) ||
      candidate_item_size != item_size || candidate_item_kind != item_kind) {
    return 0;
  }
  metadata = m68k_sim_metadata_for_instruction(instruction);
  if (metadata == NULL) return 0;
  for (operand_index = 0U; operand_index < candidate->operand_count && operand_index < instruction->operand_count &&
    ...
    uint8_t source_reg = 0U;
    if (metadata->operand_access_kinds[operand_index] == M68K_SIM_ACCESS_MEMORY_READ &&
        source_quality_operand_is_address_postincrement(&instruction->operands[operand_index], &source_reg) &&
        source_reg == reg) {
      return 1;
    }
  }
...
static uint32_t source_quality_count_consecutive_postincrement_reads(const M68kDecodeSectionIR *section,
    const uint8_t *accepted_start, size_t first_candidate_index, uint8_t reg, uint32_t item_size,
    uint8_t item_kind) {
  size_t candidate_index;
  uint32_t count = 0U;
  if (section == NULL || accepted_start == NULL) return 0U;
  for (candidate_index = first_candidate_index; candidate_index < section->candidate_count; ++candidate_index) {
    const M68kDecodeCandidate *candidate = &section->candidates[candidate_index];
    M68kInstructionIR instruction;
    if (!source_quality_candidate_is_accepted_start(section, accepted_start, candidate) ||
        m68k_decode_candidate_to_instruction(candidate, &instruction) != 0 ||
        !source_quality_candidate_reads_postincrement_from_reg(candidate, &instruction, reg, item_size, item_kind)) {
      break;
    }
    ++count;
  }
...
static int append_indexed_postincrement_data_table_items(M68kSourceAnalysisIR *source_analysis,
    const M68kDecodeIR *decode, uint8_t *const *accepted_start, uint8_t *const *accepted_bytes) {
  size_t decode_index;
  if (source_analysis == NULL || decode == NULL || accepted_start == NULL || accepted_bytes == NULL) return 0;
  for (decode_index = 0U; decode_index < decode->section_count; ++decode_index) {
    ...
    for (candidate_index = 0U; candidate_index < section->candidate_count; ++candidate_index) {
      ...
      if (source_quality_structured_data_kind_for_candidate_size(candidate, &item_size, &item_kind) &&
          item_size > 1U) {
        for (source_reg = 0U; source_reg < 8U; ++source_reg) {
          const M68kDecodeSectionIR *target_section;
          M68kSectionAnalysisIR *target_analysis;
          uint32_t target_section_index;
          uint32_t target_offset;
          uint32_t read_count;
          uint32_t span;
          if (!source_quality_candidate_reads_postincrement_from_reg(candidate, &instruction, source_reg,
              item_size, item_kind) ||
              !state.addr_regs[source_reg].known) {
            continue;
          }
          target_section_index = state.addr_regs[source_reg].target_section_index;
          target_offset = state.addr_regs[source_reg].target_offset;
          if (target_section_index >= decode->section_count || accepted_bytes[target_section_index] == NULL)
            continue;
          target_section = &decode->sections[target_section_index];
          target_analysis = source_analysis_section_by_index(source_analysis, target_section_index);
          ...
          read_count = source_quality_count_consecutive_postincrement_reads(section, accepted_start[decode_index],
            candidate_index, source_reg, item_size, item_kind);
          if (read_count < 2U) continue;
          span = source_quality_pc_relative_lookup_table_span(source_analysis, target_analysis, target_section,
            accepted_bytes[target_section_index], target_offset, item_size);
          if (span < read_count * item_size) continue;
          if (append_pc_relative_lookup_table_item(source_analysis, target_section_index, target_offset, span,
              item_kind, M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_POSTINCREMENT_READ_SEQUENCE,
              (uint32_t)section->section_index, candidate->offset, 0U, M68K_ANALYSIS_REGISTER_NONE, 0U, 0U,
              M68K_ANALYSIS_REGISTER_NONE, 0U, NULL, 0U, 0U, 0U) != 0) {
            return -1;
          }
        }
      }
...
static int source_quality_text_buffer_byte(uint8_t value) {
  return m68k_ir_byte_is_quoted_string_safe(value) || value == 0x0dU || value == 0x0aU || value == 0x09U;
}
...
static int source_quality_relocation_fact_at_offset(const M68kFactIR *facts, size_t section_index,
  ...
  for (fact_index = 0U; fact_index < facts->fact_count; ++fact_index) {
    ...
    if (fact->section_index == section_index && fact->offset == offset &&
        (fact->kind == M68K_FACT_RELOCATION_REF || fact->kind == M68K_FACT_RELOCATION_ANCHOR)) {
      return 1;
    }
  }
  return 0;
...
static int source_quality_api_text_buffer_shape_ok(const M68kSourceAnalysisIR *source_analysis,
    const M68kFactIR *facts, const M68kSectionAnalysisIR *section_analysis, const M68kDecodeSectionIR *section,
    const uint8_t *accepted_bytes, uint32_t offset, uint32_t size) {
  uint32_t cursor;
  uint32_t alpha_count = 0U;
  uint32_t printable_count = 0U;
  int has_space = 0;
  if (source_analysis == NULL || section_analysis == NULL || section == NULL || section->data == NULL ||
      accepted_bytes == NULL || section->section_index > UINT32_MAX || size < 4U ||
      offset > section->size || size > section->size - offset) {
    return 0;
  }
  for (cursor = offset; cursor < offset + size; ++cursor) {
    uint8_t value = section->data[cursor];
    if (!source_quality_text_buffer_byte(value)) return 0;
    if (source_quality_accepted_range_has_code_byte(accepted_bytes, section->size, cursor, 1U)) return 0;
    if (source_quality_relocation_fact_at_offset(facts, section->section_index, cursor)) return 0;
    if (source_quality_existing_structured_data_conflicts_with_api_text_buffer(source_analysis,
        (uint32_t)section->section_index, cursor)) {
      return 0;
    }
    if (source_quality_range_ownership_conflicts_with_api_string(section_analysis, cursor)) return 0;
    if (value == ' ') has_space = 1;
...
static int append_api_string_structured_data_segments(M68kSourceAnalysisIR *source_analysis,
    ...
    uint8_t source_pattern_id, uint8_t trim_terminal_boundary, uint8_t *const *data_target_boundaries) {
  const M68kDecodeSectionIR *target_section;
  M68kSectionAnalysisIR *target_analysis;
  uint32_t cursor;
  uint32_t string_end;
  ...
  if (trim_terminal_boundary && string_size > 1U) {
...
static int append_api_text_buffer_structured_data_segments(M68kSourceAnalysisIR *source_analysis,
    const M68kDecodeIR *decode, const M68kFactIR *facts, uint32_t target_section_index, uint32_t target_offset,
    uint32_t text_size, uint8_t *const *accepted_bytes, uint8_t *const *data_target_boundaries) {
  const M68kDecodeSectionIR *target_section;
  M68kSectionAnalysisIR *target_analysis;
  if (source_analysis == NULL || decode == NULL || target_section_index >= decode->section_count ||
      accepted_bytes == NULL || accepted_bytes[target_section_index] == NULL || text_size == 0U) {
    return 0;
  }
  target_section = &decode->sections[target_section_index];
  target_analysis = source_analysis_section_by_index(source_analysis, target_section_index);
  if (!source_quality_api_text_buffer_shape_ok(source_analysis, facts, target_analysis, target_section,
      accepted_bytes[target_section_index], target_offset, text_size)) {
    return 0;
  }
  return append_api_string_structured_data_segments(source_analysis, decode, target_section_index, target_offset,
...
static int append_write_text_buffer_structured_data_item_for_call(M68kSourceAnalysisIR *source_analysis,
  ...
  if (length == 0U || length > INT32_MAX) return 0;
  {
    ...
    if (buffer_section->data != NULL && buffer_value->target_offset < buffer_section->size &&
        length < buffer_section->size - buffer_value->target_offset &&
        buffer_section->data[buffer_value->target_offset + length] == 0U) {
      return 0;
    }
  }
  return append_api_text_buffer_structured_data_segments(source_analysis, decode, facts,
    buffer_value->target_section_index, buffer_value->target_offset, length, accepted_bytes, data_target_boundaries);
}
...
static int append_platform_call_input_string_structured_data_items_for_section(
  ...
  for (candidate_index = 0U; candidate_index < section->candidate_count; ++candidate_index) {
    ...
    if (call != NULL &&
        append_platform_call_input_string_structured_data_items_for_call(source_analysis, decode, facts, &state, call,
          accepted_bytes, data_target_boundaries) != 0) {
      return -1;
    }
...
static int append_platform_call_input_string_structured_data_items(M68kSourceAnalysisIR *source_analysis,
  ...
  for (section_index = 0U; section_index < source_analysis->section_count; ++section_index) {
    ...
    if (append_platform_call_input_string_structured_data_items_for_section(source_analysis, decode,
        section_analysis, facts, section, accepted_start[decode_index], accepted_bytes, data_target_boundaries) != 0) {
      goto done;
    }
  }
  result = 0;
...
static int append_structured_data_table_entries_for_item(M68kSourceAnalysisIR *source_analysis,
  ...
  for (cursor = 0U; cursor + entry_size <= item->size && item->offset + cursor + entry_size <= section->size;
    ...
    if (item->kind == M68K_ANALYSIS_STRUCTURED_DATA_WORDS && item->has_target) {
      ...
      uint32_t raw_long = m68k_read_u32be(section->data + entry_offset);
      uint32_t target_offset = 0U;
      uint8_t is_pointer_table = (uint8_t)source_quality_structured_data_item_is_pointer_table(item);
      ...
      if (relocation != NULL && relocation->target_section_index <= UINT32_MAX &&
        ...
        if (append_structured_data_table_entry_target(source_analysis, section_analysis, item, entry_index,
          ...
          ((is_pointer_table &&
             (source_quality_pointer_table_value_to_pointer_string_target(source_analysis, section_analysis,
                section->size, raw_long, &target_offset) ||
...
static int append_structured_data_table_entries_for_table_item(M68kSourceAnalysisIR *source_analysis,
    const M68kDecodeIR *decode, const M68kFactIR *facts, uint8_t *const *accepted_start,
    uint8_t *const *accepted_bytes, const M68kAnalysisStructuredDataItem *source_item) {
  M68kAnalysisStructuredDataItem item;
  M68kSectionAnalysisIR *section_analysis;
  const M68kDecodeSectionIR *section;
  ...
  size_t decode_index = 0U;
  uint32_t entry_size;
  if (source_analysis == NULL || decode == NULL || source_item == NULL) return -1;
  item = *source_item;
  m68k_analysis_structured_data_item_refresh_table_metadata(&item);
  if (!item.has_section_index || item.size == 0U ||
      item.table_kind_id == M68K_ANALYSIS_TABLE_KIND_UNKNOWN ||
  ...
  entry_size = m68k_analysis_structured_data_table_entry_size(&item);
  if (entry_size == 0U || item.size < entry_size) return 0;
  section_analysis = source_analysis_section_by_index(source_analysis, item.section_index);
  section = source_quality_decode_section_by_index(decode, item.section_index, &decode_index);
  if (section_analysis == NULL || section == NULL) return 0;
  ...
  return append_structured_data_table_entries_for_item(source_analysis, section, section_accepted_start,
    section_accepted_bytes, section_analysis, &item, facts);
}

static int source_quality_source_items_contain_same_structured_table(
    const M68kSourceAnalysisIR *source_analysis, const M68kAnalysisStructuredDataItem *item) {
  size_t index;
  if (source_analysis == NULL || item == NULL || !item->has_section_index) return 0;
  for (index = 0U; index < source_analysis->structured_data_item_count; ++index) {
    const M68kAnalysisStructuredDataItem *candidate = &source_analysis->structured_data_items[index];
    if (candidate->has_section_index && candidate->section_index == item->section_index &&
        candidate->offset == item->offset && candidate->size == item->size &&
...
static int append_structured_data_table_entries(M68kSourceAnalysisIR *source_analysis,
  ...
  if (decode == NULL) return 0;
  for (index = 0U; index < source_analysis->structured_data_item_count; ++index) {
    if (append_structured_data_table_entries_for_table_item(source_analysis, decode, facts, accepted_start,
        accepted_bytes, &source_analysis->structured_data_items[index]) != 0) {
      return -1;
    }
  }
  for (policy_index = 0U; policy_index < source_analysis->policy.structured_data_item_count; ++policy_index) {
    const M68kAnalysisStructuredDataItem *item = &source_analysis->policy.structured_data_items[policy_index];
    if (source_quality_source_items_contain_same_structured_table(source_analysis, item)) continue;
    if (append_structured_data_table_entries_for_table_item(source_analysis, decode, facts, accepted_start,
        accepted_bytes, item) != 0) {
      return -1;
    }
  }
  return 0;
...
#define SOURCE_QUALITY_CHECK_OK(call_) do { \
  ...
  SOURCE_QUALITY_CHECK_OK(append_indexed_word_dispatch_table_items(source_analysis, decode, accepted_start,
    accepted_bytes));
  SOURCE_QUALITY_CHECK_OK(append_indexed_local_pointer_table_items(source_analysis, decode, accepted_start,
    accepted_bytes));
  SOURCE_QUALITY_CHECK_OK(append_indexed_local_scalar_table_items(source_analysis, decode, accepted_start,
    accepted_bytes));
  SOURCE_QUALITY_CHECK_OK(append_pc_relative_lookup_table_items(source_analysis, decode, accepted_start,
    accepted_bytes));
  SOURCE_QUALITY_CHECK_OK(append_indexed_postincrement_data_table_items(source_analysis, decode, accepted_start,
    accepted_bytes));
  ...
  SOURCE_QUALITY_CHECK_OK(append_generic_string_items(source_analysis, decode, facts, accepted_bytes));
