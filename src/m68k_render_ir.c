...
static void render_asm_structured_raw_data_with_labels(M68kRenderIRPreview *preview,
    const M68kDecodeSectionIR *section, const M68kRenderLookup *lookup, const M68kAnalysisStructuredDataItem *item,
    uint32_t offset, uint32_t size, uint32_t width, const char *comment, uint32_t *io_logical_pc) {
  uint32_t cursor = 0U;
  ...
  while (cursor < size) {
    ...
    if (width == 4U && chunk_size >= 4U && (chunk_size & 3U) == 0U) {
      ...
      render_asm_dc_b_with_policy(preview, section, lookup, current, chunk_size, cursor == 0U ? comment : NULL);
    }
...
static void render_asm_copper_list_words(M68kRenderIRPreview *preview, const M68kRenderLookup *lookup,
  ...
  while (cursor + 4U <= size) {
    ...
    int copper_wait = 0;
    if (cursor != 0U &&
        (lookup_has_renderable_label(lookup, section->section_index, offset + cursor) ||
         lookup_structured_long_table_targets_offset(lookup, section->section_index, offset + cursor))) {
      render_asm_label(preview, lookup, section->section_index, offset + cursor, io_logical_pc);
    }
    if (raw_word_mode) {
    ...
    if (cursor + 2U < size &&
        (lookup_has_renderable_label(lookup, section->section_index, offset + cursor + 2U) ||
         lookup_structured_long_table_targets_offset(lookup, section->section_index, offset + cursor + 2U))) {
...
static void render_asm_structured_data_item(M68kRenderIRPreview *preview, const M68kDecodeSectionIR *section,
  ...
  if (item->kind == M68K_ANALYSIS_STRUCTURED_DATA_LONGS && available >= 4U) {
    render_asm_structured_raw_data_with_labels(preview, section, lookup, item, item->offset, available, 4U,
      comment_text, io_logical_pc);
    if (out_updated_logical_pc != NULL && io_logical_pc != NULL) *out_updated_logical_pc = 1;
  } else if (item->kind == M68K_ANALYSIS_STRUCTURED_DATA_WORDS && available >= 2U) {
    render_asm_structured_raw_data_with_labels(preview, section, lookup, item, item->offset, available, 2U,
      comment_text, io_logical_pc);
    if (out_updated_logical_pc != NULL && io_logical_pc != NULL) *out_updated_logical_pc = 1;
  } else {
    render_asm_structured_raw_data_with_labels(preview, section, lookup, item, item->offset, available, 1U,
      comment_text, io_logical_pc);
    if (out_updated_logical_pc != NULL && io_logical_pc != NULL) *out_updated_logical_pc = 1;
  }
...
static const char *trim_left_local(const char *text) {
  if (text == NULL) return NULL;
  while (*text == ' ' || *text == '\t') ++text;
  return text;
}

static void trim_right_in_place_local(char *text) {
  size_t length;
  if (text == NULL) return;
  length = strlen(text);
...
static int analyze_amiga_instruction_platform_call_candidate(M68kRenderLookup *lookup,
    M68kRenderPlatformState *platform_state, const M68kDecodeIR *decode, uint8_t **accepted_start_all,
    const M68kDecodeSectionIR *section, const uint8_t *accepted_start, const M68kDecodeCandidate *candidate,
    M68kInstructionIR *out_instruction, M68kRenderAmigaVectorResolution *out_resolution) {
  if (out_instruction != NULL) m68k_ir_instruction_init(out_instruction);
  render_amiga_vector_resolution_init(out_resolution);
  if (lookup == NULL || lookup->object == NULL || platform_state == NULL || decode == NULL ||
      ...
      out_instruction == NULL || out_resolution == NULL) {
    return 0;
  }
  if (lookup->object->platform_backend_kind != M68K_PLATFORM_BACKEND_AMIGA_HUNK) return 0;
  if (m68k_decode_candidate_to_instruction(candidate, out_instruction) != 0) return 0;
  attach_known_instruction_relocations(lookup, section->section_index, candidate, out_instruction);
  resolve_amiga_instruction_platform_vectors(lookup, platform_state, decode, accepted_start_all, section,
    accepted_start, candidate, out_instruction, out_resolution);
  return 1;
}

static int record_amiga_instruction_platform_call_facts(M68kRenderIRPreview *preview,
  ...
  M68kRenderAmigaVectorResolution vector_resolution;
  if (preview == NULL) return 0;
  if (!analyze_amiga_instruction_platform_call_candidate(lookup, platform_state, decode, accepted_start_all, section,
      accepted_start, candidate, &instruction, &vector_resolution)) {
    return 0;
  }
...
int m68k_render_ir_build_source_analysis_from_lookup(M68kRenderIRPreview *preview, M68kRenderLookup *lookup,
  ...
  if (render_asm_app_extension_rs_append_layout_facts(preview, lookup, decode, source_analysis) != 0) return -1;
  for (section_index = 0U; section_index < decode->section_count; ++section_index) {
    ...
    if (render_analysis_append_orphan_code_signals_for_section(lookup, section, accepted_start[section_index],
        accepted_bytes[section_index], &section_analysis) != 0) {
      goto fail;
    }
