...
static int render_api_text_buffer_shape_ok(const M68kRenderLookup *lookup, const M68kDecodeSectionIR *section,
  ...
  if (lookup == NULL || section == NULL || section->data == NULL || accepted_bytes == NULL ||
      size < 4U || offset > section->size || size > section->size - offset) {
    return 0;
  }
  for (cursor = offset; cursor < offset + size; ++cursor) {
    uint8_t value = section->data[cursor];
    if (!auto_multiline_text_byte(value)) return 0;
    if (accepted_range_has_code_byte(accepted_bytes, section->size, cursor, 1U) != 0) return 0;
    if (lookup_relocation_at(lookup, section->section_index, cursor) != NULL) return 0;
    {
      M68kRenderRangeOwnershipView range;
...
static int render_lookup_add_api_text_buffer_segments(M68kRenderLookup *lookup, const M68kDecodeSectionIR *section,
    const uint8_t *accepted_bytes, uint32_t offset, uint32_t size) {
  uint32_t cursor;
  uint32_t end;
  if (!render_api_text_buffer_shape_ok(lookup, section, accepted_bytes, offset, size)) return 0;
  cursor = offset;
  end = offset + size;
  while (cursor < end) {
    uint32_t segment_end = end;
    uint32_t probe;
    for (probe = cursor + 1U; probe < end; ++probe) {
      if (render_lookup_api_string_segment_boundary_at(lookup, section->section_index, probe)) {
        segment_end = probe;
        break;
      }
    }
    if (segment_end <= cursor) break;
    if (render_lookup_add_api_string_segment(lookup, section->section_index, cursor,
        segment_end - cursor, M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_API_TEXT_BUFFER) != 0) {
      return -1;
    }
    cursor = segment_end;
  }
...
static int render_lookup_add_write_text_buffer_for_vector(M68kRenderLookup *lookup, const M68kDecodeIR *decode,
    const AmigaOsLibraryVectorInfo *vector, const M68kRenderDataPointerState *state, uint8_t **accepted_bytes,
    const AmigaOsCallInputInfo *inputs, size_t input_count) {
  const AmigaOsCallInputInfo *buffer_input = NULL;
  const AmigaOsCallInputInfo *length_input = NULL;
  const M68kRenderDataPointerValue *buffer_value;
  uint32_t length;
  size_t index;
  if (lookup == NULL || decode == NULL || vector == NULL || state == NULL || accepted_bytes == NULL ||
      inputs == NULL || vector->function_id != AMIGA_OS_FUNCTION_ID_WRITE) {
    return 0;
  }
  for (index = 0U; index < input_count; ++index) {
    if (inputs[index].input_id == AMIGA_OS_SYMBOL_ID_BUFFER_2 &&
        inputs[index].reg_kind == AMIGA_OS_REGISTER_DATA && inputs[index].reg_index < 8U) {
  ...
  if (!buffer_value->known || !buffer_value->exact || buffer_value->section_index >= decode->section_count ||
  ...
  {
    const M68kDecodeSectionIR *buffer_section = &decode->sections[buffer_value->section_index];
    if (buffer_section->data != NULL && buffer_value->offset < buffer_section->size &&
        length < buffer_section->size - buffer_value->offset &&
        buffer_section->data[buffer_value->offset + length] == 0U) {
      return 0;
    }
  }
  return render_lookup_add_api_text_buffer_segments(lookup, &decode->sections[buffer_value->section_index],
...
static int render_lookup_import_source_analysis_structured_data_item(M68kRenderLookup *lookup,
  ...
  if (lookup == NULL || source_item == NULL || !source_item->has_section_index || source_item->size == 0U ||
      source_item->section_index >= lookup->section_count) {
    return 0;
  }
  section_index = source_item->section_index;
  if (lookup_range_ownership_at_offset(lookup, section_index, source_item->offset, &existing_range) &&
      !structured_data_item_is_untyped_bytes_placeholder_local(existing_range.structured_item)) {
    return 0;
  }
  for (index = 0U; index < lookup->auto_structured_data_item_count; ++index) {
...
static int candidate_indexed_read_dest_address_reg(const M68kDecodeCandidate *candidate,
  ...
  if (candidate == NULL || instruction == NULL || out_dest_reg == NULL) return 0;
  metadata = m68k_sim_metadata_for_instruction(instruction);
  if (metadata == NULL) return 0;
...
static int candidate_dest_direct_register_kind_local(const M68kDecodeCandidate *candidate,
  ...
  metadata = m68k_sim_metadata_for_instruction(instruction);
  if (metadata == NULL || metadata->dest_operand_index >= candidate->operand_count ||
      metadata->dest_operand_index >= instruction->operand_count) {
    return 0;
  }
  dest_index = metadata->dest_operand_index;
  if (operand_is_data_register_local(&instruction->operands[dest_index], &reg) && reg < 8U) {
    *out_register_kind = M68K_ANALYSIS_REGISTER_DATA;
    *out_register = reg;
    return 1;
  }
  if (operand_address_register_index_local(&instruction->operands[dest_index], &reg) && reg < 8U) {
...
static uint32_t count_word_relative_table_label_targets(const M68kRenderLookup *lookup,
    size_t target_section_index, uint32_t target_base_offset, const M68kDecodeSectionIR *table_section,
    uint32_t table_offset, uint32_t span) {
  uint32_t cursor;
  uint32_t count = 0U;
  if (lookup == NULL || table_section == NULL || table_section->data == NULL ||
      target_section_index >= lookup->section_count || table_offset > table_section->size ||
      span > table_section->size - table_offset) {
    return 0U;
  }
  for (cursor = 0U; cursor + 2U <= span; cursor += 2U) {
    ...
    if (lookup_has_label(lookup, target_section_index, (uint32_t)target_offset)) ++count;
  }
  return count;
}

static uint32_t word_relative_table_target_upper_bound(const M68kDecodeSectionIR *table_section,
    uint32_t table_offset, uint32_t span, const M68kDecodeSectionIR *target_section,
    uint32_t target_base_offset, uint32_t target_offset) {
  uint32_t cursor;
...
static uint32_t auto_renderable_word_relative_table_bounded_string_span(const M68kRenderLookup *lookup,
    const M68kDecodeSectionIR *section, const uint8_t *accepted_bytes, uint32_t offset, uint32_t upper_bound) {
  uint32_t cursor;
  uint32_t span;
  if (lookup == NULL || section == NULL || section->data == NULL || offset >= section->size ||
      upper_bound <= offset || upper_bound > section->size) {
    return 0U;
  }
...
static uint32_t add_word_relative_table_string_targets(M68kRenderLookup *lookup, const M68kDecodeIR *decode,
  ...
  if (table_section->data == NULL || target_section->data == NULL ||
      table_offset > table_section->size || span > table_section->size - table_offset ||
  ...
  render_lookup_set_auto_structured_data_item_target(lookup, table_section_index, table_offset,
    target_section_index, target_base_offset);
  render_lookup_set_auto_structured_data_item_consumer(lookup, table_section_index, table_offset,
    consumer_section_index, consumer_offset);
  render_lookup_set_auto_structured_data_item_source_pattern(lookup, table_section_index, table_offset,
    M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_WORD_OFFSET_STRING_TABLE);
  render_lookup_mark_label(lookup, target_section_index, target_base_offset);
  render_lookup_mark_label_target_ref(lookup, target_section_index, target_base_offset);
  render_lookup_mark_label_statement_ref(lookup, target_section_index, target_base_offset);
  return string_count;
}
...
static int candidate_next_is_indexed_control_through_reg(const M68kDecodeSectionIR *section,
    const uint8_t *accepted_start, const M68kDecodeCandidate *candidate, const M68kRenderDataPointerState *state,
    uint8_t reg, uint8_t index_is_address, size_t table_section_index, uint32_t table_offset,
    uint32_t *out_next_offset, uint32_t *out_target_base_offset) {
  const M68kDecodeCandidate *next_candidate;
  M68kInstructionIR next_instruction;
  const M68kSimFormMetadata *metadata;
  size_t operand_index = 0U;
...
static int candidate_next_is_indexed_control_through_data_reg(const M68kDecodeSectionIR *section,
  ...
  return candidate_next_is_indexed_control_through_reg(section, accepted_start, candidate, state, reg, 0U,
    table_section_index, table_offset, out_next_offset, out_target_base_offset);
}

static int candidate_next_is_indexed_control_through_address_reg(const M68kDecodeSectionIR *section,
    const uint8_t *accepted_start, const M68kDecodeCandidate *candidate, const M68kRenderDataPointerState *state,
    uint8_t reg, size_t table_section_index, uint32_t table_offset, uint32_t *out_next_offset,
    uint32_t *out_target_base_offset) {
  return candidate_next_is_indexed_control_through_reg(section, accepted_start, candidate, state, reg, 1U,
    table_section_index, table_offset, out_next_offset, out_target_base_offset);
}
...
static uint32_t skip_leading_zero_word_dispatch_entries(const M68kDecodeIR *decode,
    size_t section_index, uint32_t table_offset, uint32_t base_offset) {
  const M68kDecodeSectionIR *section;
...
static uint32_t scan_keyed_long_relative_dispatch_table_span(const M68kRenderLookup *lookup,
    const M68kDecodeIR *decode, uint8_t **accepted_start, uint8_t **accepted_bytes,
    size_t table_section_index, uint32_t table_offset, size_t base_section_index, uint32_t base_offset) {
  const M68kDecodeSectionIR *table_section;
  const M68kDecodeSectionIR *base_section;
  ...
  for (cursor = table_offset; cursor + 4U <= table_section->size; cursor += 4U) {
    ...
    int64_t target64;
    if (cursor != table_offset && lookup_has_label(lookup, table_section_index, cursor)) break;
    if (accepted_range_has_code_byte(accepted_bytes[table_section_index], table_section->size, cursor, 4U))
      break;
    entry = m68k_read_u32be(table_section->data + cursor);
    if (entry == 0U) break;
    displacement = (int32_t)(int16_t)((entry >> 16U) & 0xFFFFU);
    if (displacement < -M68K_RENDER_LOOKUP_WORD_DISPATCH_LOCAL_LIMIT ||
        displacement > M68K_RENDER_LOOKUP_WORD_DISPATCH_LOCAL_LIMIT) {
      break;
    }
    target64 = (int64_t)(uint64_t)base_offset + (int64_t)displacement;
    if (target64 < 0 || target64 >= (int64_t)(uint64_t)base_section->size ||
    ...
    if (!accepted_start[base_section_index][(uint32_t)target64] ||
        !lookup_has_label(lookup, base_section_index, (uint32_t)target64)) {
      break;
    }
    ++target_count;
    if (table_section_index == base_section_index && (uint32_t)target64 > table_offset &&
...
static int candidate_loads_postincrement_long_table_to_data_reg(const M68kDecodeCandidate *candidate,
  ...
  if (source_index >= 4U ||
      metadata->operand_access_kinds[source_index] != M68K_SIM_ACCESS_MEMORY_READ ||
      !operand_is_address_postincrement_local(&instruction->operands[source_index], &source_reg) ||
      !operand_is_data_register_local(&instruction->operands[dest_index], &dest_reg) ||
      source_reg >= 8U || !state->addr_regs[source_reg].known) {
    return 0;
  }
  *out_table_section_index = state->addr_regs[source_reg].section_index;
  *out_table_offset = state->addr_regs[source_reg].offset;
  *out_dest_reg = dest_reg;
  return 1;
}

static int candidate_is_swap_data_reg_local(const M68kDecodeCandidate *candidate, uint8_t reg) {
  M68kInstructionIR instruction;
  const M68kSimFormMetadata *metadata;
  uint8_t operand_reg = 0U;
  ...
  return metadata != NULL && metadata->operation_type == M68K_SIM_OP_SWAP_WORDS &&
    operand_is_data_register_local(&instruction.operands[0], &operand_reg) && operand_reg == reg;
}
...
typedef struct M68kRenderKeyedLongDispatchValue {
  uint8_t known;
  uint8_t swapped;
  size_t table_section_index;
  uint32_t table_offset;
} M68kRenderKeyedLongDispatchValue;

typedef struct M68kRenderKeyedLongDispatchState {
  M68kRenderKeyedLongDispatchValue data_regs[8];
} M68kRenderKeyedLongDispatchState;

static void keyed_long_dispatch_state_clear(M68kRenderKeyedLongDispatchState *state) {
  if (state == NULL) return;
  memset(state, 0, sizeof(*state));
}

static void keyed_long_dispatch_state_clear_reg(M68kRenderKeyedLongDispatchState *state, uint8_t reg) {
  if (state == NULL || reg >= 8U) return;
  memset(&state->data_regs[reg], 0, sizeof(state->data_regs[reg]));
}

static void keyed_long_dispatch_state_set_loaded(M68kRenderKeyedLongDispatchState *state, uint8_t reg,
    size_t table_section_index, uint32_t table_offset) {
  M68kRenderKeyedLongDispatchValue *value;
  if (state == NULL || reg >= 8U) return;
...
static void keyed_long_dispatch_state_mark_swapped(M68kRenderKeyedLongDispatchState *state, uint8_t reg) {
  if (state == NULL || reg >= 8U || !state->data_regs[reg].known) return;
  state->data_regs[reg].swapped = 1U;
}

static void keyed_long_dispatch_state_kill_data_writes(M68kRenderKeyedLongDispatchState *state,
    const M68kInstructionIR *instruction, uint8_t preserve_reg) {
  const M68kSimFormMetadata *metadata;
  size_t operand_index;
  if (state == NULL || instruction == NULL) return;
  metadata = m68k_sim_metadata_for_instruction(instruction);
  if (metadata == NULL) {
    keyed_long_dispatch_state_clear(state);
    return;
  }
  for (operand_index = 0U; operand_index < instruction->operand_count && operand_index < 4U; ++operand_index) {
    const M68kOperandIR *operand = &instruction->operands[operand_index];
    ...
    if (metadata->operand_access_kinds[operand_index] == M68K_SIM_ACCESS_REGISTER_LIST_WRITE &&
      ...
      for (bit = 0U; bit < 8U; ++bit) {
        if ((operand->value.value & (1UL << bit)) != 0U && bit != preserve_reg)
          keyed_long_dispatch_state_clear_reg(state, bit);
      }
      continue;
    }
    if (!instruction_operand_writes_register_from_metadata(instruction, operand_index)) continue;
    if (operand_is_data_register_local(operand, &reg) && reg < 8U && reg != preserve_reg)
      keyed_long_dispatch_state_clear_reg(state, reg);
  }
}

static void keyed_long_dispatch_state_update_after_instruction(M68kRenderKeyedLongDispatchState *state,
    const M68kDecodeCandidate *candidate, const M68kInstructionIR *instruction,
    const M68kRenderDataPointerState *pointer_state) {
  const M68kSimFormMetadata *metadata;
  size_t table_section_index = 0U;
  uint32_t table_offset = 0U;
  uint8_t reg = 0U;
  ...
  if (candidate_loads_postincrement_long_table_to_data_reg(candidate, instruction, pointer_state,
    ...
    for (scan_reg = 0U; scan_reg < 8U; ++scan_reg) {
      if (state->data_regs[scan_reg].known && candidate_is_swap_data_reg_local(candidate, scan_reg)) {
  ...
  keyed_long_dispatch_state_kill_data_writes(state, instruction, preserve_reg);
  if (loaded) keyed_long_dispatch_state_set_loaded(state, reg, table_section_index, table_offset);
  else if (swapped) keyed_long_dispatch_state_mark_swapped(state, reg);
...
static int render_lookup_maybe_add_keyed_long_dispatch_table(M68kRenderLookup *lookup, const M68kDecodeIR *decode,
  ...
  for (reg = 0U; reg < 8U; ++reg) {
    ...
    span = scan_keyed_long_relative_dispatch_table_span(lookup, decode, accepted_start, accepted_bytes,
      value->table_section_index, value->table_offset, value->table_section_index, value->table_offset);
    if (span == 0U) continue;
    if (render_lookup_add_auto_structured_data_item(lookup, value->table_section_index, value->table_offset,
        span, M68K_ANALYSIS_STRUCTURED_DATA_ROLE_LOOKUP_TABLE, M68K_ANALYSIS_STRUCTURED_DATA_LONGS) != 0) {
      return -1;
    }
    render_lookup_set_auto_structured_data_item_consumer(lookup, value->table_section_index,
      value->table_offset, section_index, candidate->offset);
    render_lookup_set_auto_structured_data_item_target(lookup, value->table_section_index, value->table_offset,
      value->table_section_index, value->table_offset);
    render_lookup_set_auto_structured_data_item_source_pattern(lookup, value->table_section_index,
      value->table_offset, M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_KEYED_LONG_RELATIVE_DISPATCH);
  }
  return 0;
}
...
static int candidate_loads_indexed_word_table_to_address_reg(const M68kDecodeCandidate *candidate,
    const M68kInstructionIR *instruction, const M68kRenderDataPointerState *state,
    size_t *out_table_section_index, uint32_t *out_table_offset, uint8_t *out_dest_reg,
    uint8_t *out_index_register_kind, uint8_t *out_index_register) {
  const M68kSimFormMetadata *metadata = NULL;
  ...
  if (!operand_address_register_index_local(&instruction->operands[dest_index], &dest_reg)) return 0;
  if (!indexed_control_operand_target(candidate, (uint32_t)source_index, &instruction->operands[source_index],
      state, out_table_section_index, out_table_offset)) {
    return 0;
  }
...
static int render_lookup_infer_indexed_word_dispatch_tables(M68kRenderLookup *lookup,
  ...
  for (section_index = 0U; section_index < decode->section_count; ++section_index) {
    ...
    M68kRenderKeyedLongDispatchState keyed_state;
    size_t candidate_index;
    data_pointer_state_clear_all(&state);
    keyed_long_dispatch_state_clear(&keyed_state);
    ...
    for (candidate_index = 0U; candidate_index < section->candidate_count; ++candidate_index) {
      ...
      }
      {
        ...
        size_t addr_load_table_section = 0U;
        ...
        loads_dispatch = candidate_loads_indexed_word_table_to_data_reg(candidate, &instruction, &state,
          &load_table_section, &load_table_offset, &dest_reg, &load_index_register_kind, &load_index_register);
        ...
        loads_address_dispatch = candidate_loads_indexed_word_table_to_address_reg(candidate, &instruction, &state,
          &addr_load_table_section, &addr_load_table_offset, &addr_dest_reg, &addr_load_index_register_kind,
          &addr_load_index_register);
        next_address_indexed_dispatch = loads_address_dispatch ?
          candidate_next_is_indexed_control_through_address_reg(section, accepted_start[section_index], candidate,
        ...
        if ((loads_dispatch && next_indexed_dispatch) ||
          ...
          size_t dispatch_table_section = loads_dispatch ? load_table_section : addr_load_table_section;
          ...
          if (dispatch_table_offset < target_base_offset &&
            ...
            if (render_lookup_add_auto_structured_data_item(lookup, dispatch_table_section, dispatch_table_offset,
                leading_size, M68K_ANALYSIS_STRUCTURED_DATA_ROLE_LOOKUP_TABLE,
                M68K_ANALYSIS_STRUCTURED_DATA_WORDS) != 0) {
              return -1;
            }
            render_lookup_set_auto_structured_data_item_consumer(lookup, dispatch_table_section, dispatch_table_offset,
            ...
            render_lookup_set_auto_structured_data_item_source_pattern(lookup, dispatch_table_section,
              dispatch_table_offset, M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_INDEXED_WORD_DISPATCH);
          }
          table_size = scan_indexed_word_dispatch_table_span(lookup, decode, accepted_start, accepted_bytes,
            dispatch_table_section, target_base_offset, dispatch_table_section, target_base_offset);
          if (table_size != 0U &&
              render_lookup_add_auto_structured_data_item(lookup, dispatch_table_section, target_base_offset,
                table_size, M68K_ANALYSIS_STRUCTURED_DATA_ROLE_LOOKUP_TABLE,
                M68K_ANALYSIS_STRUCTURED_DATA_WORDS) != 0) {
            return -1;
          }
          if (table_size != 0U) {
            render_lookup_set_auto_structured_data_item_consumer(lookup, dispatch_table_section, target_base_offset,
              section_index, consumer_offset);
            render_lookup_set_auto_structured_data_item_consumer_registers(lookup, dispatch_table_section,
            ...
            render_lookup_set_auto_structured_data_item_target(lookup, dispatch_table_section, target_base_offset,
              dispatch_table_section, target_base_offset);
            render_lookup_set_auto_structured_data_item_source_pattern(lookup, dispatch_table_section,
              target_base_offset, M68K_ANALYSIS_STRUCTURED_DATA_SOURCE_PATTERN_INDEXED_WORD_DISPATCH);
          }
        }
      ...
      keyed_long_dispatch_state_update_after_instruction(&keyed_state, candidate, &instruction, &state);
  ...
  for (section_index = 0U; section_index < decode->section_count; ++section_index) {
    ...
    for (candidate_index = 0U; candidate_index < section->candidate_count; ++candidate_index) {
      ...
      if (structured_data_kind_for_candidate_size(candidate, &item_size, &item_kind) &&
        ...
        uint32_t span = 0U;
        uint8_t table_offset_is_code = (uint8_t)accepted_range_has_code_byte(accepted_bytes[target_section_index],
          decode->sections[target_section_index].size, table_offset, item_size);
        uint8_t dest_reg = 0U;
        uint8_t target_register_kind = M68K_ANALYSIS_REGISTER_NONE;
        uint8_t target_register = 0U;
        uint8_t target_register_known = (uint8_t)candidate_dest_direct_register_kind_local(candidate, &instruction,
          &target_register_kind, &target_register);
        if (!table_offset_is_code &&
            !lookup_range_has_interior_label(lookup, target_section_index, table_offset, item_size)) {
          span = scan_indexed_local_pointer_table_span(lookup, decode, accepted_start, accepted_bytes,
            target_section_index, table_offset);
        }
        if (span == 0U && table_offset_is_code &&
            ...
            item_size <= decode->sections[target_section_index].size - (table_offset + item_size) &&
            !accepted_range_has_code_byte(accepted_bytes[target_section_index],
              decode->sections[target_section_index].size, table_offset + item_size, item_size) &&
            !lookup_range_has_interior_label(lookup, target_section_index, table_offset + item_size, item_size)) {
        ...
        if (span != 0U) {
          render_lookup_set_auto_structured_data_item_consumer(lookup, target_section_index, table_offset,
            section_index, candidate->offset);
          render_lookup_set_auto_structured_data_item_consumer_registers(lookup, target_section_index, table_offset,
            1U, index_register_kind, index_register,
            target_register_known, target_register_kind, target_register);
...
typedef struct M68kRenderWordOffsetValue {
  uint8_t known;
  size_t table_section_index;
  uint32_t table_offset;
} M68kRenderWordOffsetValue;

typedef struct M68kRenderWordOffsetState {
  M68kRenderWordOffsetValue data_regs[8];
} M68kRenderWordOffsetState;

static int candidate_indexed_word_read_to_data_reg(const M68kDecodeSectionIR *section,
    const M68kDecodeCandidate *candidate, const M68kInstructionIR *instruction,
    const M68kRenderDataPointerState *state, size_t *out_table_section_index, uint32_t *out_table_offset,
  ...
  if (section == NULL || candidate == NULL || instruction == NULL || state == NULL ||
      ...
      !structured_data_kind_for_candidate_size(candidate, &item_size, &item_kind) || item_size != 2U ||
  ...
  dest_index = metadata->dest_operand_index;
  if (!operand_is_data_register_local(&instruction->operands[dest_index], &dest_reg) || dest_reg >= 8U) return 0;
  *out_dest_reg = dest_reg;
  return 1;
}

static int candidate_adds_word_offset_to_known_base(const M68kDecodeCandidate *candidate,
    const M68kInstructionIR *instruction, const M68kRenderDataPointerState *data_state,
    const M68kRenderWordOffsetState *word_state, size_t *out_table_section_index, uint32_t *out_table_offset,
  ...
  if (instruction->mnemonic_id == M68K_ASM_MNEMONIC_ADD && instruction->size_suffix == 'l' &&
      operand_address_register_index_local(&instruction->operands[source_index], &addr_reg) &&
      operand_is_data_register_local(&instruction->operands[dest_index], &data_reg) &&
    ...
    *out_base_section_index = data_state->addr_regs[addr_reg].section_index;
    *out_base_offset = data_state->addr_regs[addr_reg].offset;
    return 1;
  }
  if (instruction->mnemonic_id == M68K_ASM_MNEMONIC_ADDA && instruction->size_suffix == 'w' &&
      operand_is_data_register_local(&instruction->operands[source_index], &data_reg) &&
      operand_address_register_index_local(&instruction->operands[dest_index], &addr_reg) &&
      addr_reg < 8U && data_reg < 8U && data_state->addr_regs[addr_reg].known &&
      word_state->data_regs[data_reg].known) {
    *out_table_section_index = word_state->data_regs[data_reg].table_section_index;
    *out_table_offset = word_state->data_regs[data_reg].table_offset;
...
static void word_offset_state_clear_written_data_reg(M68kRenderWordOffsetState *state,
  ...
  if (state == NULL || instruction == NULL) return;
  metadata = m68k_sim_metadata_for_instruction(instruction);
  if (metadata == NULL || metadata->dest_operand_index >= instruction->operand_count) return;
  dest_index = metadata->dest_operand_index;
  if (operand_is_data_register_local(&instruction->operands[dest_index], &dest_reg) && dest_reg < 8U)
    state->data_regs[dest_reg].known = 0U;
}

static int render_lookup_infer_indexed_local_scalar_tables(M68kRenderLookup *lookup, const M68kDecodeIR *decode,
    uint8_t **accepted_start, uint8_t **accepted_bytes) {
  size_t section_index;
  if (lookup == NULL || decode == NULL || accepted_start == NULL || accepted_bytes == NULL) return 0;
  for (section_index = 0U; section_index < decode->section_count; ++section_index) {
    const M68kDecodeSectionIR *section = &decode->sections[section_index];
    M68kRenderDataPointerState state;
    M68kRenderWordOffsetState word_state;
    size_t candidate_index;
    data_pointer_state_clear_all(&state);
    ...
    for (candidate_index = 0U; candidate_index < section->candidate_count; ++candidate_index) {
      ...
      uint32_t item_size = 0U;
      uint8_t item_kind = 0U;
      size_t string_base_section_index = 0U;
      uint32_t string_base_offset = 0U;
      ...
      if ((candidate_adds_word_offset_to_known_base(candidate, &instruction, &state, &word_state,
          ...
          accepted_bytes[target_section_index] != NULL &&
          target_offset <= decode->sections[target_section_index].size) {
        uint32_t span = scan_indexed_word_relative_data_table_span(lookup, &decode->sections[target_section_index],
          accepted_bytes[target_section_index], target_offset, &decode->sections[string_base_section_index],
          string_base_offset);
        if (span != 0U &&
      ...
      if (instruction.mnemonic_id == M68K_ASM_MNEMONIC_ADDA &&
          structured_data_kind_for_candidate_size(candidate, &item_size, &item_kind) &&
          item_size == 2U &&
          item_kind == M68K_ANALYSIS_STRUCTURED_DATA_WORDS &&
          candidate_indexed_read_from_known_local_base(section, candidate, &instruction, &state,
            &target_section_index, &target_offset, NULL, NULL)) {
      ...
      if (candidate_indexed_word_read_to_data_reg(section, candidate, &instruction, &state,
...
static int candidate_reads_postincrement_from_reg(const M68kDecodeCandidate *candidate,
  ...
  if (candidate == NULL || instruction == NULL ||
      !structured_data_kind_for_candidate_size(candidate, &candidate_item_size, &candidate_item_kind) ||
  ...
  for (operand_index = 0U; operand_index < candidate->operand_count && operand_index < instruction->operand_count &&
    ...
    uint8_t source_reg = 0U;
    if (metadata->operand_access_kinds[operand_index] == M68K_SIM_ACCESS_MEMORY_READ &&
        operand_is_address_postincrement_local(&instruction->operands[operand_index], &source_reg) &&
        source_reg == reg) {
      return 1;
    }
  }
...
static uint32_t count_consecutive_postincrement_reads(const M68kDecodeSectionIR *section,
  ...
  for (candidate_index = first_candidate_index; candidate_index < section->candidate_count; ++candidate_index) {
    const M68kDecodeCandidate *candidate = &section->candidates[candidate_index];
    M68kInstructionIR instruction;
    if (!candidate_is_accepted_start(section, accepted_start, candidate) ||
        m68k_decode_candidate_to_instruction(candidate, &instruction) != 0 ||
        !candidate_reads_postincrement_from_reg(candidate, &instruction, reg, item_size, item_kind)) {
      break;
    }
    ++count;
  }
...
static int render_lookup_infer_indexed_postincrement_data_tables(M68kRenderLookup *lookup,
    const M68kDecodeIR *decode, uint8_t **accepted_start, uint8_t **accepted_bytes) {
  size_t section_index;
  if (lookup == NULL || decode == NULL || accepted_start == NULL || accepted_bytes == NULL) return 0;
  for (section_index = 0U; section_index < decode->section_count; ++section_index) {
    ...
    for (candidate_index = 0U; candidate_index < section->candidate_count; ++candidate_index) {
      ...
      if (!candidate_is_accepted_start(section, accepted_start[section_index], candidate)) continue;
      if (m68k_decode_candidate_to_instruction(candidate, &instruction) != 0) continue;
      if (structured_data_kind_for_candidate_size(candidate, &item_size, &item_kind) && item_size > 1U) {
        for (source_reg = 0U; source_reg < 8U; ++source_reg) {
          size_t target_section_index;
          uint32_t target_offset;
          if (!candidate_reads_postincrement_from_reg(candidate, &instruction, source_reg, item_size, item_kind) ||
          ...
          }
          {
            ...
            span = pc_relative_lookup_table_span(lookup, &decode->sections[target_section_index],
              accepted_bytes[target_section_index], target_offset, item_size);
...
static int render_lookup_infer_pc_relative_lookup_scalars(M68kRenderLookup *lookup, const M68kDecodeIR *decode,
  ...
  for (section_index = 0U; section_index < decode->section_count; ++section_index) {
    ...
    for (candidate_index = 0U; candidate_index < section->candidate_count; ++candidate_index) {
      ...
      if (m68k_decode_candidate_to_instruction(candidate, &instruction) != 0) continue;
      metadata = m68k_sim_metadata_for_instruction(&instruction);
      if (metadata != NULL && structured_data_kind_for_candidate_size(candidate, &item_size, &item_kind)) {
        for (target_index = 0U; target_index < candidate->target_count; ++target_index) {
          const M68kDecodeTarget *target = &candidate->targets[target_index];
          const M68kOperandIR *operand;
          uint8_t shape;
...
int m68k_analysis_render_lookup_run_platform_passes(M68kRenderLookup *lookup, const M68kDecodeIR *decode,
  ...
  if (render_lookup_infer_indexed_local_pointer_tables(lookup, decode, accepted_start, accepted_bytes) != 0)
    return -1;
  if (render_lookup_infer_indexed_local_scalar_tables(lookup, decode, accepted_start, accepted_bytes) != 0)
    return -1;
