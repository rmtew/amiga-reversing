...
META = {'sources': [{'file': 'D:/NDK/NDK_3.1/INCLUDES&LIBS/INCLUDE_H/DOS/DOSHUNKS.H',
              ...
              'provides': ['hunk_types', 'ext_types', 'memory_flags', 'compatibility_notes']},
             {'file': 'NDK_3.1/EXAMPLES1/PCMCIA/AMIGAXIP/LOADSEG.ASM',
              'type': 'official',
              'description': 'NDK 3.1 reference LoadSeg implementation — defines wire format for '
...
HUNK_TYPES = {'HUNK_UNIT': {'id': 999, 'description': '', 'notes': 'hunk types'},
 ...
 'HUNK_RELOC32SHORT': {'id': 1020,
                       'description': '',
                       'notes': 'Note: V37 LoadSeg uses 1015 (HUNK_DREL32) by mistake.  This will '
                                'continue to be supported in future versions, since HUNK_DREL32 is '
                                'illegal in load files anyways.  Future versions will support both '
                                '1015 and 1020, though anything that should be usable under V37 '
                                'should use 1015.'},
 'HUNK_RELRELOC32': {'id': 1021,
                     'description': '',
                     'notes': 'see ext_xxx below.  New for V39 (note that LoadSeg only handles '
                              'RELRELOC32).'},
...
MEMORY_TYPE_CODES = {'0': {'name': 'ANY', 'description': 'Any available memory'},
 ...
 '3': {'name': 'EXTENDED', 'description': 'Extended: next ULONG has exec memory attrs'}}
EXT_TYPE_CATEGORIES = {'definition_range': [0, 3],
 'reference_range': [129, 139],
 'boundary': 128,
 'citation': 'DOSHUNKS.H: definitions 0-3, references 129-139; boundary at 128 (bit 7)'}
COMPATIBILITY_NOTES = [{'topic': 'HUNK_DREL32_as_RELOC32SHORT',
  'text': 'Note: V37 LoadSeg uses 1015 (HUNK_DREL32) by mistake.  This will continue to be '
          'supported in future versions, since HUNK_DREL32 is illegal in load files anyways.  '
          'Future versions will support both 1015 and 1020, though anything that should be usable '
          'under V37 should use 1015.',
...
RELOC_FORMATS = {'long': {'description': 'Standard 32-bit reloc format',
          'fields': [{'name': 'count',
                     ...
                     {'name': 'target_hunk', 'type': 'ULONG', 'note': 'Hunk index to add base of'},
                     {'name': 'offsets',
                      'type': 'ULONG[]',
          ...
          'applies_to': ['HUNK_RELOC32',
                         ...
                         'HUNK_RELRELOC32',
                         'HUNK_ABSRELOC16'],
          'citation': 'LOADSEG.ASM lines 238-261: lssHunkReloc32 uses GetLong (ULONG) for count, '
                      'target, and each offset'},
 'short': {'description': 'Compact 16-bit reloc format',
           ...
           'applies_to': ['HUNK_RELOC32SHORT'],
           'citation': 'DOSHUNKS.H lines 42-47: V37 LoadSeg uses HUNK_DREL32 (1015) for this '
                       'format in load files. HUNK_DREL32 is illegal in load files, so 1015 '
                       'unambiguously means short relocs in executables.'}}
RELOCATION_SEMANTICS = {'HUNK_RELOC32': (4, RelocMode.ABSOLUTE),
 'HUNK_RELOC32SHORT': (4, RelocMode.ABSOLUTE),
 'HUNK_RELOC16': (2, RelocMode.PC_RELATIVE),
 'HUNK_RELOC8': (1, RelocMode.PC_RELATIVE),
 'HUNK_DREL32': (4, RelocMode.DATA_RELATIVE),
 'HUNK_DREL16': (2, RelocMode.DATA_RELATIVE),
 'HUNK_DREL8': (1, RelocMode.DATA_RELATIVE),
 'HUNK_RELRELOC32': (4, RelocMode.PC_RELATIVE),
 'HUNK_ABSRELOC16': (2, RelocMode.ABSOLUTE)}
HUNK_CONTENT_FORMATS = {'HUNK_HEADER': {'fields': [{'name': 'resident_libs',
                             'type': 'ULONG[]',
                             'note': 'Sequence of BSTRs terminated by 0; LoadSeg fails if any '
                                     'present'},
                            ...
                            {'name': 'hunk_sizes',
                             'type': 'ULONG[]',
                             'note': '(last-first+1) entries; bits 30-31 = CHIP/FAST memory '
                                     'flags'}],
 ...
 'HUNK_CODE': {'fields': [{'name': 'size_longs',
               ...
               'citation': 'LOADSEG.ASM lines 188-210: lssHunkCode/lssHunkData share the same '
 ...
 'HUNK_SYMBOL': {'fields': [{'name': 'name_longs',
                            ...
                            {'name': 'name', 'type': 'UBYTE[]', 'note': 'name_longs × 4 bytes'},
 ...
 'HUNK_DEBUG': {'fields': [{'name': 'size_longs', 'type': 'ULONG'},
                ...
                'sub_formats': {'LINE': {'magic': 1279872581,
                                         ...
                                         'fields': [{'name': 'magic',
                                                    ...
                                                    {'name': 'filename',
                                                     'type': 'BSTR',
                                                     'note': 'Source filename (ULONG len + len*4 '
                                                             'bytes)'},
                                                    {'name': 'entries',
                                                     'type': 'ARRAY',
                                                     'element': [{'name': 'line', 'type': 'ULONG'},
                                                                 {'name': 'offset',
                                                                  'type': 'ULONG',
 ...
 'HUNK_EXT': {'fields': [{'name': 'type_and_len',
                          'type': 'ULONG',
                          'note': 'Bits 31-24 = ext sub-type, bits 23-0 = name_len (in longwords). '
                                  '0 = terminator.'},
                         {'name': 'name',
                          'type': 'UBYTE[]',
