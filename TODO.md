...
- [ ] Get vasm source, build locally (ext/vasm/)
- [ ] Document supported directives, pseudo-ops, and hunk output capabilities
- [ ] Write minimal test programs to verify round-trip: source → assemble → binary
...
- [ ] Build M68K disassembler — test-driven, using knowledge base (instructions, hardware registers, OS calls)
- [ ] Build test oracle — assemble known source → binary → disassemble → reassemble → binary diff; drives error detection in data, tools, and references simultaneously
